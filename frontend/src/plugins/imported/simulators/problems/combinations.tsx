...
function record({ n, k }: CombInput): Frame<CombState>[] {
  ...
  const backtrack = (start: number) => {
    if (cur.length === k) {
      ...
      emit(
        'RECORD',
        `+${fmt(cur)}`,
        `cur has ${k} numbers — record the combination ${fmt(cur)} (${results.length} so far).`,
        { pick: null },
        'good',
      );
      return;
    ...
    for (let v = start; v <= n; v++) {
      ...
      emit(
        'CHOOSE',
        `pick ${v}`,
        `Pick ${v} and recurse on values greater than ${v}. cur = ${fmt(cur)}.`,
        { pick: v },
      );
      backtrack(v + 1);
      cur.pop();
      emit(
        'BACKTRACK',
        `undo ${v}`,
        `Backtrack: remove ${v} so the next branch tries a different value. cur = ${fmt(cur)}.`,
        { pick: v },
      );
    }
  };
  ...
  emit(
    ...
    `All branches explored — ${results.length} combinations of ${k} from 1..${n}.`,
    { pick: null, done: true },
    'good',
  );
  return frames;
...
function View({ frame }: PluginViewProps<CombState>) {
  ...
  if (s.pick !== null)
    pointers.push({ i: s.pick - 1, label: 'pick', tone: 'warn', place: 'above' });
  const tone = (i: number) => {
      ...
      rail={
        <>
          ...
          {s.done && (
            <RailResult
              label="total"
              value={`${s.results.length} / ${choose(s.n, s.k)}`}
              tone="good"
            />
          )}
        </>
