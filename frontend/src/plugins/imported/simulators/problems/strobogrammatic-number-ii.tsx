...
function record({ n }: StroboInput): Frame<StroboState>[] {  const path: string[] = Array.from({ length: n }, () => '·');
  ...
  emit('INIT', `n=${n}`, `Build length-${n} strobogrammatic numbers (same upside-down: 0↔0, 1↔1, 6↔9, 8↔8, 9↔6) by filling outer pairs inward. No leading zero when n>1; the middle slot (odd n) takes 0, 1, or 8.`, { low: 0, high: n - 1 });

  const backtrack = (low: number, high: number) => {
    if (low > high) {
      const s = path.join('');
      results.push(s);
      emit('RECORD', `+${s}`, `Pointers crossed — the number "${s}" is complete and strobogrammatic (${results.length} so far).`, { low: low, high: high }, 'good');
      return;
    }
    if (low === high) {
      for (const c of ['0', '1', '8']) {
        ...
        const s = path.join('');
        results.push(s);
        emit('RECORD', `mid ${c} → ${s}`, `Single middle slot at index ${low}: it must map to itself, so try ${c} → record "${s}" (${results.length} so far).`, { low: low, high: high }, 'good');
      }
      path[low] = '·';
      return;
    }
    for (const [a, b] of PAIRS) {
      if (low === 0 && a === '0') {
        emit('REJECT', `✗ ${a}/${b}`, `Outer pair ${a}/${b} would put a leading zero at index 0 — skip it.`, { low: low, high: high });
        continue;
      }
      path[low] = a;
      path[high] = b;
      emit('CHOOSE', `${a}…${b}`, `Place ${a} at index ${low} and ${b} at index ${high} (a strobogrammatic pair). Recurse inward.`, { low: low + 1, high: high - 1 });
      backtrack(low + 1, high - 1);
      path[low] = '·';
      path[high] = '·';
      emit('BACKTRACK', `undo ${a}/${b}`, `Backtrack: clear the ${a}/${b} pair at indices ${low}/${high} and try the next pair.`, { low: low, high: high });
    }
  };

  if (n > 0) backtrack(0, n - 1);
  emit('DONE', `${results.length} number${results.length === 1 ? '' : 's'}`, `All pair choices explored — ${results.length} strobogrammatic number${results.length === 1 ? '' : 's'} of length ${n}.`, { low: 0, high: n - 1 , done: true }, 'good');
  return frames;
}
