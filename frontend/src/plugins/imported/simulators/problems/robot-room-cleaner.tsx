...
function record({ room, start }: RobotInput): Frame<RobotState>[] {
  ...
  const dfs = (r: number, c: number, d: number) => {
    ...
    emit(
      'CLEAN',
      `clean (${r},${c})`,
      `Clean cell (${r}, ${c}) and mark it visited. Cells cleaned so far: ${count}. Now try each of the 4 directions from here.`,
      {},
    );
      ...
      if (!vis.has(key)) {
        ...
        if (move()) {
          emit(
            'MOVE',
            `move ${ARROW[nd]}`,
            `From (${r}, ${c}) the robot faces ${ARROW[nd]} and moves into the open cell (${nr}, ${nc}) — recurse into it.`,
            {},
          );
          dfs(nr, nc, nd);
          ...
          emit(
            'BACKTRACK',
            `back to (${r},${c})`,
            `Done exploring from (${nr}, ${nc}); backtrack — turn around, step back to (${r}, ${c}), and turn around again to restore facing.`,
            {},
          );
        } else {
          emit(
            'WALL',
            `wall ${ARROW[nd]}`,
            `Facing ${ARROW[nd]} from (${r}, ${c}) hits a wall (or the room edge) — can't move, skip this direction.`,
            {},
          );
        }
      }
  ...
  emit(
    'DONE',
    `${count} cells cleaned`,
    `Every reachable open cell has been visited and cleaned. Total cells cleaned = ${count}.`,
    { done: true },
    'good',
  );
  return frames;
