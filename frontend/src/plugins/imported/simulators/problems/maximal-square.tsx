    ...
    for (let c = 0; c < n; c++) {
      if (matrix[r]![c] !== '1') {
        ...
        emit(
          'ZERO',
          `dp[${r}][${c}]=0`,
          `Cell (${r}, ${c}) holds '0', so no square can end here: dp[${r}][${c}] = 0.`,
          { cur: [r, c] },
        );
        continue;
      }
      if (r === 0 || c === 0) {
        ...
        emit(
          'EDGE',
          `dp[${r}][${c}]=1`,
          `Cell (${r}, ${c}) holds '1' and ${why}, so the largest square ending here is just itself: dp[${r}][${c}] = 1.`,
          { cur: [r, c] },
        );
        continue;
      }
