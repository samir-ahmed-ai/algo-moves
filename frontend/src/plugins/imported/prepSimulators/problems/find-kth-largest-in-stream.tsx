...
function record({ k, init, stream }: KthStreamInput): Frame<KthStreamState>[] {
  ...
  for (const v of init) {
    ...
    emit('PUSH', `push ${v}`, `Bootstrap: push ${v} into the min-heap.`, {
      added: v,
      heap: heapDisplay(),
    });
    if (heap.length > k) {
  ...
  for (const v of stream) {
    heap = minHeapPush(heap, v);
    emit('ADD', `push ${v}`, `Stream add ${v}: push into min-heap.`, {
      added: v,
      heap: heapDisplay(),
    });
...
export const simulator: ProblemSimulator = {
  ...
  inputs: [
    {
      id: 'kts1',
      label: 'k=3, init=[4,5], stream=[8,2,9]',
      value: { k: 3, init: [4, 5], stream: [8, 2, 9] },
    },
    {
      id: 'kts2',
      label: 'k=2, init=[], stream=[3,1,4,1,5]',
      value: { k: 2, init: [], stream: [3, 1, 4, 1, 5] },
    },
