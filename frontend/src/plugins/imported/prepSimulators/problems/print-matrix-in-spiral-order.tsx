...
function record({ mat }: SpiralInput): Frame<SpiralState>[] {  const rows = mat.length;
  ...
  if (rows === 0 || cols === 0) {
    emit('DONE', 'empty', 'The matrix is empty, so the spiral order is the empty list.', { active: null, done: true }, 'good');
    return frames;
  }

  emit('INIT', `${rows}x${cols}`, `Spiral order: peel the outer ring of a ${rows}×${cols} matrix, then shrink the four boundaries inward and repeat. Bounds start at rows [${startR}..${endR}] and cols [${startC}..${endC}].`, { active: null, done: false });
  ...
  while (startR <= endR && startC <= endC) {
    ...
    for (let c = startC; c <= endC; c++) {
      take(startR, c);
      emit('TOP', `${mat[startR][c]}`, `Top row: walk left → right across row ${startR}, appending ${mat[startR][c]} from (${startR}, ${c}).`, { active: [startR, c], done: false });
    }
    startR++;
    emit('SHRINK', `startR=${startR}`, `Top row finished. Drop the top boundary down: startR = ${startR}. Remaining rows [${startR}..${endR}].`, { active: null, done: false });

    // Right column, top -> bottom.
    for (let r = startR; r <= endR; r++) {
      take(r, endC);
      emit('RIGHT', `${mat[r][endC]}`, `Right column: walk top → bottom down column ${endC}, appending ${mat[r][endC]} from (${r}, ${endC}).`, { active: [r, endC], done: false });
    }
    endC--;
    emit('SHRINK', `endC=${endC}`, `Right column finished. Pull the right boundary in: endC = ${endC}. Remaining cols [${startC}..${endC}].`, { active: null, done: false });

    // Bottom row, right -> left (only if a row remains).
    if (startR <= endR) {
      for (let c = endC; c >= startC; c--) {
        take(endR, c);
        emit('BOTTOM', `${mat[endR][c]}`, `Bottom row: walk right → left across row ${endR}, appending ${mat[endR][c]} from (${endR}, ${c}).`, { active: [endR, c], done: false });
      }
      endR--;
      emit('SHRINK', `endR=${endR}`, `Bottom row finished. Lift the bottom boundary up: endR = ${endR}. Remaining rows [${startR}..${endR}].`, { active: null, done: false });
    }
    ...
    if (startC <= endC) {
      for (let r = endR; r >= startR; r--) {
        take(r, startC);
        emit('LEFT', `${mat[r][startC]}`, `Left column: walk bottom → top up column ${startC}, appending ${mat[r][startC]} from (${r}, ${startC}).`, { active: [r, startC], done: false });
      }
      startC++;
      emit('SHRINK', `startC=${startC}`, `Left column finished. Push the left boundary in: startC = ${startC}. Remaining cols [${startC}..${endC}].`, { active: null, done: false });
    }
  }

  emit('DONE', `${res.length} cells`, `Bounds crossed (startR > endR or startC > endC), so every cell has been peeled. Spiral order: [${res.join(', ')}].`, { active: null, done: true }, 'good');
  return frames;
}
