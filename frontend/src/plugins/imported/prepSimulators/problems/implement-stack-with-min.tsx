...
function record({ ops }: MinStackInput): Frame<MinStackState>[] {
  const vals: number[] = [];
  const mins: number[] = [];
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain:
      'Returned . The stack is back to  cell(s); the running-min of the new top is already correct, no recomputation needed.',
  },
  {
    id: 'state',
    ...
    explain:
      'The recorder snapshots `vals` on every emit so each frame shows the algorithm mid-step.',
  },
  {
    id: 'complexity',
  ...
  },
  {
    ...
    explain:
      'Min() just reads the running-min stored in the top cell: . No scan of the whole stack is needed — that is why it is O(1).',
  },
];
