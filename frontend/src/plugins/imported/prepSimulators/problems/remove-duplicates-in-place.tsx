...
function record({ nums }: RemoveDupInput): Frame<RemoveDupState>[] {
  ...
  if (n === 0) {
    emit('DONE', 'length 0', `The array is empty, so the de-duplicated length is 0.`, { length: 0, done: true }, 'good');
    return frames;
  }
...
function View({ frame }: PluginViewProps<RemoveDupState>) {
  ...
  return (
    <div className="board-area">
      ...
      <ArrayRow values={s.nums} cellTone={tone} pointers={pointers} windowRange={s.done && s.length !== null ? [0, Math.max(0, s.length - 1)] : null} />
