        ...
        {s.stack.length === 0 ? (
          ...
          s.stack.map((c, i) => (
            <span
              ...
              className={cn(
                'mr-1 rounded px-1',
                i === s.stack.length - 1 ? 'bg-accentbg text-accent' : 'text-ink',
              )}
            >
              {c}
            </span>
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain:
      'The recorder snapshots `stream` on every emit so each frame shows the algorithm mid-step.',
  },
  {
    id: 'complexity',
    ...
    explain:
      'O(1) amortized per letter. O(n). top==ch -> pop (matched) else push; ok && stack empty',
  },
  {
    id: 'outcome',
