      ...
      rail={
        <>
          ...
          <RailResult
            label="LCA"
            value={lcaVal ?? '…'}
            tone={s.done ? (lcaVal !== null ? 'good' : 'bad') : 'accent'}
          />
        </>
      }
    >
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain:
      'Node  received a target from BOTH sides:  came up the left,  came up the right. p and q live in different subtrees here, so node  is their lowest common ancestor.',
  },
  {
    id: 'state',
    ...
    explain:
      'The recorder keeps `current` in sync: node index currently being visited (pre-order enter)',
  },
  {
    id: 'complexity',
    prompt:
      'What are the time and space complexities for "Lowest Common Ancestor of a Binary Tree"?',
    choices: [
      {
    ...
    explain:
      'O(n). O(h). Recursive DFS: if `root == nil || root == p || root == q`, return `root`; Recurse left and right. If both return non-nil → root is the LCA',
  },
  {
    id: 'outcome',
    ...
    explain:
      'Only the left side returned something (); the right returned nil. Node  passes that non-nil result straight up — both targets (or the single one found so far) are on the left.',
  },
];
export const simulator: ProblemSimulator = {
  ...
  inputs: [
    // Tree: [3,5,1,6,2,0,8,null,null,7,4]; p = node 5 (index 1), q = node 1 (index 2) → LCA 3.
    {
      id: 'lca1',
      label: 'LCA(5,1) → 3',
      value: { tree: [3, 5, 1, 6, 2, 0, 8, null, null, 7, 4], p: 1, q: 2 },
    },
    // Same tree; p = node 5 (index 1), q = node 4 (index 10) → LCA 5 (5 is ancestor of 4).
    {
      id: 'lca2',
      label: 'LCA(5,4) → 5',
      value: { tree: [3, 5, 1, 6, 2, 0, 8, null, null, 7, 4], p: 1, q: 10 },
    },
