...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain: "Delete every Kth node in a doubly linked list. We walk left→right with a step counter; each time step reaches k we unlink that node on BOTH sides (prev.next = next, next.prev = prev) and reset the counter. Time O(n), Space O(1)."
  },
  {
    id: "key-step",
    ...
    explain: "Unlink node : set prev.next = next and next.prev = prev so it is bypassed on both sides of the DLL. Reset step to 0 and continue from the next node."
  },
  {
    id: "state",
  ...
  },
  {
    ...
    explain: "Node  (value ) is kept — it is only # since the last reset (need ). Advance the cursor."
  }
];
