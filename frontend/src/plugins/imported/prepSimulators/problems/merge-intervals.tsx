...
function record({ intervals }: MergeInput): Frame<MergeState>[] {
  ...
  emit(
    ...
    `Merge Intervals: sort the intervals by start time so overlaps are always adjacent — ${sorted
      ...
      .join(
        ' ',
      )}. Then sweep left to right, extending the open interval whenever the next one touches it.`,
    {},
  );
    ...
    <div className="board-area">
      <div className={cn(vizText.sm, 'text-ink3')}>
        ...
        {curStart !== null && !s.done && (
          <>
            {' · '}cur start = <span className="font-mono text-ink">{curStart}</span>
          </>
        )}
      </div>
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain:
      " overlaps the open interval because its start  ≤ open end . Its end  reaches further, so extend the open interval's right edge to  → .",
  },
  {
    id: 'state',
