  ...
  return (
    <div className="board-area">
      ...
      <div className={cn('mt-1 flex flex-wrap gap-x-4 gap-y-1 font-mono', vizText.sm, 'text-ink3')}>
        ...
        {s.evicted !== null && <span className="text-bad">evicted {s.evicted}</span>}
      </div>
      {s.average !== null && (
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain:
      'Moving Average: keep a queue holding at most k= of the most recent values plus their running sum. Each Next(v) appends v and adds it to the sum, evicts the oldest if the window overflows, then returns sum ÷ window length — all in O(1).',
  },
  {
    id: 'key-step',
    ...
    explain:
      'The window exceeded k=, so evict the oldest value  from the front and subtract it: sum = . The window is back down to  value.',
  },
  {
    id: 'state',
    ...
    explain:
      'The recorder keeps `window` in sync: current contents of the sliding window (front = index 0)',
  },
  {
    id: 'complexity',
  ...
  },
  {
    ...
    explain:
      'Return the average over the current window: sum  ÷  = . This is the result of Next().',
  },
];
