    ...
    <div className="board-area">
      ...
      <div className={cn('mt-1 font-mono', vizText.sm, 'text-ink3')}>
        {s.candidate !== null && s.candidate !== '' ? (
          <>
            path ={' '}
            <span className={cn(s.isNew ? 'text-good' : s.isPali ? 'text-ink' : 'text-ink3')}>
              "{s.candidate}"
            </span>
            {s.isPali !== null && (
...
function Inspector({ frame }: InspectorProps<PaliState>) {
  ...
  return (
    <VarGrid>
      ...
      <InspectorRow
        k="current path"
        v={s.candidate !== null ? (s.candidate === '' ? 'ε' : `"${s.candidate}"`) : '—'}
      />
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain:
      'Find distinct palindromic substrings of "". We insert every suffix into a suffix trie so that each root→node path spells a distinct substring, then DFS the trie and count the paths that are palindromes.',
  },
  {
    id: 'key-step',
    ...
    explain:
      'The path "" reads the same both ways, and we have not seen it before — record it as distinct palindrome # and increment the count.',
  },
  {
    id: 'state',
