...
function record({ values }: PrintFromEndInput): Frame<PrintFromEndState>[] {  const stack: number[] = [];
  const out: number[] = [];
...
const practiceQuiz: QuizQuestion[] = [
  ...
  },
  {
    ...
    explain: "Print from end: recurse all the way to the tail first, then append each value as the call stack unwinds. out = rec(next); out = append(out, head.Val). The result lists the values back-to-front."
  },
  {
    id: "key-step",
    ...
    explain: "The deeper call returned, so node 's call resumes and appends its value  to the output. Because the tail finished first, values come out in reverse order."
  },
  {
    id: "state",
