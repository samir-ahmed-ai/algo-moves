  ...
  quiz: [
    {
      ...
      explain:
        'The code allocates only a 1D slice of length n and updates it row-by-row with `dp[j] += dp[j-1]`, achieving O(n) space instead of the O(m·n) a 2D table would need.',
    },
    {
      id: 'base-case',
      ...
      explain:
        'From the top-left corner, the first row is reachable only by moving right, so there is exactly one path to each cell in row 0. Initializing `dp[j] = 1` for all j encodes this before processing rows 1 through m-1.',
    },
    {
      id: 'recurrence',
      ...
      explain:
        'Before the update, `dp[j]` holds the count for the cell directly above (i-1, j). `dp[j-1]` was just updated for (i, j-1). Adding them gives the number of paths arriving at (i, j) from either direction — exactly the grid DP recurrence.',
    },
    {
      ...
      explain:
        'In a grid where you can only move right or down, the leftmost column (j = 0) is reachable only by moving straight down — one path for every row. Starting the inner loop at j = 1 preserves dp[0] = 1 throughout, correctly reflecting this.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'The double loop iterates m×n cells total — O(m·n) time. Only a 1D array of size n is allocated, so space is O(n) regardless of m.',
    },
    {
      id: 'edge-case',
      ...
      explain:
        'When m = 1, the outer loop condition `i < m` is immediately false, so dp is returned as initialized: all 1s. `dp[n-1]` is 1, which is correct — there is exactly one path in a single row (move right all the way).',
    },
  ],
};
