  ...
  quiz: [
    {
      id: 'category',
      prompt:
        'Which algorithm does `floydWarshall` implement, and what problem class does it solve?',
      choices: [
        {
      ...
      explain:
        'The triple-nested loop over k, i, j with the relaxation `dist[i][j] = min(dist[i][j], dist[i][k]+dist[k][j])` is the defining structure of Floyd-Warshall. It computes shortest paths between every pair of nodes in a single pass.',
    },
    {
      id: 'recurrence',
      ...
      explain:
        'The code checks `if dist[i][k]+dist[k][j] < dist[i][j]` and updates — relaxing the path from i to j through intermediate node k. Updating unconditionally would overwrite a known shorter distance whenever no path through k exists, since `dist[i][k]+dist[k][j]` would be an INF-based sum far larger than the real edge.',
    },
    {
      id: 'initialization',
      ...
      explain:
        'Floyd-Warshall builds up shortest paths by considering intermediate nodes one by one. Starting with `dist[i][i]=0` and `dist[i][j]=INF` (no direct edge) correctly represents the initial knowledge before relaxation runs.',
    },
    {
      id: 'loop-order',
      ...
      explain:
        "Floyd-Warshall's correctness relies on a DP invariant: after iteration k, `dist[i][j]` is the shortest path using only nodes 1..k as intermediates. Swapping the loop order breaks this invariant — you'd relax through k before knowing the best paths into or out of k.",
    },
    {
      id: 'complexity',
      ...
      explain:
        'Three nested loops each of size n give O(n³) time. The `dist` matrix is (n+1)×(n+1), so space is O(n²). Space cannot be O(n) because the algorithm requires storing every pairwise distance simultaneously.',
    },
    {
      id: 'query-sentinel',
      ...
      explain:
        "`INF = 1 << 30` is used as a sentinel meaning 'no path'. If the Floyd-Warshall relaxation never found a path, `dist[u][v]` stays at INF and the query returns -1 to communicate 'unreachable'. The separate bounds check in `floydWarshall` also returns -1 for out-of-range nodes.",
    },
  ],
};
