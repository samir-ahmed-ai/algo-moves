  ...
  quiz: [
    {
      ...
      explain:
        'The code uses level-by-level BFS (steps increments once per level, sz captures the layer size) to find the minimum number of transformations. Because all edges have cost 1, BFS gives the shortest path; Dijkstra is unnecessary.',
    },
    {
      id: 'visited-mechanism',
      prompt:
        'How does this solution prevent revisiting the same word during BFS, and why is this approach efficient?',
      choices: [
        {
      ...
      explain:
        '`delete(wordSet, w)` removes a word the moment it is added to the queue. This reuses the existing hash map for O(1) membership checks and O(1) deletion, avoiding the overhead of a second data structure.',
    },
    {
      ...
      explain:
        "The inner loops `for j := 0; j < len(buf); j++` and `for c := byte('a'); c <= 'z'; c++` systematically try every letter at every position (L·25 candidates). Checking wordSet is O(1), making this O(L·26) per word. Comparing every word pair would be O(n·L) per node, much worse.",
    },
    {
      id: 'early-termination',
      prompt:
        'Where in the BFS loop does the code check whether it has reached `endWord`, and why is that placement correct?',
      choices: [
        {
      ...
      explain:
        "The check `if cur == endWord { return steps }` fires when the word is dequeued. At that point `steps` already reflects this node's level (incremented at the top of the outer loop before processing the level), so the returned value is correct.",
    },
    {
      id: 'complexity',
      ...
      explain:
        'Each of the n words is dequeued at most once. Generating neighbors for one word costs O(L·26) (L positions × 26 letters), with O(L) to build the string and O(1) map lookup. Total: O(n·L·26), which simplifies to O(n·L).',
    },
    {
      id: 'early-exit',
      prompt:
        'What does the function return if `endWord` is not in `wordSet` at the very start, and why is this check necessary?',
      choices: [
        {
      ...
      explain:
        'Neighbor generation only enqueues words found in wordSet. If endWord is absent from the set it can never be enqueued, so BFS would finish and fall through to `return 0`. The early check avoids unnecessary BFS work and is the contract the problem specifies.',
    },
  ],
};
