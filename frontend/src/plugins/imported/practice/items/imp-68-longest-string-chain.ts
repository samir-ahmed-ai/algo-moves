  ...
  quiz: [
    {
      ...
      explain:
        'Words are sorted by length so that predecessors (shorter words) are always processed before successors. A map `dp[word]` stores the longest chain ending at that word, built by checking all one-deletion predecessors — classic DP on a DAG.',
    },
    {
      id: 'sort-purpose',
      ...
      explain:
        'In a string chain, each word is derived from the previous by inserting one character. Sorting by ascending length guarantees that when we process word `w`, all possible predecessors (length `len(w)-1`) have already been assigned their `dp` values.',
    },
    {
      ...
      explain:
        '`w[:i]` takes everything before position `i`, `w[i+1:]` takes everything after, and concatenation skips character `i`. Iterating `i` from 0 to `len(w)-1` generates all possible length-(L-1) predecessors by trying every single deletion.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'The inner loop runs `len(w)` times (≤ L). Each iteration creates a new string via concatenation `w[:i] + w[i+1:]`, which takes O(L) time. So per word: O(L) iterations × O(L) string creation = O(L²). Over all n words: O(n·L²).',
    },
    {
      id: 'dp-init',
      prompt:
        'The code sets `dp[w] = 1` before checking predecessors. What does this initialization mean?',
      choices: [
        {
      ...
      explain:
        'Setting `dp[w] = 1` before the predecessor loop ensures that even if no predecessor exists in the map, the word itself contributes a chain of length 1. The subsequent checks only update `dp[w]` upward when a predecessor is found.',
    },
    {
      id: 'result-tracking',
      prompt:
        'The code maintains a `res` variable updated inside the outer loop. Why not just return `dp[words[len(words)-1]]` after the loop?',
      choices: [
        {
      ...
      explain:
        'Sorting is by word length, and multiple words can share the same length. The globally longest chain can terminate at any word. Tracking the running maximum in `res` avoids a second pass and handles ties correctly.',
    },
  ],
};
