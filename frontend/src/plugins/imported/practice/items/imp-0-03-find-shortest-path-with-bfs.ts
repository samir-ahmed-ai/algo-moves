  ...
  quiz: [
    {
      ...
      explain:
        'The code uses a plain BFS (queue, dist array initialized to -1) on an unweighted adjacency list and reconstructs the unique shortest path using a `pred` (predecessor) array. Dijkstra would need a priority queue and is unnecessary without edge weights.',
    },
    {
      id: 'pred-array',
      ...
      explain:
        '`pred[nb] = v` stores the BFS parent of each discovered node. After BFS, the loop `for at := dest; at != -1; at = pred[at]` walks backwards through these parent links to rebuild the path.',
    },
    {
      id: 'early-exit',
      ...
      explain:
        '`if v == dest { break }` exits as soon as the destination is dequeued, which in BFS is the moment the shortest path length is confirmed. This is an optional optimization — the path is already correct at that point.',
    },
    {
      ...
      explain:
        'Prepending with `append([]int{at}, path...)` inserts at the front, so walking dest→src via `pred` naturally builds the path in src→dest order without an explicit reverse step.',
    },
    {
      id: 'no-path',
      ...
      explain:
        '`if dist[dest] == -1 { return nil }` — since dist is initialized to -1 and only updated when a node is reached, a remaining -1 at dest means no path exists, and nil is returned.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'The code allocates `dist`, `pred` (both length n), the queue (at most n entries), and the output path (at most n entries) — all O(V). The adjacency list is given as input and not allocated here.',
    },
  ],
};
