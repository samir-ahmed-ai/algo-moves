  ...
  quiz: [
    {
      ...
      explain:
        '`btPartition` scans left-to-right keeping a single boundary `pos` and swaps the pivot into place at the end — a Lomuto partition. `btQuickSelect` then recurses only into the side that contains rank `k`. This is Quickselect, not a heap or binary search.',
    },
    {
      id: 'rank-translation',
      prompt:
        'In `kthLargest`, the call is `btQuickSelect(arr, 0, n-1, n+1-k)`. Why `n+1-k` instead of `k`?',
      choices: [
        {
      ...
      explain:
        '`btQuickSelect` finds the element of rank `k` in ascending order. The kth largest in ascending order is at rank `n+1-k` (e.g., 1st largest = nth smallest).',
    },
    {
      id: 'partition-mechanic',
      prompt:
        'In `btPartition`, the pivot is always `a[start]`. After the loop, `a[start]` and `a[pos]` are swapped. What invariant does this establish?',
      choices: [
        {
      ...
      explain:
        'The loop counts elements smaller than the pivot and moves them to the front. After the swap, the pivot lands in its final sorted position `pos`, with smaller values to its left and larger-or-equal values to its right.',
    },
    {
      id: 'base-case',
      ...
      explain:
        "When the subarray has a single element, there's nothing left to partition — that element is definitionally the answer for any rank within a size-1 range.",
    },
    {
      id: 'complexity',
      ...
      explain:
        'Quickselect discards one side each round, and on average the kept side shrinks geometrically, giving O(n) expected time. With adversarial input and a fixed pivot like `a[start]`, every partition can be size-1, degrading to O(n²). Space is O(log n) for the average recursion depth.',
    },
    {
      id: 'copy-rationale',
      prompt:
        'Both `kthLargest` and `kthSmallest` copy the input with `make` + `copy` before calling `btQuickSelect`. Why?',
      choices: [
        {
      ...
      explain:
        'Quickselect rearranges elements as a side effect of partitioning. Without the copy, the original slice `a` passed by the caller would be shuffled, which is typically undesirable.',
    },
  ],
};
