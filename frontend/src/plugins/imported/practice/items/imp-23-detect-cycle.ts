  ...
  quiz: [
    {
      id: 'category',
      prompt:
        'What algorithm family do both `detectCycleUndirected` and `detectCycleDirected` belong to?',
      choices: [
        {
      ...
      explain:
        'Both use recursive DFS helpers (`btDetectCycleUndirectedDfs`, `btDetectCycleDirectedDfs`). BFS-based and Union-Find approaches are alternative techniques not used here.',
    },
    {
      ...
      explain:
        'In an undirected graph, revisiting the node we came from (the parent) is expected and not a cycle. A cycle only exists if we reach a visited node that is NOT our parent — checked by `nb != parent`.',
    },
    {
      id: 'directed-state',
      ...
      explain:
        "State 1 means the node is on the current DFS path (open). Finding a state-1 neighbor means we've found a back edge — a cycle. State 2 means fully explored with no cycle from that node.",
    },
    {
      id: 'why-parent-param',
      prompt:
        'Why does `btDetectCycleUndirectedDfs` need a `parent` parameter, but `btDetectCycleDirectedDfs` does not?',
      choices: [
        {
      ...
      explain:
        "In an undirected adjacency list, edge (u,v) appears in both `adj[u]` and `adj[v]`. Without ignoring the parent, DFS from u would immediately see v as 'visited' and falsely report a cycle.",
    },
    {
      id: 'complexity',
      ...
      explain:
        'DFS visits every node and edge once, giving O(V+E) time. The auxiliary space is the `visited`/`state` array of size V plus the recursion stack, which is O(V) in the worst case.',
    },
    {
      id: 'disconnected-graph',
      prompt:
        'How do `detectCycleUndirected` and `detectCycleDirected` handle disconnected graphs?',
      choices: [
        {
      ...
      explain:
        'Both outer functions loop `for i := range adj` and skip nodes already visited (or with state != 0). This ensures every connected component is explored independently.',
    },
  ],
};
