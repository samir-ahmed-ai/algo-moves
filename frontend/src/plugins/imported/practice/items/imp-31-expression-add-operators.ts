  ...
  quiz: [
    {
      id: 'category',
      prompt:
        'What makes `addOperators` harder than a plain DFS enumeration of operator placements?',
      choices: [
        {
      ...
      explain:
        'When a `*` is placed, the running `eval` must be corrected: `eval - prev + prev*val`. This `prev` bookkeeping is the distinctive mechanic. Leading-zero handling is present but secondary.',
    },
    {
      id: 'multiplication-precedence',
      prompt:
        'For the `*` branch, the new eval is `eval - prev + prev*val`. What role does `prev` play here?',
      choices: [
        {
      ...
      explain:
        'Say `eval = a + b` and now we place `* c`. We stored `prev = b`. To get `a + b*c` we compute `eval - b + b*c = (eval - prev) + prev*val`. The `prev*val` also becomes the new `prev` for chained multiplications like `a * b * c`.',
    },
    {
      id: 'leading-zero-prune',
      prompt:
        "The code breaks out of the inner loop with `if i > idx && num[idx] == '0' { break }`. What case does this prune?",
      choices: [
        {
      ...
      explain:
        "`i > idx` means we've tried `num[idx:idx+1]` (a single '0') and are now considering a longer slice starting with '0'. A leading zero on a multi-digit number is not a valid numeric literal, so the loop breaks early. The single '0' operand is still allowed.",
    },
    {
      id: 'first-operand',
      prompt:
        'When `idx == 0`, the code calls `btAddOperators` without prefixing an operator. Why?',
      choices: [
        {
      ...
      explain:
        'An expression cannot start with an operator. At `idx == 0`, `path = sub`, `eval = val`, `prev = val` initialises the state cleanly. All subsequent calls (idx > 0) prepend one of `+`, `-`, or `*`.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'For each position after the first digit we can either absorb it into the current operand (no split) or insert one of three operators — 4 choices total per boundary. With n digits there are n-1 boundaries, giving O(4^n) paths.',
    },
    {
      id: 'space-complexity',
      ...
      explain:
        'O(n) captures the call-stack depth (at most n frames) and the length of `path` at any point (n digits plus at most n-1 operators, so O(n) characters). The result slice is excluded from auxiliary-space analysis in the stated bound.',
    },
  ],
};
