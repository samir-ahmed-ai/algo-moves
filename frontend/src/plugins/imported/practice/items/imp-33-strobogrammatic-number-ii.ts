  ...
  quiz: [
    {
      ...
      explain:
        '`btStrobo` advances `low` and retreats `high` in each recursive call, placing mirror-symmetric digit pairs from the outside in. That is a classic two-pointer backtracking approach, not BFS or DP.',
    },
    {
      id: 'pair-set',
      ...
      explain:
        '6 rotated 180° becomes 9 and vice versa, so the pair (6,9) is valid (and (9,6) for the other direction), but (6,6) or (9,9) would not be strobogrammatic. 0, 1, and 8 each map to themselves, giving the five pairs in the code.',
    },
    {
      ...
      explain:
        'The outermost position (`low == 0`) is the most-significant digit. Placing \'0\' there creates strings like "00" or "0110", which have leading zeros and are not valid n-digit numbers. The guard `if low == 0 && pair[0] == \'0\' { continue }` skips these.',
    },
    {
      id: 'middle-digits',
      prompt:
        "When `low == high` (odd-length number), which digits does the code place in the middle, and why not '6' or '9'?",
      choices: [
        {
      ...
      explain:
        "The center digit must look the same when the whole number is rotated 180°, so it must map to itself. '6' maps to '9' and vice versa, so neither is self-mapping; only '0', '1', and '8' qualify. Note the center position is never subject to the leading-zero guard, so '0' is allowed there.",
    },
    {
      id: 'complexity',
      ...
      explain:
        "At each of the ~n/2 levels of recursion there are at most 5 pair choices (4 for outermost since '00' is excluded, 5 thereafter), giving roughly 5^(n/2) strings. Each string has length n, so total work is O(n · 5^(n/2)), which the problem states as O(n · 5^n) using a loose bound.",
    },
    {
      id: 'base-case',
      ...
      explain:
        '`low > high` means all positions have been filled (even-length numbers cross over; odd-length numbers hit `low == high` first for the center). At this point `path` is fully constructed and is copied into `res` via `string(path)`.',
    },
  ],
};
