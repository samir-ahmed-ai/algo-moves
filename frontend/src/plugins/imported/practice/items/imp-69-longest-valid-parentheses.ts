  ...
  quiz: [
    {
      id: 'category',
      prompt:
        "Longest Valid Parentheses is tagged both 'Stack' and 'Dynamic Programming'. Which pattern does this specific solution use?",
      choices: [
        {
      ...
      explain:
        'The code pushes/pops indices onto a `stack` seeded with a sentinel `-1`; span is computed as `i - stack[len(stack)-1]`. A DP array approach would never appear here.',
    },
    {
      id: 'sentinel',
      prompt:
        'The stack is initialized as `[]int{-1}` instead of an empty slice. Why is that sentinel needed?',
      choices: [
        {
      ...
      explain:
        "After popping a matching '(' the top of stack is the last unmatched ')' index (or -1 at the start). Subtracting it from the current index gives the length of the current valid run without any special-case code.",
    },
    {
      id: 'closing-empty-stack',
      prompt:
        "When a ')' is processed and the stack becomes empty after the pop, the code pushes `i`. What does this accomplish?",
      choices: [
        {
      ...
      explain:
        "An unmatched ')' can never be part of a valid substring, so `i` becomes the new left boundary (sentinel). Any subsequent valid run will be measured from `i` outward.",
    },
    {
      id: 'span-formula',
      prompt:
        "When a ')' matches and the stack is non-empty, the code computes `l := i - stack[len(stack)-1]`. What does `stack[len(stack)-1]` represent at that moment?",
      choices: [
        {
      ...
      explain:
        "After popping the matched '(', the new top is the last 'barrier' — an unmatched ')' or the -1 sentinel. `i - barrier` gives the length of the contiguous valid substring ending at `i`.",
    },
    {
      id: 'complexity',
      ...
      explain:
        "A single pass over the string gives O(n) time. The stack holds at most n entries in the worst case (all '(' characters), giving O(n) space.",
    },
    {
      id: 'edge-all-open',
      ...
      explain:
        "Every '(' is pushed and never popped (no ')' to pop them). The stack grows but `res` is never updated, so it stays 0. The sentinel prevents any out-of-bounds access.",
    },
  ],
};
