  ...
  quiz: [
    {
      ...
      explain:
        'The code fills `deg` and `edges` in one pass over `roads`, then iterates every pair (i, j) with i < j to compute `deg[i] + deg[j] - shared_edge`. No graph traversal or sorting is needed.',
    },
    {
      id: 'shared-edge-deduction',
      prompt:
        'When computing the rank of pair (i, j), the code does `rank--` if `edges[[2]int{i, j}]` is true. Why is 1 subtracted?',
      choices: [
        {
      ...
      explain:
        'Network rank = total distinct roads connected to either i or j. The shared road is included in both `deg[i]` and `deg[j]`, so the naive sum counts it twice. Subtracting 1 gives the correct count of distinct roads.',
    },
    {
      id: 'edge-normalisation',
      prompt:
        'Before inserting into the `edges` map, the code swaps u and v if `u > v`. What is the purpose of this normalisation?',
      choices: [
        {
      ...
      explain:
        'The pair-checking loop always queries with `i < j`, so the key `[2]int{i, j}` has the smaller index first. Normalising insertions to (min, max) guarantees the key matches. Without it, an edge stored as (5, 2) would not be found by a lookup for (2, 5).',
    },
    {
      id: 'complexity',
      ...
      explain:
        'Building `deg` and `edges` is O(E). The nested pair loop is O(n²). Space is O(n) for the degree array plus O(E) for the edge set. The problem states the known complexity as O(n²) for time and O(n+E) for space, matching this breakdown.',
    },
    {
      id: 'loop-bounds',
      ...
      explain:
        'Network rank is defined for a pair of distinct cities, and rank(i,j) == rank(j,i), so each pair needs one evaluation. Using i < j halves the work and avoids self-pairs. The edge-lookup correctness (point C) is a side benefit but not the primary reason for i+1.',
    },
    {
      ...
      explain:
        'With no roads, `deg` is all zeros and `edges` is empty. The pair loop computes `0 + 0 - 0 = 0` for every pair and `res` stays 0. The map is initialised at declaration (`edges := map[[2]int]bool{}`), so no panic occurs.',
    },
  ],
};
