  ...
  quiz: [
    {
      ...
      explain:
        "Coins can be reused any number of times ('unbounded'). The code iterates coins inside the amount loop without restricting how many times each coin is used — the classic unbounded-knapsack structure.",
    },
    {
      id: 'recurrence',
      ...
      explain:
        'The code picks the best (minimum) `dp[a]` over all valid coins c. Adding coin c to the optimal solution for `a-c` gives a valid solution for `a` costing one more coin.',
    },
    {
      id: 'sentinel',
      ...
      explain:
        '`1 << 30` acts as infinity. If no combination of coins reaches amount a, `dp[a]` stays at this sentinel. The final check `if dp[amount] == inf { return -1 }` uses this to detect the impossible case.',
    },
    {
      id: 'loop-order',
      prompt:
        'In `coinChange`, the outer loop is over amounts and the inner loop is over coins. What would happen if you swapped them (outer = coins, inner = amounts)?',
      choices: [
        {
      ...
      explain:
        "For unbounded-knapsack minimization (min coins), both loop orders are correct because you're taking the minimum — reusing a coin multiple times naturally emerges from the recurrence regardless of which dimension is outer.",
    },
    {
      id: 'complexity',
      ...
      explain:
        'The two nested loops run amount × len(coins) iterations. The `dp` array has length amount+1, so space is O(amount). No 2-D table is needed because coins are reusable.',
    },
    {
      id: 'edge-amount-zero',
      ...
      explain:
        '`make([]int, amount+1)` zero-initializes all cells, so `dp[0] = 0`. This is the correct base case: 0 coins are needed to reach amount 0, and all larger amounts build on this foundation.',
    },
  ],
};
