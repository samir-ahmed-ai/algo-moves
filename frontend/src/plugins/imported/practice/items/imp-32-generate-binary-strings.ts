  ...
  quiz: [
    {
      ...
      explain:
        "`btBin` branches into two recursive calls ('0' and '1') and builds strings down the call stack — that is DFS-based backtracking enumeration. BFS would use an explicit queue; this never does.",
    },
    {
      id: 'base-case',
      ...
      explain:
        'The guard `if len(path) == n` detects a complete string and records it. There is no `n == 0` guard in `btBin` — that detail is a distractor.',
    },
    {
      id: 'branching',
      ...
      explain:
        'Each call to `btBin` spawns `btBin(path+"0", …)` and `btBin(path+"1", …)`. At depth n the recursion terminates, giving a full binary tree with 2^n leaves — one per output string.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'There are 2^n strings, each of length n, so collecting them costs O(n · 2^n) time. Storing all results requires O(2^n) space (each string is length n, but the dominant factor is the count of strings). The recursion stack depth is only O(n), but the output dominates.',
    },
    {
      id: 'path-mutation',
      prompt:
        'Why does `btBin` pass `path+"0"` and `path+"1"` (new strings) rather than mutating a shared byte slice?',
      choices: [
        {
      ...
      explain:
        "Because Go strings are value types, `path+\"0\"` allocates a new string for the '0' branch, leaving `path` unchanged for the '1' branch. This naturally handles backtracking without an explicit undo step.",
    },
    {
      id: 'edge-empty',
      ...
      explain:
        'With n=0, `btBin("", 0, &res)` is called. `len("") == 0 == n` is immediately true, so `""` is appended and returned. The result is `[""]`, a slice with one (empty-string) element.',
    },
  ],
};
