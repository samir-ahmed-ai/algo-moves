  ...
  quiz: [
    {
      ...
      explain:
        'The code uses two variables `prev` and `cur` updated in a loop — textbook bottom-up 1-D DP that avoids storing an array. There is no recursion, no queue, and no combinatorial formula.',
    },
    {
      id: 'recurrence',
      ...
      explain:
        'After the update, `cur` holds `prev+cur` (old values), which equals ways at step i-1 plus ways at step i-2. This is exactly the Fibonacci recurrence: you reach stair i from i-1 (1-step) or i-2 (2-step).',
    },
    {
      id: 'base-case',
      ...
      explain:
        'For n=1 there is exactly 1 way (one 1-step). For n=2 there are 2 ways (1+1 or 2). In both cases the answer equals n, so returning n handles both without separate branches.',
    },
    {
      id: 'space-complexity',
      ...
      explain:
        'The solution stores only `prev` and `cur`, discarding earlier values. No array or call stack grows with n, so space is constant O(1).',
    },
    {
      id: 'complexity',
      ...
      explain:
        "The loop runs from i=3 to i=n, performing O(1) work per iteration, giving O(n) overall. The naive recursive approach without memoization would be O(2^n), but that's not what this code does.",
    },
  ],
};
