  ...
  quiz: [
    {
      ...
      explain:
        'Every time `a[mid] > key` the code records `mid` as a candidate and continues left (`high = mid - 1`), ensuring it finds the smallest such index.',
    },
    {
      id: 'result-tracking',
      prompt:
        'The variable `res` is initialized to `-1` and updated inside the loop. What does `res == -1` after the loop signify?',
      choices: [
        {
      ...
      explain:
        '`res` is only set when `a[mid] > key`. If the loop exits without ever entering that branch, every element is ≤ `key`, and the function correctly returns `-1`.',
    },
    {
      id: 'key-mechanic',
      prompt:
        'When `a[mid] > key`, the code sets `res = mid` and then `high = mid - 1`. What is the purpose of `high = mid - 1`?',
      choices: [
        {
      ...
      explain:
        "We've found *a* valid answer at `mid`, but the smallest such element could be further left. Narrowing right boundary to `mid - 1` forces the search into smaller indices.",
    },
    {
      id: 'return-value',
      ...
      explain:
        'The problem asks for the *letter* (value), not its position, so returning `a[res]` is correct. The index `res` is only used internally to track the best candidate.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'Each loop iteration halves the search window, giving O(log n). Only a constant number of variables (`low`, `high`, `mid`, `res`) are used.',
    },
  ],
};
