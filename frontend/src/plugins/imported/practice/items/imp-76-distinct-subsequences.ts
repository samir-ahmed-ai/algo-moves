  ...
  quiz: [
    {
      ...
      explain:
        'The code allocates a 2D table `dp[m+1][n+1]` and fills it by considering every prefix of s against every prefix of t — classic 2D string DP.',
    },
    {
      id: 'base-case',
      ...
      explain:
        'There is exactly one subsequence of any string that equals the empty string: the empty selection. Setting `dp[i][0] = 1` encodes this base case. `dp[0][j] = 0` for j > 0 is left at its zero-initialized value, correctly reflecting that an empty s cannot form a non-empty t.',
    },
    {
      id: 'recurrence',
      ...
      explain:
        '`dp[i-1][j]` is the number of ways to form t[:j] using s[:i-1]. Because we can always ignore the current character s[i-1], those ways carry over to dp[i][j] unconditionally. The match branch then adds diagonal ways on top.',
    },
    {
      id: 'match-step',
      ...
      explain:
        'When characters match, we get an additional family of subsequences: pick s[i-1] as the match for t[j-1], then rely on the dp[i-1][j-1] ways to match the remaining t[:j-1] using s[:i-1]. The diagonal term exactly captures this.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'The double loop fills an (m+1)×(n+1) table with O(1) work per cell, giving O(m·n) time. The table itself occupies O(m·n) space. A 1-D rolling optimization is possible but not implemented here.',
    },
    {
      id: 'edge-case',
      ...
      explain:
        'The base-case loop sets `dp[i][0] = 1` for every i including m. There is one way to form the empty string (select nothing), so this is correct. The function returns `dp[m][n]`, not this cell, but the base case is what allows the recurrence to propagate correctly.',
    },
  ],
};
