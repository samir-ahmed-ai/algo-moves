  ...
  quiz: [
    {
      ...
      explain:
        '`btDepthSum` calls itself recursively with `depth+1` when it encounters a nested list. This is DFS — the call stack encodes the nesting depth. BFS would require an explicit queue and process all items at one depth before moving deeper.',
    },
    {
      id: 'key-mechanic',
      ...
      explain:
        'The problem defines weight as the depth level, so an integer at depth d contributes d times its value. The code does `item.Val * depth` exactly.',
    },
    {
      id: 'base-case',
      ...
      explain:
        'There is no explicit depth-limit base case. Recursion stops naturally when every element in a list is an integer (not a nested list), because the `else` branch (which recurses) is never taken. An empty list also terminates the loop immediately.',
    },
    {
      id: 'initial-depth',
      ...
      explain:
        'LeetCode #339 defines the outermost list as depth 1. If depth started at 0, top-level integers would be multiplied by 0 and contribute nothing to the sum, giving a wrong answer.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'Every NestedInteger node is visited exactly once, giving O(N) time. Space is O(N) in the worst case because the recursion stack can be as deep as N (a fully nested structure like `[[[[[1]]]]]` has depth N).',
    },
    {
      id: 'edge-empty',
      ...
      explain:
        '`btDepthSum` initializes `sum := 0` and loops over `list`. An empty slice means zero iterations, so `sum` stays 0 and is returned. No panic occurs.',
    },
  ],
};
