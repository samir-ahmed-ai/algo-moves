  ...
  quiz: [
    {
      ...
      explain:
        "The code initializes `disc[]` and `low[]` arrays and calls `btTarjan` (a DFS) — that is Tarjan's bridge-finding algorithm. BFS cannot naturally compute low-link values because it doesn't produce DFS trees.",
    },
    {
      id: 'data-structure',
      ...
      explain:
        "The code does `if low[v] < low[u] { low[u] = low[v] }` after the recursive call — the child's lowest reachable discovery time is pulled up to the parent. Taking `disc[v]` instead (second choice) would not propagate through multi-hop back edges.",
    },
    {
      id: 'bridge-condition',
      prompt:
        'Which condition in `btTarjan` identifies the edge (u, v) as a bridge (critical connection)?',
      choices: [
        {
      ...
      explain:
        '`low[v] > disc[u]` means v cannot reach u or any ancestor of u through back edges — removing (u,v) disconnects the graph. Using `>=` would also flag articulation-point edges that are not bridges (applicable to biconnected components, not bridges).',
    },
    {
      id: 'parent-skip',
      ...
      explain:
        'The graph is undirected, so every tree edge appears in both directions. Without the `v == parent` skip, the code would use `disc[parent]` to lower `low[u]`, masking real bridges. Note that `disc[parent] != -1` alone is not sufficient — it would still update `low[u]`.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'Each vertex and edge is visited exactly once in the DFS; `disc` and `low` arrays are O(V) and the adjacency list is O(V+E), giving O(V+E) time and space overall.',
    },
    {
      id: 'edge-case',
      ...
      explain:
        'The outer function calls `btTarjan(adj, 0, -1, ...)` starting only from node 0 — there is no outer loop over all unvisited nodes. Unreachable components are never visited, so their bridges are not reported. A correct general-graph implementation would loop over all nodes and start DFS wherever `disc[i] == -1`.',
    },
  ],
};
