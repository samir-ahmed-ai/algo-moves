  ...
  quiz: [
    {
      ...
      explain:
        "`left[j]` carries the best score reachable from the left with the move cost already deducted (decremented by 1 each step), and `right[j]` does the same from the right. This reduces each row's transition from O(n²) to O(n).",
    },
    {
      id: 'left-sweep',
      prompt:
        'In the left sweep, the update is `left[j] = max(dp[j], left[j-1] - 1)`. What does subtracting 1 represent?',
      choices: [
        {
      ...
      explain:
        'The cost of picking column `k` in the previous row and landing on column `j` in the current row is `|k - j|`. Moving one column at a time and decrementing by 1 at each step accumulates exactly this cost by the time the sweep reaches `j`.',
    },
    {
      id: 'why-two-sweeps',
      ...
      explain:
        'For column `j`, the best predecessor could be to its left (k < j) or to its right (k > j). The left sweep propagates the maximum from smaller column indices; the right sweep propagates from larger ones. Taking the max of both at each `j` gives the global best.',
    },
    {
      id: 'space-opt',
      prompt:
        'The code uses a 1-D `dp` array that is overwritten each row rather than a full m×n table. Why is this valid?',
      choices: [
        {
      ...
      explain:
        'The transition for row `i` only reads `dp` values from row `i-1`. After computing `left`, `right`, and the new `dp[j]` values, the previous row is no longer needed, so the single array can be safely overwritten.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'Each row processes n columns three times (left sweep, right sweep, update). Across m rows this is O(m·n). Only the `dp`, `left`, and `right` arrays of length n are kept, giving O(n) space.',
    },
    {
      id: 'final-answer',
      ...
      explain:
        'The optimal last-row column is unknown in advance, so the code iterates over all entries of `dp` and returns the maximum — the best total score over any ending column.',
    },
  ],
};
