  ...
  quiz: [
    {
      ...
      explain:
        '`btOps` splits the token slice at every position (divide), solves left and right subproblems recursively (conquer), and caches results in a `memo` map. There is no iterative DP table or BFS queue.',
    },
    {
      id: 'base-case',
      ...
      explain:
        'When the token slice holds exactly one element, there is nothing left to split, so the only possible result is that single integer. `len(t) == 0` is never reached because the split loop starts at `i := 1`, keeping every left sub-slice non-empty.',
    },
    {
      id: 'combination-step',
      prompt:
        'After computing `left` and `right` sub-results, the code combines them with `+`, `-`, `*`, and optionally `/`. What guards the division?',
      choices: [
        {
      ...
      explain:
        'The code appends `a/b` only inside `if b != 0`, preventing a divide-by-zero panic. `a` (the left operand) can be zero without issue.',
    },
    {
      id: 'memo-key',
      prompt:
        'Why does `btOps` use a string key built from the token slice content rather than the slice indices `(start, end)`?',
      choices: [
        {
    ...
    },
    {
      id: 'complexity',
      prompt:
        'The comments state time and space are both `O(C_n^2)`. What does `C_n` represent here?',
      choices: [
        {
      ...
      explain:
        'The number of distinct ways to parenthesize an expression with n operators is the n-th Catalan number C_n. The number of results produced grows with the Catalan numbers, and the combine step pairs left results against right results, so the per-call work scales like C_n^2 — hence the `O(C_n^2)` annotation.',
    },
    {
      id: 'split-loop',
      prompt:
        'The split loop is `for i := 1; i < len(t); i++`. What would happen if it started at `i := 0` instead?',
      choices: [
        {
