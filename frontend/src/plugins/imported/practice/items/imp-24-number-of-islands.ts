  ...
  quiz: [
    {
      ...
      explain:
        "Each time a '1' is found, `btFloodFill` is called to recursively turn all reachable '1' cells to '0', erasing the island. This is classic DFS flood fill. BFS and Union-Find are valid alternatives but not what this code does.",
    },
    {
      id: 'visited-strategy',
      ...
      explain:
        "Setting `grid[r][c] = '0'` mutates the grid itself, eliminating the need for an auxiliary visited structure. The base case `grid[r][c] != '1'` then prevents re-entry.",
    },
    {
      id: 'base-case',
      prompt:
        'What does `btFloodFill` check at the top of every call, and why is the order of checks important?',
      choices: [
        {
      ...
      explain:
        'Go evaluates `||` left-to-right with short-circuit: checking bounds before accessing `grid[r][c]` prevents an out-of-bounds panic. Reversing the order would crash on edge cells.',
    },
    {
      id: 'connectivity',
      ...
      explain:
        'The four recursive calls use offsets (r±1, c) and (r, c±1), giving 4-connectivity. Diagonal neighbors are not checked, matching the standard grid-island problem definition.',
    },
    {
      id: 'complexity',
      ...
      explain:
        "Every cell is processed at most once (land cells get flipped to '0' after first visit). The recursion stack can be as deep as m·n in the worst case (a single large island), giving O(m·n) space despite the in-place modification.",
    },
    {
      id: 'edge-case',
      ...
      explain:
        "The outer loop only calls `btFloodFill` when `grid[r][c]=='1'`. An all-water grid never triggers that branch, so `cnt` remains 0 and is returned directly.",
    },
  ],
};
