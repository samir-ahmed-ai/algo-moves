  ...
  quiz: [
    {
      ...
      explain:
        "The code makes one left-to-right pass, maintaining `bCount` (b's seen so far) and `dp` (min deletions so far), updating `dp` at each 'a'. There is no second pointer, 2D table, or stack.",
    },
    {
      id: 'state-variables',
      ...
      explain:
        "`bCount` is incremented each time a 'b' is encountered, so it always equals the number of 'b's in the prefix processed so far. `dp` is the minimum deletions to make that prefix balanced — it is updated only when an 'a' is seen.",
    },
    {
      id: 'recurrence',
      ...
      explain:
        "The code checks `if dp+1 < bCount { dp++ } else { dp = bCount }`, which is exactly `dp = min(dp+1, bCount)`. Deleting this 'a' costs dp+1; deleting all preceding 'b's costs bCount. The minimum is taken.",
    },
    {
      id: 'why-no-array',
      ...
      explain:
        'The transition `dp = min(dp+1, bCount)` depends only on the current `dp` and the running `bCount`. No earlier DP values are revisited, so a single scalar suffices instead of a full dp array.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'A single loop over n characters gives O(n) time. Only two integer variables (`bCount`, `dp`) are maintained regardless of input size, giving O(1) space.',
    },
    {
      id: 'edge-case',
      prompt:
        'What does `minimumDeletions` return for a string that contains only \'b\'s (e.g., "bbb")?',
      choices: [
        {
      ...
      explain:
        "The `dp` update branch executes only when `s[i] == 'a'`. A string of all 'b's never triggers it, so `dp` stays 0 — correctly reflecting that no deletions are needed.",
    },
  ],
};
