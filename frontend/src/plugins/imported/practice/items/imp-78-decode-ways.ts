  ...
  quiz: [
    {
      ...
      explain:
        'The code uses only two scalars (`prev` and `cur`) that roll forward through the string, capturing the O(1)-space equivalent of a 1D dp array — the classic space-optimized Decode Ways pattern.',
    },
    {
      ...
      explain:
        "`if s[i] != '0' { next = cur }` sets next to the number of ways to decode up to the previous character, extending each of those decodings by one more single-digit character. A '0' cannot stand alone, so it contributes nothing as a single-digit code.",
    },
    {
      id: 'two-digit-transition',
      prompt:
        "The code computes `two := int(s[i-1]-'0')*10 + int(s[i]-'0')` and checks `two >= 10 && two <= 26`. Why is the lower bound 10, not 01?",
      choices: [
        {
      ...
      explain:
        'Codes 1–9 are accounted for by the single-digit branch. Two-digit codes must be at least 10 (otherwise the leading digit would be 0, which is not a valid standalone code) and at most 26 (the highest letter). Hence the range [10, 26].',
    },
    {
      id: 'early-exit',
      ...
      explain:
        "Letters map to '1'–'26'. A string starting with '0' has no valid first character decoding, so the total number of ways is 0. The check `s[0] == '0'` catches this before the loop runs.",
    },
    {
      ...
      explain:
        'Before the swap, `prev` covered s[:i-1] and `cur` covered s[:i]. After `prev, cur = cur, next`, `prev` now covers s[:i] and `cur` covers s[:i+1]. This shift lets the next iteration reuse them correctly.',
    },
    {
      id: 'complexity',
      ...
      explain:
        'A single pass over n characters gives O(n) time. Only `prev`, `cur`, `next`, and `two` are maintained — constant extra space regardless of input length.',
    },
  ],
};
