  ...
  quiz: [
    {
      id: 'category',
      prompt:
        'What algorithmic pattern does `maximumScore` use to find the highest-scoring 4-node sequence?',
      choices: [
        {
      ...
      explain:
        'The code builds a top-3 neighbor list for each node, then greedily tries every edge (u,v) as the middle pair, extending with the best-scoring distinct neighbor from each side. No BFS or DP is involved.',
    },
    {
      id: 'data-structure',
      ...
      explain:
        '`insertTop3` appends the new node, bubble-sorts it upward by score, then truncates to length 3 — producing a small sorted slice, not a heap or hash set.',
    },
    {
      id: 'key-mechanic',
      prompt:
        'Inside the nested loop over `top3[u]` and `top3[v]`, what do the `continue` conditions guard against?',
      choices: [
        {
      ...
      explain:
        'A valid 4-node path requires all distinct nodes. The checks `a==v`, `b==u`, and `b==a` skip any candidate that would duplicate one of the four positions in the sequence.',
    },
    {
      id: 'why-top3',
      prompt:
        'Why is it sufficient to keep only the top-3 neighbors per node instead of all neighbors?',
      choices: [
        {
      ...
      explain:
        "Only the middle edge endpoint (v) can disqualify one of u's top-3 neighbors, so among three candidates at least two remain valid. Thus the global optimum is always reachable without checking beyond the top 3.",
    },
    {
      id: 'complexity',
      ...
      explain:
        'Building `top3` is O(E) (each edge updates two lists of constant size). The main loop iterates over E edges with at most 9 inner iterations, giving O(E) total.',
    },
    {
      id: 'edge-case',
      prompt:
        'What does the function return when no valid 4-node sequence exists (e.g., the graph has fewer than 4 nodes)?',
      choices: [
        {
      ...
      explain:
        '`res` starts at -1. If no four-distinct-node combination is found the inner loops simply never execute an update, so -1 is returned as specified by the problem.',
    },
  ],
};
