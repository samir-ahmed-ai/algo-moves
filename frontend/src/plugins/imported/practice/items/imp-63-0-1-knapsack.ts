  ...
  quiz: [
    {
      ...
      explain:
        'The inner loop runs from `capacity` down to `w`, which prevents an item from being counted more than once. Unbounded knapsack would iterate left-to-right so the same item could be reused.',
    },
    {
      id: 'inner-loop-direction',
      prompt:
        'The inner capacity loop runs `for c := capacity; c >= w; c--`. Why does it iterate right-to-left (high to low)?',
      choices: [
        {
      ...
      explain:
        'If we iterated left-to-right, dp[c-w] would already reflect the current item being added, allowing the item to be used multiple times in a single pass. Right-to-left guarantees dp[c-w] still holds the state from BEFORE this item was considered.',
    },
    {
      id: 'recurrence',
      ...
      explain:
        '`dp[c-w] + v` says: if we use this item (costing weight `w`, gaining value `v`), the remaining capacity is `c-w`, and we inherit the best value already stored there. The `if` guard only updates when this is better than skipping.',
    },
    {
      id: 'space',
      prompt:
        'The solution allocates `dp := make([]int, capacity+1)`. What space complexity does this achieve, compared to a naive 2D DP table?',
      choices: [
        {
      ...
      explain:
        'By reusing a single 1D array and sweeping right-to-left, the previous row of the 2D table is never explicitly stored. This reduces space from O(n * capacity) to O(capacity).',
    },
    {
      id: 'complexity',
      ...
      explain:
        'The outer loop iterates over `n` items and the inner loop over up to `capacity` values, giving O(n * capacity). This is pseudo-polynomial — it depends on the numeric value of capacity, not just the input length.',
    },
    {
      id: 'base-case',
      ...
      explain:
        "Go's `make([]int, capacity+1)` zero-initializes all entries, encoding the base case: a knapsack of any capacity with no items considered yet has value 0. No explicit loop is needed.",
    },
  ],
};
