      ...
      good: [
        {
          ...
          answer:
            'There is one way to reach step 1 — a single 1-step. The loop (i from 2) never runs, so the answer is dp[1] = 1 directly. This is one of the two seeds the recurrence builds on.',
        },
        {
          id: 'n2',
          title: 'Two stairs',
          ...
          answer:
            '1+1 or a single 2-step. dp[2] = dp[1] + dp[0] = 1 + 1 = 2: reach step 2 either from step 1 (a 1-step) or step 0 (a 2-step). Both base cases dp[0]=1 and dp[1]=1 are needed here.',
        },
        {
          id: 'n3',
          title: 'Three stairs',
          ...
          answer:
            'dp[3] = dp[2] + dp[1] = 2 + 1 = 3. The last move onto step 3 was either a 1-step (from step 2) or a 2-step (from step 1), so the count is the sum of those already-solved subproblems — no recomputation.',
        },
        {
          id: 'n5',
          ...
          answer:
            'dp runs 1, 1, 2, 3, 5, 8 — each term is the sum of the previous two, exactly the Fibonacci recurrence. A naive recursion would re-solve dp[i-1] and dp[i-2] over and over (overlapping subproblems); the table solves each once, so n = 5 gives 8 ways.',
        },
      ],
    },
      ...
      good: [
        {
          ...
          answer:
            'Every character matches, so each cell carries the diagonal neighbour unchanged: dp[i][j] = dp[i-1][j-1]. No insert, delete, or replace is ever needed, and dp[m][n] stays at 0.',
        },
        {
          id: 'substitute',
          ...
          answer:
            '"c" and "t" match on the diagonal, but at the differing letter we take 1 + min(insert, delete, replace). The cheapest is replace: dp[i][j] = 1 + dp[i-1][j-1], turning "a" into "u" for a total of 1.',
        },
        {
          id: 'insert',
          ...
          answer:
            'The shared prefix "ab" matches along the diagonal at distance 0. The extra "c" has no partner, so the cell takes its left neighbour + 1 (an insertion): dp[i][j] = 1 + dp[i][j-1] = 1.',
        },
        {
          id: 'horse-ros',
          ...
          answer:
            'Row 0 and column 0 are the base cases: dp[i][0] = i deletions (empty target), dp[0][j] = j insertions (empty source). From there the optimal path replaces "h"→"r", keeps "o", deletes "r" and "e" — three edits, read off dp[5][3] = 3.',
        },
      ],
    },
    simulateQuestion: 'Which dp cell is filled next?',
      ...
      good: [
        {
          ...
          answer:
            'Exactly one: the empty set {} itself (2^0 = 1). The helper records it immediately and the for-loop never runs.',
        },
        {
          id: 'one',
          ...
          answer:
            'Two: {} and {7} (2^1 = 2). One decision — include 7 or not — doubles the empty case.',
        },
        {
          id: 'three',
          ...
          answer:
            '2^3 = 8. Because each subset is recorded on entry (before the loop), they come out in pre-order: {}, {1}, {1,2}, {1,2,3}, {1,3}, {2}, {2,3}, {3}.',
        },
        {
          id: 'four',
          ...
          answer:
            'It doubles to 2^4 = 16. Every existing subset spawns a twin that also contains the new element, which is exactly why the work is exponential.',
        },
      ],
    },
      ...
      good: [
        {
          ...
          answer:
            'The scan hits the first unvisited land cell, then flood-fills every 4-directionally connected cell so the whole patch is sunk at once. The top-left L-shape, the right-edge vertical pair, and the bottom T each form one connected component, so the count lands on 3.',
        },
        {
          id: 'one-big',
          ...
          answer:
            'The flood-fill from (0,0) walks orthogonally through the top row, down the middle column, and into the bottom-left pair, sinking six connected cells as one island. The lone cell at (2,3) touches only water in its four neighbours, so the scan reaches it later and starts a second flood-fill. Diagonal adjacency does not connect cells here.',
        },
        {
          id: 'all-water',
          ...
          answer:
            'The double loop never finds a cell whose status is still land, so no flood-fill ever starts and the count stays at 0. This is the natural base case — connected-components of an empty set is zero.',
        },
        {
          id: 'all-land',
          ...
          answer:
            'Exactly one. The first cell (0,0) seeds a DFS that reaches every other cell through orthogonal moves, sinking them all. When the scan resumes, no land remains, so the count is a single island.',
        },
      ],
    },
      ...
      good: [
        {
          ...
          tone: 'ok',
          question:
            'Each edge u→v means "take u before v". Can you finish a straight chain 0→1→2→3?',
          answer:
            'Yes. DFS walks the chain to the end, painting each node black on the way back. No edge ever points at a grey (on-stack) node, so there is no back edge and no cycle — schedule them in order 0,1,2,3.',
        },
        {
          id: 'independent',
          ...
          answer:
            'Trivially yes. Each DFS call has no outgoing edges, so every node goes white → grey → black immediately. With zero edges there is nothing to loop back on.',
        },
        {
          id: 'diamond',
          ...
          tone: 'ok',
          question:
            'Course 3 depends on both 1 and 2, which both depend on 0. Two paths reach 3 — is that a cycle?',
          answer:
            'No. Sharing a node from two paths is fine; a cycle needs an edge back onto the current stack. When DFS reaches 3 the second time (via 2) it is already black/finished, so it is skipped, not flagged. Still a DAG.',
        },
      ],
      bad: [
        {
          ...
          answer:
            'DFS enters 0 (grey), follows 0→1, enters 1 (grey), then 1→0 points at 0 — still grey on the stack. That back edge proves the two courses each wait on the other, so neither can ever be taken.',
        },
        {
          id: 'three-cycle',
          ...
          answer:
            'Yes. All of 0, 1, 2 sit grey on the recursion stack when DFS reaches the edge 2→0. Since 0 is grey, it is a back edge and the whole loop is unschedulable, no matter how long the cycle is.',
        },
        {
          id: 'self-loop',
          ...
          answer:
            'DFS marks 0 grey, then immediately sees edge 0→0 pointing at the grey 0. The shortest possible back edge — a node depending on itself can never start, so the answer is false.',
        },
      ],
    },
      ...
      good: [
        {
          ...
          answer:
            'Yes. Even-length cycles 2-colour cleanly: 0 and 2 land on team 1, 1 and 3 on team 2, every edge crosses.',
          input: {
            adj: [
              [1, 3],
              [0, 2],
        ...
        },
        {
          ...
          answer:
            'Yes. Every tree or forest is bipartite — just colour nodes by the parity of their depth.',
          input: {
            adj: [[1], [0, 2], [1, 3], [2]],
            pos: [
              [176, 44],
        ...
        },
        {
          ...
          answer:
            'Yes, bipartite by construction: {0,1} versus {2,3,4}. Every edge already crosses the split.',
          input: {
            adj: [
              [2, 3, 4],
              [2, 3, 4],
        ...
        },
        {
          ...
          answer:
            'Yes. The outer loop re-seeds each uncoloured component independently, and both 2-colour without conflict.',
          input: {
            adj: [[1], [0], [3], [2]],
            pos: [
              [176, 44],
      ...
      bad: [
        {
          ...
          answer:
            'No. 0 = team 1 forces 1 and 2 onto team 2, but the 1–2 edge then joins two same-team nodes. Clash.',
          input: {
            adj: [
              [1, 2],
              [0, 2],
        ...
        },
        {
          ...
          answer:
            'No. Colours alternate around the ring and collide at the closing edge — every odd cycle fails.',
          input: {
            adj: [
              [1, 4],
              [0, 2],
        ...
        },
        {
          ...
          answer:
            'No. Any complete graph on 3+ nodes contains a triangle, so it can never be 2-coloured.',
          input: {
            adj: [
              [1, 2, 3],
              [0, 2, 3],
        ...
        },
        {
          ...
          answer:
            'No. The pendant node 3 is harmless, but the embedded 0–1–2 triangle still forces a same-team edge.',
          input: {
            adj: [[1, 2, 3], [0, 2], [0, 1], [0]],
            pos: [
              [176, 44],
      ...
      good: [
        {
          ...
          answer:
            'Nearest-first. It settles 0 (0), relaxes to 1 (2), settles 1, relaxes to 2 (5), settles 2, relaxes to 3 (6). Each node has only one tentative value, so the final distances are [0, 2, 5, 6].',
        },
        {
          id: 'detour',
          ...
          answer:
            'This is the key insight. Dijkstra first settles 1 at distance 1 (the smallest tentative distance), then relaxes 1→2 to give 1 + 1 = 2, which beats the direct 0→2 of 5. The greedy direct edge is not the shortest route — a cheaper 2-hop detour wins.',
        },
        {
          id: 'diamond',
          ...
          answer:
            'The detour. dist[2] settles at 3 (0→1→2), so 2→4 gives 6 and 4→3 gives 8, beating 1→3 = 2 + 7 = 9. Because each node is finalized only after its smallest tentative distance is settled, the answer [0, 2, 3, 8, 6, 9] needs no revisiting — that is why non-negative weights are required.',
        },
        {
          id: 'island',
          ...
          answer:
            'Infinity. Node 3 only has an outgoing edge (3→0), so no relaxation ever lowers dist[3] below its initial ∞. When the smallest unvisited distance is ∞, the loop stops and 3 is reported as unreachable: dist = [0, 2, 5, ∞].',
        },
      ],
    },
      ...
      good: [
        {
          ...
          tone: 'ok',
          question:
            'A straight chain of dependencies — what order comes out, and why is it forced?',
          answer:
            'Exactly [0, 1, 2, 3]. Only node 0 starts with in-degree 0; outputting it drops 1 to 0, then 2, then 3. Each step exposes exactly one new source, so the order is unique.',
        },
        {
          id: 'diamond',
          title: 'Diamond dependency',
          ...
          answer:
            'Only after BOTH 1 and 2 are output: its in-degree starts at 2 and must hit 0 first. Order [0, 1, 2, 3] works; 1 and 2 are interchangeable, so [0, 2, 1, 3] is equally valid.',
        },
        {
          id: 'forest',
          ...
          tone: 'ok',
          question:
            'Two nodes (0 and 1) have no prerequisites — how does Kahn handle multiple sources?',
          answer:
            'Both 0 and 1 enter the queue at the start. They are output in any order; node 2 only becomes ready once both are placed (in-degree drops 2→1→0), then 3 follows.',
        },
      ],
      bad: [
        {
          ...
          answer:
            'Every node has in-degree 1, so the seed queue is empty from the start. With no source to pop, the loop never runs and 0 of 3 nodes are placed — a cycle has no valid order.',
        },
        {
          id: 'tail-into-cycle',
          ...
          answer:
            'No. 0 is output, dropping 1 to in-degree 1, but 1, 2, 3 form a loop (3→1) so none of them ever reaches 0. Only 1 of 4 nodes is placed; the leftover cycle proves no ordering exists.',
        },
      ],
    },
