      ...
      quiz: [
        {
          id: 'bucket-vs-leaky',
          prompt:
            'What is the key behavioral difference between a token-bucket limiter (as in x/time/rate) and a strict leaky-bucket-as-a-queue limiter?',
          choices: [
            {
          ...
          explain:
            'A token bucket accumulates up to `burst` tokens during idle periods, so it permits a short burst then throttles to rate r. A leaky bucket drains at a fixed rate; queued requests only leave at that rate, so saved-up capacity cannot fire as an output burst.',
        },
        {
          id: 'limiter-safety',
          prompt:
            'Regarding concurrency, which statement about a single *rate.Limiter shared across goroutines is correct?',
          choices: [
            {
          ...
          explain:
            'rate.Limiter methods (Allow, Reserve, Wait) are safe for concurrent use; the limiter guards its own state internally. You only need your own lock to protect the map that hands out per-key limiters.',
        },
        {
          id: 'reserve-vs-allow',
          prompt:
            'You call `r.Reserve()` on a rate.Limiter, then decide not to proceed. What must you do to avoid starving other callers?',
          choices: [
            {
          ...
          explain:
            "Reserve() consumes tokens optimistically for a future time. If you abandon it, calling Reservation.Cancel() returns the reserved tokens (as much as possible) so they aren't wasted; without Cancel the tokens stay consumed.",
        },
        {
          id: 'refill-model',
          prompt:
            'How does x/time/rate compute available tokens on each call, and why does that matter for correctness?',
          choices: [
            {
          ...
          explain:
            'The limiter is lazy: it derives current tokens from the time delta since the last event (tokens += elapsed * rate, capped at burst). No goroutine or ticker runs, so there is nothing to leak and no per-limiter background cost — important for millions of per-key limiters.',
        },
        {
          id: 'perkey-eviction',
          prompt:
            'In a per-key limiter map keyed by client ID, what is the primary production risk and its standard mitigation?',
          choices: [
            {
          ...
          explain:
            'Distinct keys (IPs, user IDs) create limiters that never get freed, so the map grows without bound and leaks memory. The standard fix is a sweeper that removes entries whose last-seen time exceeds an idle threshold.',
        },
        {
          id: 'infinite-limit',
      ...
      quiz: [
        {
          ...
          prompt:
            'You must cap concurrency at N for a long-lived, high-throughput ingestion service. What is the key runtime advantage of a fixed pool of N long-lived worker goroutines over spawning one goroutine per job gated by a size-N semaphore?',
          choices: [
            {
          ...
          explain:
            'A fixed pool amortizes goroutine creation/teardown and stack growth across all jobs; the semaphore variant still creates and destroys a goroutine per job. Neither approach guarantees completion ordering, and both still need context for cancellation.',
        },
        {
          id: 'backpressure-buffer',
          prompt:
            'A producer sends to an unbuffered jobs channel feeding N workers. Under a sustained input rate exceeding total worker throughput, what actually happens?',
          choices: [
            {
          ...
          explain:
            "An unbuffered (or bounded) channel blocks the sender once workers can't keep up, propagating backpressure to the producer. Channels never drop values or auto-scale the pool; unbounded growth only happens with an unbounded queue, which a channel is not.",
        },
        {
          id: 'graceful-drain',
          prompt:
            'For graceful drain — let in-flight and already-queued jobs finish, then stop — which sequence is correct?',
          choices: [
            {
          ...
          explain:
            "Closing jobs lets each worker's for-range consume remaining buffered items and then return, so wg.Wait() unblocks after a full drain. Cancelling ctx is abort, not drain; closing out (the receive side) makes workers panic on send, and waiting before closing jobs deadlocks because workers never leave the range.",
        },
        {
          id: 'close-out-owner',
          prompt:
            'Multiple workers send Results to a shared out channel. Where must close(out) happen to avoid a panic?',
          choices: [
            {
          ...
          explain:
            'A channel with multiple senders must be closed exactly once, after all senders are done — hence close(out) in a dedicated goroutine gated by wg.Wait(). Closing per-worker double-closes (panic), and the producer and the receiver are not senders on out.',
        },
        {
          id: 'ctx-cancel-inflight',
          prompt:
            'Under context cancellation, why must the blocking send `out <- Result{...}` also be guarded by a select on ctx.Done() in a production pool?',
          choices: [
            {
          ...
          explain:
            'If the collector returns on cancellation and stops draining out, a worker blocked on an unguarded send leaks forever; a select over ctx.Done() lets it exit. Cancelling a context does not close channels, and ctx.Err() is non-nil once Done() fires.',
        },
        {
          id: 'errgroup-first-error',
          prompt:
            "You replace manual error collection with x/sync/errgroup using WithContext and SetLimit(N). What is the precise semantics of the returned error and the group's context?",
          choices: [
            {
          ...
          explain:
            "errgroup.WithContext cancels the derived context the moment any Go func returns a non-nil error, and Wait returns that first error. It does not join all errors (that's a manual pattern) nor cancel only on full completion.",
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          id: 'why-dll',
          prompt:
            'Why does the classic LRU pair a hash map with a doubly linked list instead of using the map alone?',
          choices: [
            {
          ...
          explain:
            'The map gives O(1) key->node lookup, but recency ordering must be maintained; a doubly linked list lets you unlink and move a node to the front in O(1) given the node pointer, which a slice (O(n) shift) or the unordered map cannot do.',
        },
        {
          id: 'shard-vs-mutex',
          prompt:
            'Under heavy concurrent read/write load, why does sharding (N sub-caches keyed by hash(key)) usually beat one global mutex-guarded LRU?',
          choices: [
            {
          ...
          explain:
            'Sharding partitions the keyspace so each shard has its own mutex; unrelated keys hit different locks, cutting contention. It does not raise hit rate (it can slightly lower it by fragmenting capacity), still needs a mutex per shard, and gives up a single global recency order.',
        },
        {
          id: 'get-writes',
          prompt:
            'A reviewer proposes taking only a read lock (RLock) in Get so reads run concurrently. What is the flaw?',
          choices: [
            {
          ...
          explain:
            'A read hit must splice the node to the front of the list and may lazily evict an expired entry, both of which mutate shared state. That write under a shared RLock is a data race, so Get needs the exclusive lock (or a design that defers reordering).',
        },
        {
          id: 'ttl-eviction',
          prompt:
            'With per-entry TTL and purely lazy expiry (checked only on access), what failure mode must the design address?',
          choices: [
            {
          ...
          explain:
            'Lazy expiry only removes an entry when it is next accessed, so keys that are never read again stay resident and consume capacity/memory until LRU eviction reaches them. Production caches add active expiry (a background sweeper or sampling) to reclaim them.',
        },
        {
          id: 'syncmap-fit',
          prompt:
            'Why is sync.Map a poor foundation for an LRU cache despite being a concurrent map?',
          choices: [
            {
        ...
        },
        {
          id: 'amortized-evict',
          prompt:
            'On Put of a brand-new key when the shard is at capacity, what is the cost of the eviction step?',
          choices: [
            {
          ...
          explain:
            'The tail of the list is by construction the least-recently-used node; ll.Back() plus a map delete is O(1). No scan or heap is needed because recency order is maintained incrementally on every access.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          ...
          prompt:
            'What is the essential behavioral difference between http.Server.Shutdown(ctx) and http.Server.Close()?',
          choices: [
            {
          ...
          explain:
            'Shutdown stops listeners, closes idle connections, then waits for active requests to finish (bounded by ctx). Close abruptly closes listeners and all connections without waiting for in-flight work.',
        },
        {
          id: 'listenandserve-return',
          prompt:
            'After Shutdown is called, what does the still-blocked ListenAndServe call in the goroutine return?',
          choices: [
            {
          ...
          explain:
            'ListenAndServe always returns a non-nil error; on Shutdown/Close it returns http.ErrServerClosed. Idiomatic code checks errors.Is(err, http.ErrServerClosed) and treats only other errors as fatal.',
        },
        {
          id: 'notifycontext-restore',
          prompt:
            'Why call stop() (the cancel returned by signal.NotifyContext) right after <-ctx.Done() unblocks?',
          choices: [
            {
          ...
          explain:
            "NotifyContext installs handlers that swallow the signal (only cancelling the context). Calling stop() uninstalls them, so if a slow drain hangs, an impatient operator's second SIGTERM reverts to the default terminate behavior instead of being ignored.",
        },
        {
          id: 'shutdown-timeout-hang',
          prompt:
            'A handler runs a 60s job ignoring r.Context(); Shutdown is given a 10s context. What happens at t=10s?',
          choices: [
            {
          ...
          explain:
            "Shutdown returns the context's error (DeadlineExceeded) but Go cannot forcibly kill a goroutine. The handler keeps running unless it observes cancellation; graceful shutdown requires handlers to honor their request context or you follow up with Close().",
        },
        {
          id: 'close-ordering',
          prompt:
            'You have an HTTP server plus a DB pool the handlers use. What is the correct shutdown ordering?',
          choices: [
            {
          ...
          explain:
            'Dependencies must outlive their users. Draining the server first lets in-flight handlers finish their DB work; closing the pool first would make those still-draining requests fail. Close in reverse order of initialization.',
        },
        {
          id: 'registeronshutdown',
          prompt:
            'What does http.Server.RegisterOnShutdown(f) actually guarantee about when f runs during Shutdown?',
          choices: [
            {
          ...
          explain:
            'RegisterOnShutdown callbacks fire early (each in its own goroutine) when Shutdown starts, primarily to notify hijacked/WebSocket connections to close gracefully, since Shutdown itself does not track hijacked connections.',
        },
      ],
      design: {
        prompt:
          'Design a graceful shutdown sequence for a Go service that runs an HTTP API, a Kafka consumer loop, and a background worker pool, all fronted by a Kubernetes load balancer. Cover signal handling, ordering, timeouts, and the readiness/traffic-draining interaction. What are the tradeoffs?',
        answer:
      ...
      quiz: [
        {
          ...
          prompt:
            "In the pipeline, why must each stage's send be wrapped in `select { case out <- v: case <-ctx.Done(): }` rather than a bare `out <- v`?",
          choices: [
            {
          ...
          explain:
            'If the consumer stops reading (break/cancel), a bare `out <- v` on an unbuffered channel blocks with no receiver forever, leaking the goroutine. Selecting on ctx.Done() gives the blocked sender an exit path.',
        },
        {
          id: 'who-closes',
          prompt:
            'Which stage is responsible for closing the merged output channel `out` in `merge`, and when?',
          choices: [
            {
          ...
          explain:
            'Only one closer is allowed; closing from each fan-in goroutine would panic on the second close. A single goroutine waits on the WaitGroup, then closes out exactly once.',
        },
        {
          id: 'leak-on-early-break',
          prompt:
            'After main does `cancel(); break`, what actually lets the still-running `square` and `gen` goroutines terminate instead of leaking?',
          choices: [
            {
          ...
          explain:
            "Goroutines are never GC'd while runnable, and closing a channel does not propagate upstream. Cancellation unblocks each blocked send via the ctx.Done() case, and the returning stages then close their own outputs.",
        },
        {
          id: 'shared-source-fanout',
          prompt:
            'Both `w1` and `w2` read from the same `source` channel. What is the semantic consequence of this fan-out?',
          choices: [
            {
          ...
          explain:
            'Multiple receivers on one channel is safe; each sent value is handed to exactly one ready receiver. That is precisely how a single source is load-balanced across N workers.',
        },
        {
          id: 'unbuffered-cancel-visibility',
          prompt:
            'All stage channels are unbuffered. When main calls cancel() mid-range, why might a worker have already computed a value it can never deliver — and is that a leak?',
          choices: [
            {
          ...
          explain:
            'On the next select both the send and the ctx.Done() case may be ready; if Done wins, the already-computed value is simply dropped and the goroutine returns. Dropping an in-flight value under cancellation is expected, not a leak.',
        },
        {
          id: 'context-vs-done-channel',
          prompt:
            'Compared to threading an explicit `done chan struct{}` through every stage, what does using `context.Context` primarily add here?',
          choices: [
            {
          ...
          explain:
            'A plain done channel only broadcasts stop. context.Context adds deadlines/timeouts (WithTimeout), cancellation propagation through a context tree, and request-scoped values, while still using the same select-on-Done() discipline.',
        },
      ],
      design: {
        prompt:
