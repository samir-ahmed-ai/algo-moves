      ...
      quiz: [
        {
          ...
          prompt:
            'In Go 1.20+, what does fmt.Errorf("a: %w: %w", e1, e2) return, and how do errors.Is/As traverse it?',
          choices: [
            {
          ...
          explain:
            'Multiple %w produce an error whose Unwrap() returns []error; errors.Is/As perform a depth-first walk visiting every wrapped error.',
        },
        {
          id: 'as-target-kind',
          ...
          explain:
            'errors.As panics if target is not a non-nil pointer to either a type implementing error or to any interface type; it sets *target to the first match. A pointer to an interface (including interface{}) is accepted, so the notion that interfaces are rejected is false.',
        },
        {
          id: 'is-comparability',
          prompt:
            'errors.Is walks the chain. For an error in the chain that is NOT comparable and has no Is method, what happens at that node?',
          choices: [
            {
          ...
          explain:
            'errors.Is only performs == when the value is comparable; a non-comparable node with no Is method is silently skipped for the equality test (no panic) and the walk continues unwrapping, so a deeper node can still match.',
        },
        {
          id: 'verb-w-vs-v',
          ...
          explain:
            "%v formats the error's text only and creates no Unwrap link, so errors.Is/As cannot reach the sentinel; only %w records the wrapped error.",
        },
        {
          id: 'custom-is-override',
          prompt:
            'A custom error type defines Is(target error) bool returning true for any *NetError. Calling errors.Is(err, someNetErr) where err wraps that type:',
          choices: [
            {
          ...
          explain:
            "errors.Is calls a node's Is(target) bool method when present, enabling semantic matching independent of pointer identity or ==; the method signature is Is(error) bool, not (bool, error).",
        },
        {
          id: 'as-first-match',
          prompt:
            'A chain contains two *QueryError values at different depths. errors.As(err, &qe) sets qe to:',
          choices: [
            {
          ...
          explain:
            'errors.As returns on the first error in the tree that matches the target type, using depth-first pre-order traversal, and stops there.',
        },
      ],
      design: {
        prompt:
          'You are designing the error strategy for a library consumed across many services. Decide what to wrap with %w versus what to keep opaque, and how to expose sentinels/typed errors as a stable API. What are the tradeoffs?',
        answer:
      ...
      quiz: [
        {
          ...
          prompt:
            'A library exports `var ErrClosed = errors.New("closed")`. Callers across the codebase check `errors.Is(err, ErrClosed)`. Within any single running program, what makes that check reliable?',
          choices: [
            {
          ...
          explain:
            'errors.New returns a distinct *errorString each call; errors.Is compares plain sentinels with ==, which compares pointers, so every caller must reference the one exported variable. Matching by text or by type is not how Is resolves a plain sentinel.',
        },
        {
          id: 'custom-is-semantics',
          prompt:
            'In the code, `QueryError.Is` returns true when codes match. Given `err` has Code 42 wrapping ErrNotFound, what does `errors.Is(err, &QueryError{Code: 42, Err: errors.New("other")})` return and why?',
          choices: [
            {
          ...
          explain:
            "errors.Is calls the node's Is method when present; QueryError.Is ignores Err and compares only Code, so 42==42 yields true regardless of the differing wrapped cause or the distinct pointers.",
        },
        {
          id: 'as-target-type',
          prompt:
            '`Error()`, `Unwrap()`, and `Is()` are all declared on `*QueryError` (pointer receiver). What must the second argument to errors.As be?',
          choices: [
            {
          ...
          explain:
            'As needs a non-nil pointer whose pointee is a type implementing error. Only *QueryError implements error, so the pointee must be *QueryError and the target must be **QueryError, i.e. &qe where qe is *QueryError. A &qe over a QueryError value, or a bare *QueryError, panics at runtime because the pointee does not implement error.',
        },
        {
          id: 'missing-unwrap',
          prompt:
            'If `QueryError` had NO Unwrap method but still stored ErrNotFound in its Err field, what would `errors.Is(err, ErrNotFound)` return?',
          choices: [
            {
          ...
          explain:
            'errors.Is descends only via Unwrap (or a custom Is). Without Unwrap the Err field is invisible to Is; the walk stops at QueryError, whose own Is only matches another *QueryError, so ErrNotFound is never reached and Is returns false.',
        },
        {
          id: 'sentinel-api-coupling',
          prompt:
            'Why is exporting a sentinel like `ErrNotFound` a heavier API commitment than returning an unexported typed error checked via a helper predicate?',
          choices: [
            {
        ...
        },
        {
          ...
          explain:
            'Multiple %w verbs produce an error whose Unwrap() returns []error, and errors.Is/As do a depth-first traversal over the whole tree, so a match on any wrapped branch succeeds. Two %w verbs have been valid since Go 1.20.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          id: 'recover-scope',
          prompt:
            "A goroutine calls f(), which calls a helper() that recover()s (helper is NOT deferred) and then f() panics. Does helper's recover() stop the panic?",
          choices: [
            {
          ...
          explain:
            'recover only has effect when called directly by a function that was itself deferred and is running during unwinding; a plain call returns nil and never stops a panic.',
        },
        {
          id: 'defer-arg-eval',
          ...
          explain:
            "Deferred call arguments are evaluated when the defer statement executes, so i's value of 10 is captured immediately; only the invocation is delayed.",
        },
        {
          id: 'named-return-mutation',
          prompt:
            'In safeDivide with named returns (result, err), the deferred func sets err after a panic. What does the caller observe?',
          choices: [
            {
          ...
          explain:
            "Named return variables are addressable slots; a deferred function that recovers can assign to them, and those values become the function's actual return after unwinding stops.",
        },
        {
          id: 'repanic-value',
          prompt:
            'A deferred func calls recover(), decides not to handle the value, and calls panic(r). What propagates to the caller?',
          choices: [
            {
          ...
          explain:
            'recover() stops the current unwinding, but calling panic(r) starts a fresh panic carrying that same value, so it continues propagating to callers as if re-raised.',
        },
        {
          id: 'defer-order-panic',
          prompt:
            'Three defers registered in a function that panics before returning. In what order do they run?',
          choices: [
            {
          ...
          explain:
            'Deferred calls always run in last-in-first-out order, and a panic still executes all registered defers of the frame as it unwinds.',
        },
        {
      ...
      quiz: [
        {
          id: 'unwrap-signature',
          prompt:
            "For errors.Is and errors.As to walk into a custom error's wrapped cause, which method must the custom type provide (in Go 1.26)?",
          choices: [
            {
          ...
          explain:
            'The stdlib recognizes Unwrap() error for single-parent chains; Cause()/Wrapped() are not recognized. Unwrap() []error is only for multi-error trees (Join), not a general requirement.',
        },
        {
          id: 'is-target-comparable',
          prompt:
            'errors.Is(err, target) is called where target is a *NotFoundError with a filled Key. NotFoundError does NOT implement an Is method. What decides a match at each node in the chain?',
          choices: [
            {
          ...
          explain:
            'Without a custom Is method, errors.Is compares each node to target with ==. For *NotFoundError that == is pointer-identity, so two distinct instances never match regardless of Key. Use errors.As to extract by type instead.',
        },
        {
          id: 'as-pointer-arg',
          prompt:
            'var e MyError (a struct implementing error via pointer receiver). What is the correct errors.As call, and why?',
          choices: [
            {
          ...
          explain:
            "error is satisfied by *MyError (pointer receiver), so the target must be **MyError, i.e. &ep where ep is *MyError. Passing a non-pointer or a type that doesn't implement error panics or fails to match.",
        },
        {
          ...
          explain:
            'Join returns an unexported type with Unwrap() []error; Is/As recursively descend into every branch, so a and its chain remain reachable. j itself is not a *NotFoundError.',
        },
        {
          id: 'wrap-verb-vs-v',
          prompt:
            'fmt.Errorf("ctx: %v", cause) versus fmt.Errorf("ctx: %w", cause). Difference for error inspection?',
          choices: [
            {
          ...
          explain:
            '%w makes the returned wrapError expose the cause via Unwrap; %v only interpolates the string, so errors.Is/As cannot see the underlying error. Multiple %w in one Errorf produces a multi-error (Unwrap() []error).',
        },
        {
          id: 'nil-unwrap-chain',
          prompt:
            'A *NotFoundError has Unwrap() returning its err field, which is nil. errors.Is(nfe, someTarget) behaves how at that node?',
          choices: [
            {
          ...
          explain:
            'When Unwrap returns nil, errors.Is treats the chain as ended and stops without matching further; it does not panic. errors.Is(err, nil) is only true if err itself is nil.',
        },
      ],
      design: {
        prompt:
