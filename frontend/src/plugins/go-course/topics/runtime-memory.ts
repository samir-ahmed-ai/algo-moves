  ...
  "concepts": [
      ...
      "quiz": [
        {
          ...
          "prompt": "In newPoint, a local Point is created and &p is returned. What does escape analysis decide, and why?",
          "choices": [
            {
          ...
          "explain": "The address of p is returned, so p must remain valid after newPoint returns; the compiler moves it to the heap (\"moved to heap: p\"). Struct size is irrelevant to this decision, and returning &p is exactly what forces the escape."
        },
        {
          "id": "interface-boxing",
          "prompt": "leak returns myInt(v) as an fmt.Stringer. Why does the concrete value escape to the heap?",
          "choices": [
            {
          ...
          "explain": "An interface holds a (type, data) pair where data is a pointer; storing a concrete value in an interface that escapes forces the value onto the heap. It is the boxing, not the value's size or the method body, that causes the escape."
        },
        {
          "id": "gcflags-m",
          "prompt": "Which command reveals the compiler's escape decisions without changing generated code?",
          "choices": [
            {
          ...
          "explain": "-gcflags=-m emits escape-analysis and inlining diagnostics (add a second -m for more detail) at compile time without altering codegen. -race changes the binary, pprof samples at runtime, and go vet has no -escape flag."
        },
        {
          "id": "slice-append-escape",
          "prompt": "A function appends to a slice passed by value, then returns the (possibly reallocated) slice header. Does the backing array escape?",
          "choices": [
            {
          ...
          "explain": "Escape is decided by lifetime, not the word \"slice.\" If the returned backing array can be reachable after the frame returns it escapes; a slice used and dropped locally can stay on the stack. Growth alone does not force the heap when the compiler proves the array does not outlive the frame."
        },
        {
          "id": "why-perf",
          ...
          "explain": "Stack allocation and reclamation is essentially free (adjusting SP on return), while heap allocations add allocator cost and later GC mark/scan work. Heap and stack are the same physical memory, and escaping does not inherently add copies."
        },
        {
          "id": "closure-capture",
          "prompt": "A closure captures a local variable by reference and the closure is returned to the caller. What happens to that variable?",
          "choices": [
            {
      ...
      "quiz": [
        {
          ...
          "prompt": "What invariant does Go's concurrent collector conceptually preserve during the mark phase, and why does it matter?",
          "choices": [
            {
          ...
          "explain": "The hybrid write barrier upholds the tricolor invariant so that no black (fully scanned) object is left holding a pointer to a white (unscanned) object, ensuring nothing reachable is missed while mutators run concurrently."
        },
        {
          "id": "gogc-meaning",
          "prompt": "With GOGC=100 and a live heap of 50 MB after a cycle, roughly when does the next GC trigger?",
          "choices": [
            {
          ...
          "explain": "GOGC is a growth ratio: the heap goal is live_heap * (1 + GOGC/100). At GOGC=100 that is 50 MB * 2 = 100 MB, so GC targets when the heap reaches about 100 MB."
        },
        {
          ...
          "explain": "Go's hybrid write barrier is enabled only while marking is concurrently in progress; outside a mark phase pointer writes are plain stores, so steady-state code pays no barrier cost."
        },
        {
          ...
          "explain": "GOMEMLIMIT is a soft limit: as heap usage approaches it the pacer schedules GC more frequently regardless of GOGC, but it never fails allocation and can still exceed the limit under pressure, risking a GC death-spiral."
        },
        {
          "id": "stw-phases",
          ...
          "explain": "Go brackets concurrent marking with two short STW phases: mark setup (enable the write barrier and prepare roots) and mark termination (finish marking and disable the barrier). The bulk of marking and all sweeping run concurrently."
        },
        {
          "id": "mark-assist",
    ...
    },
    {
      ...
      "quiz": [
        {
          "id": "race-is-ub",
          "prompt": "A program has one goroutine writing an int variable and another reading it with no synchronization. What does the Go memory model say the reader may observe?",
          "choices": [
            {
          ...
          "explain": "The current Go memory model (published with Go 1.19) makes a data race on ordinary non-atomic memory undefined behavior: the compiler is entitled to assume no race occurs, so the read is not guaranteed to yield even an old-or-new single value."
        },
        {
          "id": "chan-hb-direction",
          "prompt": "For a Go channel, which happens-before edge does the memory model guarantee about a send and its corresponding receive?",
          "choices": [
            {
          ...
          "explain": "The spec states 'a send on a channel happens-before the completion of the corresponding receive.' The receiver therefore observes every write the sender made before the send. (Unbuffered channels add a second edge in the other direction: the receive happens-before that send completes, since they rendezvous.)"
        },
        {
          "id": "buffered-hb",
          "prompt": "For a channel with capacity C, what happens-before guarantee lets a bounded semaphore work correctly?",
          "choices": [
            {
          ...
          "explain": "The memory model states the k-th receive from a channel with capacity C happens-before the (k+C)-th send completes, which is exactly the back-pressure ordering a counting-semaphore pattern relies on."
        },
        {
          "id": "double-checked",
          "prompt": "A lazy singleton reads a shared *T without a lock, and only takes the mutex if it's nil (classic double-checked locking). Why is this broken in Go?",
          "choices": [
            {
          ...
          "explain": "The lock-free read shares no happens-before edge with the write performed under the lock, so it is a data race and undefined behavior. The fix is sync.Once or atomic.Pointer, not a bare nil check."
        },
        {
          "id": "init-vs-goroutine",
          "prompt": "Package-level var x is set in init(), then a goroutine started in main reads x. Is the read guaranteed to see the init value?",
          "choices": [
            {
          ...
          "explain": "The model guarantees all package initialization happens-before the start of main, and starting a goroutine happens-before the goroutine's execution, so the transitive edge makes the init value visible to the goroutine."
        },
        {
          "id": "atomic-vs-plain",
          "prompt": "One goroutine does atomic.StoreInt64(&flag,1) then a plain write data=42. Another spins on atomic.LoadInt64(&flag)==1 then reads data. Is reading data safe?",
          "choices": [
            {
    ...
    },
    {
      ...
      "quiz": [
        {
          "id": "pool-gc-lifetime",
          "prompt": "An object is put into a sync.Pool and never retrieved again. Under Go 1.26 semantics, how long can it be expected to survive?",
          "choices": [
            {
          ...
          "explain": "Go's Pool keeps a primary and a victim cache; each GC promotes the primary to victim and drops the old victim, so an unused object survives at most about two GC cycles rather than indefinitely."
        },
        {
          "id": "pool-reset",
          ...
          "explain": "Pool returns objects with whatever state they had; without Reset the next Get sees leftover contents. Reset zeroes the length while retaining the backing capacity, which is exactly the reuse you want."
        },
        {
          "id": "pool-value-vs-ptr",
          "prompt": "Why does storing *bytes.Buffer rather than bytes.Buffer in the Pool matter for allocation control?",
          "choices": [
            {
          ...
          "explain": "Pool's Get and Put take and return any (interface{}). Boxing a non-pointer struct value into an interface generally heap-allocates on each Put, defeating the pool's purpose; a pointer boxes without a per-call heap allocation."
        },
        {
          "id": "false-sharing-pad",
          "prompt": "The counter struct pads to 64 bytes. What problem does this padding specifically prevent?",
          "choices": [
            {
          ...
          "explain": "Adjacent counters would otherwise sit on one 64-byte cache line; a write by one core invalidates the line for others (false sharing), causing coherence traffic. Padding each to a full line isolates them. Note this does not make the ++ atomic — it only removes the coherence contention."
        },
        {
          "id": "prealloc-cap",
          "prompt": "Which preallocation choice most reduces GC pressure when building a slice whose final length you know is exactly n?",
          "choices": [
            {
          ...
          "explain": "make([]T, 0, n) reserves capacity n at length 0, so n appends never reallocate. make([]T, n) plus append leaves the first n zero elements and appends beyond n; nil-slice append reallocates repeatedly during growth. make([]T, 0) has no capacity hint at all."
        },
        {
          "id": "pool-get-nil",
          "prompt": "A sync.Pool is created with no New func. What does Get() return when both the primary and victim caches are empty?",
          "choices": [
            {
