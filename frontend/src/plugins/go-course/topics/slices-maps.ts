  ...
  "concepts": [
    {
      ...
      "quiz": [
        {
          "id": "three-index-cap",
          "prompt": "Given `b := make([]int, 5, 8)` and `v := b[1:3:4]`, what are `len(v)` and `cap(v)`?",
          "choices": [
            {
          ...
          "explain": "For `b[low:high:max]`, len = high-low = 3-1 = 2 and cap = max-low = 4-1 = 3. The third index bounds cap without touching len; the parent's own cap of 8 is irrelevant once max is supplied."
        },
        {
          "id": "append-alias",
          "prompt": "After `v := b[1:3:4]` on `b := []int{0,1,2,3,4}` (cap 5), then `v = append(v, 99)`, what is `b`?",
          "choices": [
            {
          ...
          "explain": "v has len 2, cap 3, so one append stays within cap and writes into the shared backing at index 3 (b[3]), overwriting the 3 with 99. No reallocation occurs."
        },
        {
          "id": "realloc-detach",
          ...
          "explain": "len==cap==3, so append allocates a new backing array, copies the elements, and appends 100. From then on v points elsewhere; mutating v no longer affects b."
        },
        {
          "id": "growth-factor",
          "prompt": "For appending 2048 ints one at a time to a nil slice in Go 1.26, which statement about growth is correct?",
          "choices": [
            {
          ...
          "explain": "Go grows capacity geometrically (roughly doubling below 256 elements, then tapering toward ~1.25x for large slices), giving amortized O(1) appends. It is neither strict doubling across all sizes nor per-element growth."
        },
        {
          "id": "header-copy",
          "prompt": "`a := []int{1,2,3}; b := a; b = append(b, 4)` where a has cap 3. Which is true about a and b afterward?",
          "choices": [
            {
          ...
          "explain": "b := a copies the header (same ptr/len/cap). Since len==cap==3, append reallocates b onto a new array; a still sees {1,2,3} with len 3. The copies diverged at the reallocation, and a has no fourth slot to reveal 4."
        },
        {
          "id": "passing-slice",
          "prompt": "A function `func f(s []int) { s = append(s, 9) }` is called as `f(x)` where `len(x) < cap(x)`. What does the caller observe in x?",
          "choices": [
            {
          ...
          "explain": "The header is passed by value, so the caller's len is unaffected. But because cap>len, append writes 9 into the shared backing array at index len(x), so that spare element is mutated even though x's length stays the same."
        }
      ],
      "design": {
    ...
    },
    {
      ...
      "quiz": [
        {
          "id": "append-clobber",
          "prompt": "Given `base := []int{1,2,3,4}` with len==cap==4, then `v := base[0:2]` followed by `v = append(v, 99)`, what is `base` afterward?",
          "choices": [
            {
          ...
          "explain": "v has len 2 but cap 4 (it inherits base's capacity from index 0), so append writes into the shared slot at index 2, overwriting the 3 with 99. base still has len 4, so the trailing 4 is unchanged: [1 2 99 4]."
        },
        {
          "id": "full-slice-detach",
          "prompt": "To make `v := base[0:2]` such that a later `append(v, x)` can NEVER touch base's backing array (base has cap 4), which single expression is both minimal and correct?",
          "choices": [
            {
          ...
          "explain": "The full-slice expression base[0:2:2] sets cap==len==2, so the very next append has no spare capacity and must allocate a fresh array, guaranteeing isolation. base[0:2] and base[:2] are identical and keep cap 4; base[0:2:4] explicitly keeps cap 4, so all three let append write into base in place."
        },
        {
          "id": "nil-vs-empty-json",
          "prompt": "For `var a []int` (nil) and `b := []int{}` (empty), which statement is TRUE under Go 1.26 semantics?",
          "choices": [
            {
          ...
          "explain": "A nil slice compares equal to nil; an empty non-nil slice does not. Both have len 0 and range safely (a nil slice ranges zero times, no panic), but they differ in the nil comparison, and in encoding/json a nil slice marshals to null while an empty slice marshals to []."
        },
        {
          "id": "shared-after-realloc",
          "prompt": "After `s := make([]int, 2, 2); t := append(s, 1); t[0] = 9`, does `s[0]` change?",
          "choices": [
            {
          ...
          "explain": "s had cap==len==2, so append had no spare capacity and allocated a new backing array for t (copying s's elements into it). s and t are now independent, so mutating t[0] leaves s[0] untouched. The reason is reallocation, not that append avoids index 0."
        },
        {
          "id": "copy-min-len",
          "prompt": "What does `copy(dst, src)` do when `dst` has len 3 and `src` has len 5, and what does it return?",
          "choices": [
            {
          ...
          "explain": "copy never reallocates or grows the destination; it copies min(len(dst), len(src)) elements and returns that count. Here it copies 3 and returns 3."
        },
        {
          "id": "append-two-branches",
          "prompt": "Two slices `x` and `y` both view `base[0:2]` (cap 4). You do `x = append(x, 10)` then `y = append(y, 20)`. What is `x[2]`?",
          "choices": [
            {
          ...
          "explain": "Both appends have spare capacity and write into base's index 2 in place. y's append happens last, so it overwrites x's 10 with 20; since x still aliases base, x[2] reads 20."
        }
      ],
      "design": {
      ...
      "quiz": [
        {
          ...
          "prompt": "Given `var m map[string]int` (never made with make), what happens when you first read `m[\"x\"]` and then execute `m[\"x\"] = 1`?",
          "choices": [
            {
          ...
          "explain": "Reads, len, and delete on a nil map behave like an empty map: the read returns the zero value (0). Any write (assignment) to a nil map panics with 'assignment to entry in nil map'."
        },
        {
          "id": "iteration-order",
          ...
          "explain": "The runtime picks a random starting bucket and intra-bucket offset per range so programs cannot accidentally rely on a stable order; it is a correctness and portability safeguard, not a performance or security feature. Insertion order is never preserved regardless."
        },
        {
          "id": "map-addressability",
          "prompt": "With `m := map[string]Point{}` where Point is a struct, why does `m[\"a\"].X = 5` fail to compile?",
          "choices": [
            {
          ...
          "explain": "Because rehashing and growth can relocate elements, map values are not addressable, so you cannot assign to a field of a map value in place. You must read the whole struct, modify it, and write it back, or store a pointer value type. This is a compile-time error, not a runtime one."
        },
        {
          "id": "delete-during-range",
          "prompt": "Which statement about mutating a map during `for k := range m` is correct under Go 1.26 semantics?",
          "choices": [
            {
          ...
          "explain": "Deleting during range is explicitly allowed by the spec: a key removed before the iterator reaches it will not be produced. Adding keys during range is legal but whether a newly added entry is produced is unspecified, so it is not guaranteed either way."
        },
        {
          "id": "load-factor-growth",
          "prompt": "In Go's traditional bucket (pre-Swiss-table) map implementation, growth is triggered primarily when which condition holds?",
          "choices": [
            {
          ...
          "explain": "Growth is driven by average load (element count over bucket count) crossing ~6.5 (the runtime's 13/2 load factor), or by too many overflow buckets triggering same-size growth to defragment. A single full bucket just allocates an overflow bucket rather than forcing a resize."
        },
        {
          ...
          "explain": "The runtime has lightweight concurrent-write detection that calls fatal (an unrecoverable crash, not a catchable panic) when it observes unsynchronized concurrent access. Even a concurrent read racing a write can trip it; use a mutex or sync.Map."
        }
      ],
      "design": {
    ...
    },
    {
      ...
      "quiz": [
        {
          "id": "len-vs-runecount",
          "prompt": "For s := \"héllo, 世界\" (é is U+00E9 = 2 bytes; 世 and 界 are 3-byte CJK each), what do len(s) and utf8.RuneCountInString(s) return?",
          "choices": [
            {
          ...
          "explain": "len returns the byte length: h,l,l,o (4) + é (2) + comma + space (2) + 世 (3) + 界 (3) = 14. RuneCountInString decodes UTF-8 and counts 9 code points."
        },
        {
          "id": "index-yields-byte",
          "prompt": "Given s := \"héllo\" where é is 2 bytes (0xC3 0xA9), what is the type and value of s[1]?",
          "choices": [
            {
          ...
          "explain": "Indexing a string returns a byte (uint8), never a rune. s[1] is the first byte of é's two-byte encoding, 0xC3 (the lead byte). To get runes you must range or convert to []rune."
        },
        {
          "id": "range-index-gap",
          "prompt": "When ranging over s := \"a世b\" with `for i, r := range s`, what index values does i take across the three iterations?",
          "choices": [
            {
          ...
          "explain": "range over a string yields (byte-offset, rune). 'a' is at offset 0, '世' (3 bytes) at offset 1, and 'b' at offset 4 because the decoder advanced past 世's three bytes. Indices are byte positions, not rune ordinals."
        },
        {
          "id": "invalid-utf8-range",
          "prompt": "Ranging over a string containing an invalid UTF-8 byte (e.g. a lone 0xFF), what rune value does that iteration produce and how far does it advance?",
          "choices": [
            {
          ...
          "explain": "range on a string treats invalid encoding leniently: each malformed byte yields RuneError (U+FFFD) and the offset advances by exactly one byte, guaranteeing progress. It never panics."
        },
        {
          "id": "conversion-copy",
          "prompt": "Which statement about b := []byte(s) and back string(b) under standard Go 1.26 semantics is correct?",
          "choices": [
            {
          ...
          "explain": "[]byte(s) allocates and copies because the resulting slice is mutable while the string's backing bytes must stay immutable. The compiler does optimize away some string(b) allocations (e.g. m[string(b)] lookups, range over string(b)), but []byte(s) itself copies."
        },
        {
          "id": "rune-slice-mutation",
          "prompt": "After rs := []rune(\"héllo\"), how does len(rs) compare to len(\"héllo\"), and is rs mutable?",
          "choices": [
            {
