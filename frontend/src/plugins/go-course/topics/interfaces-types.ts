      ...
      quiz: [
        {
          id: 'iface-vs-eface',
          prompt:
            'How does the runtime representation of a non-empty interface (e.g. io.Reader) differ from an empty interface (any)?',
          choices: [
            {
          ...
          explain:
            "Both are two words with a data pointer second, but a non-empty interface's first word is an *itab (which contains the type plus a method-pointer array), whereas an empty interface's first word is a bare *_type since it has no methods to dispatch.",
        },
        {
          id: 'typed-nil',
          ...
          explain:
            'An interface is nil only when both the type/itab word and the data word are nil. Assigning a typed nil pointer sets the itab word (type *Dog is known), so the interface is non-nil even though its data word is nil.',
        },
        {
          id: 'dispatch-cost',
          prompt:
            'What is the primary runtime cost of an interface method call compared to a direct static call?',
          choices: [
            {
          ...
          explain:
            "The method address is loaded from a fixed offset in the itab's fun array and called indirectly; there is no per-call hashing or allocation. The real cost is the extra indirection plus the loss of inlining and cross-call optimization the compiler could apply to a static call.",
        },
        {
          ...
          explain:
            'For statically known pairs the compiler emits the itab; otherwise runtime.getitab builds it once and caches it in a global hash table keyed by (interface, type), so repeated conversions of the same pair reuse the same *itab rather than reconstructing it.',
        },
        {
      ...
      quiz: [
        {
          ...
          prompt:
            'Given `var p *ValidationError = nil` and `var e error = p`, what does `e == nil` evaluate to and why?',
          choices: [
            {
          ...
          explain:
            'An interface equals nil only when both its type and value words are nil. Assigning a typed nil pointer sets the type word to *ValidationError, so e != nil even though the boxed value is nil. No dereference happens during comparison.',
        },
        {
          id: 'named-return-leak',
          prompt:
            'A function `func f() error` declares `var err *MyErr` and unconditionally `return err` on success paths. Callers doing `if err != nil` always see an error. What is the root cause?',
          choices: [
            {
          ...
          explain:
            "Returning a concrete `*MyErr` (even nil) through the `error` return type boxes it, populating the interface's type word. The fix is to return the literal `nil` (untyped) on success so both words stay nil.",
        },
        {
          id: 'func-signature-fix',
          prompt:
            'Which change most reliably prevents accidentally returning a non-nil error interface that wraps a nil concrete pointer?',
          choices: [
            {
          ...
          explain:
            'Returning the untyped `nil` literal keeps both interface words nil. `(*MyErr)(nil)` still fills the type word, `errors.Is(err, nil)` reports true only for an already-nil interface, and receiver kind does not change how a typed nil boxes.',
        },
        {
          id: 'reflect-detect',
          prompt:
            'You must detect a typed-nil hiding in an `any` at runtime. Which check correctly reports it as effectively nil?',
          choices: [
            {
          ...
          explain:
            'Only reflection can look past the type word: check that the boxed kind is a nillable kind (Ptr, Map, Chan, Func, Slice, Interface) and call reflect.Value.IsNil. Direct `== nil` returns false; `%v` prints `<nil>` for both typed and genuine nils, so it cannot distinguish them; and errors.Is does not unwrap typed nils.',
        },
        {
          id: 'panic-source',
          prompt:
            'Code guards `if err != nil { return err }`, the guard passes, and later `err.Error()` panics with a nil dereference. What happened?',
          choices: [
            {
          ...
          explain:
            'The interface was non-nil (type word set) so the guard was skipped, but the boxed pointer was nil. Calling a pointer-receiver method that dereferences the receiver then panics. A value receiver would tolerate it only if it never dereferences.',
        },
        {
          id: 'map-typed-nil',
          prompt:
            '`m := map[string]any{}; m["x"] = (*int)(nil)`. What does `m["x"] == nil` return?',
          choices: [
            {
          ...
          explain:
            'Storing `(*int)(nil)` boxes a typed nil into the `any`, so its type word is *int and the comparison is false. Contrast a genuinely absent key, whose zero value IS an untyped nil interface and compares true.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          ...
          prompt:
            'Type T has exactly one method, declared with a pointer receiver: func (t *T) M(). Interface I requires M(). Which assignment compiles?',
          choices: [
            {
          ...
          explain:
            "A pointer-receiver method is in *T's method set only, not T's. So &T{} satisfies I but a plain T value does not.",
        },
        {
          id: 'map-element-not-addressable',
          prompt:
            'With m := map[string]T{"k": {}} and M declared as func (t *T) M(), why does m["k"].M() fail to compile?',
          choices: [
            {
          ...
          explain:
            'Calling a pointer method on an addressable value auto-takes its address, but a map index expression is not addressable, so the compiler cannot synthesize &m["k"].',
        },
        {
          ...
          explain:
            'Unlike map elements, slice elements are addressable. The compiler auto-takes &s[0], so the pointer method runs on the actual element and mutations persist.',
        },
        {
          id: 'embedded-value-promotes',
          prompt:
            "S embeds a value field of type T (not *T). T's method M has a pointer receiver. Is M in the method set of the value type S?",
          choices: [
            {
          ...
          explain:
            "Promotion mirrors receiver rules: embedding T by value puts T's value methods in S and *S, but T's pointer methods appear only in *S's method set.",
        },
        {
          id: 'nil-interface-vs-typed-nil',
          prompt:
            'func f() error returns var p *myErr = nil; return p, where *myErr implements error. Caller does if f() != nil. What happens?',
          choices: [
            {
          ...
          explain:
            'Storing a typed nil pointer in an interface yields an interface with a non-nil type word, so the interface itself is not nil even though the pointer is.',
        },
        {
          id: 'reflect-set-addressability',
          prompt:
            'You have a value v of type T (methods on *T) inside an interface{}. Why can\'t reflect.ValueOf(x).MethodByName("M") find M and mutate the original?',
          choices: [
            {
          ...
          explain:
            "An interface holds an unaddressable copy of the value, so reflect sees only T's method set; to expose *T's pointer methods you must store &v so the interface's dynamic type is *T.",
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          id: 'promotion-depth',
          prompt:
            'A struct C embeds B, and B embeds A; A has method M and C also declares its own M. On a value of type C, what does c.M() call, and is it a compile error?',
          choices: [
            {
          ...
          explain:
            'Method promotion favors the shallowest depth. C.M is at depth 0, A.M is promoted at depth 2, so C.M unambiguously wins and shadows A.M with no error.',
        },
        {
          id: 'diamond-ambiguity',
          prompt:
            'Struct D embeds both X and Y (each at depth 1), and both X and Y define a method Ping. D declares no Ping of its own. What happens when you write d.Ping()?',
          choices: [
            {
          ...
          explain:
            'When two promoted methods with the same name sit at the same depth, the selector is ambiguous and the program fails to compile unless D declares its own Ping or you qualify it (d.X.Ping()).',
        },
        {
          id: 'iface-embed-nil',
          prompt:
            'You embed an interface (not a struct) in a decorator struct and forget to set that field, then call a promoted method that your struct does not override. What happens?',
          choices: [
            {
          ...
          explain:
            'Embedding an interface promotes its methods, but the dynamic value is nil. Calling a promoted method dispatches through a nil interface, producing a runtime panic — a classic decorator pitfall. Go interfaces have no default method bodies.',
        },
        {
          id: 'satisfies-via-embed',
          prompt:
            'Interface R requires Read and Close. Type T embeds an *os.File (which has both) and adds nothing. Does T satisfy R, and how?',
          choices: [
            {
          ...
          explain:
            "Promoted methods become part of the embedder's method set, so T satisfies R. Embedding a pointer *os.File promotes its methods to both T and *T, so the value type T already satisfies R.",
        },
        {
          id: 'override-no-super',
          prompt:
            'Service overrides promoted Log but its body calls s.Logger.Log(...). What does this idiom accomplish, and could you instead call s.Log inside the body?',
          choices: [
            {
          ...
          explain:
            'There is no super; you must qualify the embedded field (s.Logger.Log) to reach the shadowed method. Calling s.Log would re-invoke the override and recurse forever.',
        },
        {
          id: 'iface-embed-composition',
          prompt:
            'An interface RW embeds interfaces Reader and Writer, both of which declare Close() with identical signatures. Is the RW definition valid in Go 1.26?',
          choices: [
            {
          ...
          explain:
            'Since Go 1.14, embedding interfaces with overlapping methods is allowed as long as the duplicated method has an identical signature; the method set simply unions them. Only conflicting signatures for the same name would be an error.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          ...
          explain:
            'A nil interface holds no dynamic type, so every type assertion misses; the comma-ok form returns the zero value of T (a nil io.Reader) and ok=false rather than panicking.',
        },
        {
          ...
          explain:
            "The single-return assertion panics with 'interface conversion: interface {} is string, not int' when the dynamic type does not match; only the comma-ok form is safe.",
        },
        {
          id: 'iface-to-iface',
          prompt:
            'With `var v any = Named{}` where `Named` has a `String() string` method, what does `v.(fmt.Stringer)` test?',
          choices: [
            {
          ...
          explain:
            "Asserting to an interface type succeeds iff the value's dynamic type's method set implements that interface; it is a runtime method-set check, not a concrete-type identity check.",
        },
        {
          ...
          explain:
            "Interfaces are comparable at compile time, but if both hold the same non-comparable dynamic type (slice, map, func), the equality check panics at runtime with 'comparing uncomparable type []int'.",
        },
        {
          id: 'switch-order',
          prompt:
            'In a type switch, `int` implements no interfaces here but a `case Stringer` precedes `case int` — a value of dynamic type `int` matches which case?',
          choices: [
            {
          ...
          explain:
            'A type switch evaluates cases top to bottom and takes the first match; since int does not implement Stringer, that case is skipped and case int matches. Ordering matters when interface and concrete cases overlap. (fallthrough is illegal in a type switch.)',
        },
        {
          id: 'switch-var-type',
          prompt:
            'In `switch x := v.(type)` with `case Stringer, io.Reader:`, what is the type of `x` inside that multi-type case?',
          choices: [
            {
