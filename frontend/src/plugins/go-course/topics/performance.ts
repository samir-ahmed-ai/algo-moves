      ...
      quiz: [
        {
          id: 'what-is-bn',
          prompt:
            'What determines the value of b.N inside a benchmark function, and why must the body be a loop over it?',
          choices: [
            {
          ...
          explain:
            'The testing framework calls the function repeatedly with increasing b.N until the timed body accumulates roughly the benchmark time (default ~1s), then reports ns/op = total time / b.N. The loop lets it amortize a stable per-op cost. -count controls how many times each benchmark is re-run, not b.N.',
        },
        {
          id: 'resettimer-purpose',
          prompt:
            'A benchmark does expensive setup (loading a file) before its loop but does NOT call b.ResetTimer(). What is the concrete consequence?',
          choices: [
            {
          ...
          explain:
            'The timer starts when the function is entered, so pre-loop work is included in the measured elapsed time. Since ns/op divides by b.N, the fixed setup cost is amortized differently at each b.N, producing unstable, inflated results unless ResetTimer clears it.',
        },
        {
          id: 'sink-elimination',
          prompt:
            'Why can a benchmark that computes a pure result and ignores it report an absurdly low or zero ns/op?',
          choices: [
            {
          ...
          explain:
            "If the result never escapes or is observed, the optimizer may prove the call has no effect and eliminate it entirely, timing an empty loop. Assigning to a package-level sink forces the compiler to treat the result as observed; alternatively the Go 1.24+ 'for b.Loop() { ... }' form keeps loop-body values alive (via a runtime.KeepAlive intrinsic) so the body cannot be optimized away.",
        },
        {
          id: 'reportallocs-scope',
          prompt:
            'You call b.ReportAllocs() in one benchmark but run the package with -benchmem absent. What is reported for allocations?',
          choices: [
            {
          ...
          explain:
            'b.ReportAllocs() enables allocation reporting for just the benchmark that calls it, independent of the global -benchmem flag. -benchmem enables the same columns for every benchmark in the run; the two are complementary, not mutually required.',
        },
        {
          id: 'benchstat-count',
          prompt:
            "You run 'go test -bench=.' once into old.txt, change code, run once into new.txt, and benchstat reports '~' (no significant difference). Why is this expected regardless of the real change?",
          choices: [
            {
          ...
          explain:
            "benchstat performs a statistical test (Mann-Whitney U under its default AssumeNothing model) and needs multiple samples per side to estimate variance. With one measurement each it lacks the samples to reject the null hypothesis, so it prints '~'. Use -count=10 (or more) on both runs.",
        },
        {
          id: 'parallel-timer',
          prompt:
            'In a b.RunParallel benchmark, where should b.ResetTimer() and per-goroutine setup live to produce correct ns/op?',
          choices: [
            {
      ...
      quiz: [
        {
          ...
          explain:
            'The runtime arms a SIGPROF timer (default 100Hz); each signal records the currently executing stack, so samples reflect on-CPU time, not blocked/off-CPU time.',
        },
        {
          id: 'flat-vs-cum',
          ...
          explain:
            "Flat is time in the function's own instructions; cum includes time in everything it calls. Low flat with high cum means the cost is in its callees, so optimizing this function's body won't help.",
        },
        {
          id: 'inuse-vs-alloc',
          prompt:
            'You have a suspected memory leak. Which heap profile sample type should you inspect?',
          choices: [
            {
          ...
          explain:
            'inuse_space/inuse_objects report live (unfreed) memory, which is what grows in a leak. alloc_space/alloc_objects are cumulative since start and stay high even for short-lived garbage.',
        },
        {
          id: 'heap-sampling-rate',
          prompt:
            'Why might a small but frequently-allocated struct be underrepresented in a default heap profile?',
          choices: [
            {
          ...
          explain:
            'The heap profiler samples on a byte interval (runtime.MemProfileRate, default 512KiB). Rare allocations of small objects may fall between sample points; results are statistically scaled but individual small hotspots can be under-sampled unless the rate is lowered.',
        },
        {
          id: 'http-pprof-import',
          prompt:
            'What is the effect of `import _ "net/http/pprof"` in a server with a default mux?',
          choices: [
            {
          ...
          explain:
            "The blank import runs the package's init, which registers /debug/pprof/* handlers on http.DefaultServeMux. It does not start any profiling; profiles are captured on demand when those endpoints are hit.",
        },
        {
          id: 'cpu-vs-blocked',
          prompt:
            'A goroutine spends most of its time blocked on a channel receive. Where does it show up in a CPU profile?',
          choices: [
            {
          ...
          explain:
            'CPU profiling only samples goroutines actually running on a CPU. Time blocked on channels, mutexes, syscalls, or I/O is invisible there; you need the block or mutex profile (or execution tracer) to see off-CPU waits.',
        },
      ],
      design: {
        prompt:
          "You need continuous profiling for a fleet of latency-sensitive Go services in production. Design an approach covering which profiles to collect, how to capture them safely, overhead considerations, and how you'd surface regressions.",
        answer:
      ...
      quiz: [
        {
          id: 'builder-grow',
          prompt:
            'You call b.Grow(n) on a strings.Builder before writing exactly n bytes with WriteString, then call b.String(). How many heap allocations does the write phase incur, and does String() copy?',
          choices: [
            {
          ...
          explain:
            'Grow reserves the backing []byte once; writing n bytes never re-grows. Builder.String() does an unsafe []byte-to-string conversion of that same buffer, so no copy occurs, though the buffer itself was heap-allocated once.',
        },
        {
          id: 'iface-box',
          prompt:
            'Why can passing an int to a func(v any) parameter in a hot loop cause a heap allocation, whereas passing it to func(v int) does not?',
          choices: [
            {
          ...
          explain:
            'An interface value is two words: a type word plus a data word. A non-pointer like int must be boxed, and the runtime allocates to hold it unless the compiler proves it does not escape (small-int values 0-255 use a static cache). The concrete int parameter stays on the stack. The interface header is 16 bytes, not 24, so that distractor is wrong.',
        },
        {
          id: 'inline-budget',
          prompt:
            "Go's mid-stack inliner assigns each function a cost budget (~80 nodes) it must fit under. Which construct historically makes a function non-inlinable and thus prevents inlining into its callers' hot path?",
          choices: [
            {
          ...
          explain:
            'Constructs like closures (and, in older toolchains, certain range/for loops) inflate the node cost or were outright disallowed, pushing a function over budget. A named result or a plain return is cheap, and reading a global does not block inlining.',
        },
        {
          id: 'bce-hoist',
          prompt:
            'In `for i := 0; i < len(s); i++ { _ = s[i] }`, when can the compiler eliminate the bounds check on s[i]?',
          choices: [
            {
          ...
          explain:
            'The prover sees 0<=i (from the init) and i<len(s) (from the condition), so s[i] is provably in range and the check is removed. BCE works on slices and needs no flag; -gcflags=-B disables all bounds checks globally and is unsafe.',
        },
        {
          id: 'prealloc-cap',
          prompt:
            'You do `out := make([]T, 0, n)` then append n elements. Compared to `var out []T` + n appends, what is the concrete benefit?',
          choices: [
            {
          ...
          explain:
            'Preallocating capacity avoids geometric regrowth (each growth allocates a new array and copies), collapsing many allocations into one. It does not force the slice onto the stack, unused capacity in a pointer-typed backing array is still scanned, and append is already amortized O(1) either way.',
        },
        {
          id: 'builder-copy',
          prompt:
            'What runtime check makes copying a strings.Builder value after it has been written to a dangerous bug the API guards against?',
          choices: [
            {
      ...
      quiz: [
        {
          ...
          prompt:
            'On a 64-bit platform, why is `struct{a bool; b int64; c bool}` 24 bytes while reordering to `struct{b int64; a bool; c bool}` is 16 bytes?',
          choices: [
            {
          ...
          explain:
            'int64 requires 8-byte alignment. In the bad layout a leading bool is followed by 7 padding bytes so the int64 lands on an 8-byte boundary, then the trailing bool needs 7 more tail padding bytes to keep the whole struct a multiple of 8, giving 24. Reordering puts the int64 first and packs both bools into the next 8-byte word, dropping the size to 16.',
        },
        {
          id: 'slice-cache-locality',
          prompt:
            'You iterate a large collection of small structs summing one field. Why does `[]T` typically outperform a hand-rolled linked list of `*T` for this scan?',
          choices: [
            {
          ...
          explain:
            'A slice stores elements in one contiguous backing array, so a linear scan streams through cache lines with hardware prefetching; a pointer-linked list chases scattered heap addresses, causing cache misses that dominate the constant factor even though both are O(n).',
        },
        {
          id: 'map-key-slice',
          ...
          explain:
            'Map keys must be comparable, and slices (like maps and funcs) are not comparable, so the program fails to compile; the idiomatic workaround is to key by `string(b)`, which Go can hash and compare by content.',
        },
        {
          id: 'map-iteration-memory',
          prompt:
            'A senior claims `map[int]V` gives cache-friendly sequential iteration like a slice. What is the correct rebuttal?',
          choices: [
            {
          ...
          explain:
            'A Go map is a hash table of buckets whose in-memory arrangement is neither contiguous in key order nor stable; iteration order is randomized and jumps across buckets, so it lacks the sequential locality of a slice regardless of key type, including integer keys.',
        },
        {
          id: 'struct-key-vs-pointer',
          prompt:
            'You want to dedupe (x,y) coordinates. Comparing `map[Point]bool` (Point is a comparable struct) with `map[*Point]bool`, which statement is accurate?',
          choices: [
            {
          ...
          explain:
            'A comparable struct key hashes and compares its field values, so equal coordinates collapse to one entry; a `*Point` key compares pointer identity, so two distinct allocations holding the same coordinates are treated as different keys and dedup fails.',
        },
        {
          id: 'aos-vs-soa',
          prompt:
            'For a hot loop that only reads one field from each element of a huge collection, why can struct-of-arrays (separate `[]field`) beat array-of-structs (`[]T`)?',
          choices: [
            {
