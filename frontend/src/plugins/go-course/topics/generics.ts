  ...
  "concepts": [
    {
      ...
      "quiz": [
        {
          "id": "gcshape-sharing",
          "prompt": "Given `func F[T any](x T)`, which set of instantiations is guaranteed by the current gc implementation to share a single generated function body (stencil)?",
          "choices": [
            {
          ...
          "explain": "gc groups instantiations by GCShape; all pointer-shaped types (*T, and pointer-representable types) collapse to one shape and share a body, whereas distinct non-pointer scalar shapes each get their own stencil."
        },
        {
          "id": "dictionary-role",
          "prompt": "In gc's GCShape stenciling with dictionaries, what does the hidden dictionary argument passed to a shared generic body primarily carry?",
          "choices": [
            {
          ...
          "explain": "Because one body serves many types of the same shape, it can't hardcode type-specific facts; the dictionary supplies runtime type descriptors, itabs for constraint methods, and sub-dictionaries for nested generic calls."
        },
        {
          "id": "vs-interface-monomorph",
          "prompt": "Why can a generic `Max[T cmp.Ordered]` outperform an `any`-based sort helper for a slice of ints, semantically?",
          "choices": [
            {
          ...
          "explain": "An `any` API forces each int into an interface value (boxing and indirection), while the generic keeps T as a concrete int in the stencil, avoiding boxing and the associated allocations and indirection."
        },
        {
          "id": "instantiation-identity",
          "prompt": "For `type Pair[A, B any] struct{...}`, what is true about `Pair[int, string]` versus `Pair[string, int]`?",
          "choices": [
            {
          ...
          "explain": "Each instantiation of a generic type is its own distinct defined type; different type arguments yield unrelated types even when field layouts coincide, so no implicit assignment is allowed."
        },
        {
          "id": "inference-limits",
          "prompt": "Regarding `Sum[float64](floats)` (explicit) versus `Sum(ints)` (inferred), how does gc decide the type argument T?",
          "choices": [
            {
          ...
          "explain": "Type inference derives T from the types of the passed arguments; when arguments pin the type it is inferable, and explicit instantiation is available to disambiguate, document, or drive cases inference cannot resolve (Go does not infer from the return/assignment context)."
        },
        {
          "id": "constraint-method-call",
          "prompt": "Inside a shared (dictionary-backed) stencil, how is a call to a method required by the type parameter's constraint dispatched?",
          "choices": [
            {
    ...
    },
    {
      ...
      "quiz": [
        {
          "id": "tilde-meaning",
          "prompt": "In the constraint `~int | ~string`, what exactly does the `~int` term admit as a valid type argument?",
          "choices": [
            {
          ...
          "explain": "`~int` denotes the set of all types whose underlying type is int, so both `int` and defined types like `type ID int` qualify. Other integer widths have distinct underlying types, and float is unrelated, so neither is admitted by `~int`."
        },
        {
          "id": "tilde-defined-only",
          "prompt": "Why must the type after `~` be an underlying type itself (e.g. `~int` is legal but `~ID` where `type ID int` is not)?",
          "choices": [
            {
          ...
          "explain": "The spec requires the type in a `~T` term to be its own underlying type. Since `ID`'s underlying type is `int`, `~ID` is rejected and `~int` is the correct spelling of that type set."
        },
        {
          "id": "comparable-vs-any",
          "prompt": "A generic function forms `map[K]V` from its type parameter `K`. Which constraint on `K` is both necessary and minimally sufficient in Go 1.26?",
          "choices": [
            {
          ...
          "explain": "Map keys must support == and !=, which is precisely what `comparable` guarantees. `any` does not permit == on a type parameter, `cmp.Ordered` excludes many comparable types such as structs, and a custom Equal method is never used for map keying."
        },
        {
          "id": "strictly-comparable",
          "prompt": "You instantiate a `comparable`-constrained function with an interface type whose dynamic value is a slice, causing a run-time panic on ==. What changed in Go 1.20 that even allowed this instantiation?",
          "choices": [
            {
          ...
          "explain": "Go 1.20 loosened `comparable` so ordinary interface types, which are spec-comparable but not strictly comparable, satisfy it. `==` on such values can still panic at run time when the dynamic type is uncomparable, for example a slice."
        },
        {
          ...
          "explain": "A union term must be a non-interface type, a `~T` term, or an interface with no methods. `fmt.Stringer` declares a method, so it cannot appear in a union; this is a hard syntactic spec rule, not about set overlap."
        },
        {
          "id": "ordered-vs-number",
          "prompt": "`Max` uses `cmp.Ordered` while `Sum` uses a numeric union. Regarding `Sum`'s body `total += x`, which statement about switching `Sum` to `cmp.Ordered` is accurate?",
          "choices": [
            {
          ...
          "explain": "`+` is defined for every type in `cmp.Ordered` (integers, floats, and strings), so `total += x` would compile fine. The problem is semantic: `cmp.Ordered` includes string, where `+` concatenates rather than sums, so a numeric union is used to keep `+=` meaning arithmetic addition uniformly."
        }
      ],
      "design": {
      ...
      "quiz": [
        {
          ...
          "prompt": "Given `func Parse[T any](s string) (T, error)`, why does `x, _ := Parse(\"42\")` fail to compile while `x, _ := Parse[int](\"42\")` succeeds?",
          "choices": [
            {
          ...
          "explain": "Go infers type arguments from the ordinary (typed) function arguments only. T appears solely in the result, and the compiler never uses the return context or the assignment target to infer type parameters, so it must be supplied explicitly."
        },
        {
          "id": "method-type-params",
          "prompt": "Why can't you write `func (r Repo) Fetch[T any](id int) T` on a type in Go 1.26?",
          "choices": [
            {
          ...
          "explain": "Go disallows type parameters on methods entirely (\"method must have no type parameters\"). Put the type parameter on the enclosing type, or make it a top-level generic function."
        },
        {
          ...
          "explain": "When a type parameter is inferred from an untyped constant argument, the constant's default type is used. The default type of the integer literal 3 is int, so T is int."
        },
        {
          ...
          "explain": "To appears only in the return, so it can never be inferred; you must supply it. Supplying only From (partial) still leaves To unresolved, and Go will not read To from the assignment target, so only the fully explicit call compiles."
        },
        {
          "id": "func-literal-inference",
          "prompt": "In `Map(nums, func(n int) string {...})` for `Map[T,U any]([]T, func(T)U)`, how are T and U resolved?",
          "choices": [
            {
          ...
          "explain": "T unifies with the element type of nums ([]int gives T=int), and U unifies with the function literal's declared result type (string). Both come from arguments; the assignment target plays no role."
        },
        {
          "id": "typed-nil-inference",
          "prompt": "For `func First[T any](v T) T`, why does `First(nil)` fail but `First([]byte(nil))` compile?",
          "choices": [
            {
          ...
          "explain": "The untyped nil identifier carries no type, so unification cannot determine T and inference fails. Converting to a typed nil, []byte(nil), gives a concrete argument type, letting T infer as []byte."
        }
      ],
      "design": {
      ...
      "quiz": [
        {
          ...
          "prompt": "Why does Go reject `func (b Box[T]) MapTo[R any](f func(T) R) R` as a compile error?",
          "choices": [
            {
          ...
          "explain": "Go disallows parameterized methods entirely; only top-level functions and type declarations may introduce type parameters, so the R on a method is rejected. Generic receivers like Box[T] are perfectly legal, and R shadowing rules are irrelevant here."
        },
        {
          "id": "switch-on-typeparam",
          "prompt": "Inside `func f[T any](v T)`, why can't you write `switch v.(type)` directly on `v`?",
          "choices": [
            {
          ...
          "explain": "A type switch requires an interface value as its operand; a type parameter is not an interface type, so you must first convert via any(v) to obtain an interface to switch on. The copy from any(v) is real but irrelevant to why the direct switch is rejected."
        },
        {
          "id": "interface-vs-generic",
          "prompt": "You need a function that calls `Write([]byte)` on its argument and returns nothing type-dependent. Interface param vs generic param — which is the better default, and why?",
          "choices": [
            {
          ...
          "explain": "When the function only dispatches a method and returns nothing whose type depends on the input, an interface is simpler and adds no code bloat; generics buy you nothing here, and constraints can require methods just fine, so the anti-generic method-set claim is false."
        },
        {
          "id": "code-bloat",
          "prompt": "Regarding the binary-size cost of generics, what does the Go 1.26 compiler actually do?",
          "choices": [
            {
          ...
          "explain": "Go uses GC-shape stenciling: types with the same underlying memory shape (e.g. all pointer types) share a single instantiation that receives a dictionary, reducing — but not eliminating — the code duplication of full monomorphization."
        },
        {
          "id": "dictionary-cost",
          "prompt": "A generic function instantiated for pointer-shaped types receives a runtime dictionary. What is the practical performance consequence versus a hand-written concrete version?",
          "choices": [
            {
          ...
          "explain": "Shared (dictionary-based) instantiations add indirection for method calls and type info, so a generic function can be measurably slower than a specialized concrete implementation — a reason not to reach for generics in hot paths. It does not add interface-style double-dispatch, and dictionaries are not erased."
        },
        {
          "id": "constraint-not-union-method",
          "prompt": "You write a constraint `interface{ ~int | ~string }` and try to call a method on values of that type param. What happens?",
          "choices": [
            {
          ...
          "explain": "A constraint that is a union of type terms permits operators shared by those types but exposes no methods; to call a method you must add it to the interface constraint explicitly, otherwise it is a compile-time error — there is no inferred method set and no auto-promotion."
        }
      ],
      "design": {
