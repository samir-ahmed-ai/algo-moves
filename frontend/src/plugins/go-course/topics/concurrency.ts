  ...
  "concepts": [
      ...
      "walkthrough": [
        {
          ...
          "caption": "GOMAXPROCS(4) sets the number of logical processors (P) to 4, returning the previous value, so at most 4 goroutines run Go code simultaneously.",
          "focus": [
            "prev := runtime.GOMAXPROCS(4)"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "NumCPU reports the number of logical CPUs usable by the current process, which is independent of the GOMAXPROCS cap we just set.",
          "focus": [
            "runtime.NumCPU()",
            "previous GOMAXPROCS=%d"
          ],
          "state": [
            {
              "k": "P",
        ...
        },
        {
          ...
          "caption": "WaitGroup counter is set to 100,000 up front so main can later block until every spawned goroutine calls Done.",
          "focus": [
            "const goroutines = 100_000",
            "wg.Add(goroutines)"
          ],
        ...
        },
        {
          ...
          "caption": "Each 'go' creates a lightweight goroutine (starting ~2KB stack) queued onto a P's local run queue, far exceeding the 4 P's so most G's wait runnable.",
          "focus": [
            "go func(n int) {",
            "}(i)"
          ],
        ...
        },
        {
          ...
          "caption": "A runnable G is bound to a P and executed on an M (OS thread); when a P's local queue empties, an idle P steals half the runnable G's from another P's queue.",
          "focus": [
            "sum := 0",
            "sum += j"
          ],
        ...
        },
        {
          ...
          "caption": "The tight for loop is preemptible: the sysmon thread signals long-running G's with SIGURG so the scheduler can preempt at an async-safe point and give other G's a turn.",
          "focus": [
            "for j := 0; j < n%64; j++ {"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "completed.Add(1) is a lock-free atomic increment, so 100,000 concurrent G's can record completion without a mutex or data race.",
          "focus": [
            "completed.Add(1)",
            "defer wg.Done()"
          ],
        ...
        },
        {
          ...
          "caption": "wg.Wait parks main until the counter hits zero, then NumGoroutine reflects the drained population (near 1 plus runtime helpers) since the workers have exited.",
          "focus": [
            "wg.Wait()",
      ...
      "walkthrough": [
        {
          ...
          "caption": "main creates an unbuffered channel a and a buffered channel b with capacity 3, so b can hold three values before a send blocks.",
          "focus": [
            "a := make(chan int)",
            "b := make(chan int, 3)"
          ],
        ...
        },
        {
          ...
          "caption": "Two producer goroutines start, each with a send-only view (chan<- int) of its channel; the compiler forbids them from receiving.",
          "focus": [
            "go producer(a, 3)",
            "go producer(b, 3)"
          ],
        ...
        },
        {
          ...
          "caption": "merge makes the unbuffered out channel, adds 2 to the WaitGroup, and starts one drain goroutine per input plus a closer goroutine.",
          "focus": [
            "out := make(chan int)",
            "wg.Add(2)",
            "go drain(a)",
        ...
        },
        {
          ...
          "caption": "Each drainer receives with comma-ok; while the channel is open and delivering, ok is true and v holds the produced value, which is forwarded to out.",
          "focus": [
            "v, ok := <-c",
            "out <- v"
          ],
        ...
        },
        {
          ...
          "caption": "After sending all n values, each producer calls close on its send-only channel; already-buffered values stay drainable after the close.",
          "focus": [
            "close(out)"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "Once a channel is drained and closed, the comma-ok receive returns immediately with the zero value and ok=false, so the drainer hits the !ok branch and returns.",
          "focus": [
            "if !ok {",
            "return // closed channel: ok is false, v is zero value"
          ],
        ...
        },
        {
          ...
          "caption": "When both drainers have returned, wg.Wait unblocks and the closer goroutine closes out; sending on a closed channel would panic, but only the closer touches out now.",
          "focus": [
            "wg.Wait()",
            "close(out)"
          ],
        ...
        },
        {
          ...
          "caption": "The range loop in main pulls every forwarded value into sum and terminates cleanly when out is closed, printing the total 0+1+2+0+1+2 = 6.",
          "focus": [
      ...
      "walkthrough": [
        {
          ...
          "caption": "Two buffered channels each get one value, then both are closed so their buffered value is still drainable but no more can be sent.",
          "focus": [
            "a := make(chan int, 2)",
            "a <- 1",
            "close(a)"
        ...
        },
        {
          ...
          "caption": "A 50ms deadline context is created; cancel is deferred so the timer resources are released on return.",
          "focus": [
        ...
        },
        {
          ...
          "caption": "Inside merge, ctx.Done() is not ready but both a and b have a buffered value ready, so select picks one of the two ready receives at random.",
          "focus": [
            "for a != nil || b != nil",
            "select {",
            "case v, ok := <-a:"
        ...
        },
        {
          ...
          "caption": "The chosen channel yields its buffered value with ok=true, which is appended to got.",
          "focus": [
            "got = append(got, v)"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "On the next receive from the same channel it is now closed and empty, so ok=false; setting the local variable to nil permanently disables that select case instead of busy-spinning.",
          "focus": [
            "if !ok {",
            "a = nil // disable this case, don't spin on the closed channel",
            "continue"
        ...
        },
        {
          ...
          "caption": "After both a and b are drained and set to nil the loop condition becomes false, merge returns both values, and the count is printed as 2.",
          "focus": [
            "return got",
            "fmt.Println(\"merged count:\", len(res))"
          ],
        ...
        },
        {
          ...
          "caption": "trySend on an unbuffered channel with no receiver cannot proceed, so the default case runs immediately and it returns false without blocking.",
          "focus": [
            "full := make(chan int) // unbuffered, no receiver",
            "case ch <- v:",
            "default:"
        ...
        },
        {
          ...
          "caption": "Because merge drained instantly and returned well under 50ms, ctx.Err() is still nil, so errors.Is against DeadlineExceeded is false and \"deadline hit\" is not printed.",
          "focus": [
      ...
      "walkthrough": [
        {
          ...
          "caption": "A cancellable parent context is derived from Background, and its cancel is deferred so the context tree is always released on return.",
          "focus": [
            "parent, cancel := context.WithCancel(context.Background())",
            "defer cancel()"
          ],
        ...
        },
        {
          ...
          "caption": "A child is derived from parent with a 50ms timeout, and childCancel is deferred to release the timer even if the deadline never fires.",
          "focus": [
            "child, childCancel := context.WithTimeout(parent, 50*time.Millisecond)",
            "defer childCancel()"
          ],
        ...
        },
        {
          ...
          "caption": "worker(child, 200ms) enters a select that blocks on either its 200ms timer (time.After) or the child context's Done channel.",
          "focus": [
            "case <-time.After(d):",
            "case <-ctx.Done():"
          ],
        ...
        },
        {
          ...
          "caption": "At ~50ms the child's deadline expires, closing Done before the 200ms timer, so worker returns ctx.Err() which is DeadlineExceeded.",
          "focus": [
            "return ctx.Err()"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "The child error is printed and errors.Is confirms it matches context.DeadlineExceeded, the sentinel to test against rather than string-matching.",
          "focus": [
            "fmt.Println(\"child:\", err, errors.Is(err, context.DeadlineExceeded))"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "A new context gc is derived directly from parent (not from the already-expired child), inheriting parent's cancellation.",
          "focus": [
            "gc, gcCancel := context.WithCancel(parent)",
            "defer gcCancel()"
          ],
        ...
        },
        {
          ...
          "caption": "Calling cancel() on parent cancels every context derived from it, so gc.Done() closes even though its own gcCancel was never called.",
          "focus": [
            "cancel()"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "worker(gc, time.Hour) selects on gc.Done() which is already closed, returning Canceled immediately instead of waiting an hour; errors.Is confirms context.Canceled.",
          "focus": [
            "case <-ctx.Done():",
      ...
      "walkthrough": [
        {
          ...
          "caption": "A Cache is allocated with its zero value; the embedded RWMutex and Once are immediately usable without any constructor, and data is still a nil map.",
          "focus": [
            "c := &Cache{}"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "Before each goroutine is launched, wg.Add(1) increments the WaitGroup counter on the parent goroutine, so the count is raised before the goroutine runs and can never race with wg.Wait().",
          "focus": [
            "wg.Add(1)",
            "go func(n int) {"
          ],
        ...
        },
        {
          ...
          "caption": "Each goroutine receives its own copy of i via the (n int) parameter, so all five see distinct values 0..4 rather than sharing one variable.",
          "focus": [
            "}(i)",
            "defer wg.Done()"
          ],
        ...
        {
          "title": "Once.Do races",
          "caption": "All goroutines call c.init(), but sync.Once guarantees the map is created exactly once even under concurrent Set calls; every other caller blocks until that first initialization returns.",
          "focus": [
            "c.once.Do(func() { c.data = make(map[string]int) })"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "Set takes the full write lock so only one goroutine mutates the map at a time; defer c.mu.Unlock() releases it even if the map write were to panic.",
          "focus": [
            "c.mu.Lock()",
            "c.data[k] = v"
          ],
        ...
        {
          "title": "Wait for all",
          "caption": "wg.Wait() blocks main until the counter returns to zero, i.e. all five deferred wg.Done() calls have run, establishing a happens-before edge to every prior Set.",
          "focus": [
            "wg.Wait()"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "Get uses RLock, which many readers could hold simultaneously without blocking each other; here it safely reads k4 after all writes are complete.",
          "focus": [
            "c.mu.RLock()",
            "v, ok := c.data[k]"
          ],
        ...
        {
          "title": "Print result",
          "caption": "The value for k4 (4*4) is printed, confirming the deterministic result guaranteed by the WaitGroup synchronization.",
          "focus": [
            "fmt.Println(v)"
          ],
          "state": [
      ...
      "walkthrough": [
        {
          ...
          "caption": "main creates two unbuffered channels: jobs to fan out work and results to fan in outcomes, so every send blocks until a matching receive.",
          "focus": [
            "jobs := make(chan int)",
            "results := make(chan result)"
          ],
        ...
        },
        {
          ...
          "caption": "The loop starts a fixed set of 4 worker goroutines, calling wg.Add(1) before each go so the WaitGroup counter can never race behind the launches.",
          "focus": [
            "wg.Add(1)",
            "go worker(w, jobs, results, &wg)"
          ],
        ...
        },
        {
          ...
          "caption": "A separate producer goroutine sends jobs 1..10 into the unbuffered jobs channel; each send hands the value directly to whichever worker is ready to receive.",
          "focus": [
            "jobs <- j",
            "for j := 1; j <= numJobs; j++"
          ],
        ...
        },
        {
          ...
          "caption": "Each worker ranges over jobs, computes the triangular sum for its job, and sends the result into the shared results channel, fanning many workers into one stream.",
          "focus": [
            "for j := range jobs {",
            "results <- result{job: j, sum: s}"
          ],
        ...
        },
        {
          ...
          "caption": "Once all 10 jobs are sent, the producer closes jobs; each worker's range loop drains remaining values then exits, letting the deferred wg.Done() fire.",
          "focus": [
            "close(jobs)",
            "defer wg.Done()"
          ],
        ...
        },
        {
          ...
          "caption": "The dedicated closer goroutine blocks on wg.Wait() until all workers have returned, then closes results — the only safe place to close it, since no sender remains.",
          "focus": [
            "wg.Wait()",
            "close(results)"
          ],
        ...
        },
        {
          ...
          "caption": "main's range over results receives every in-flight result, accumulates the total, and the loop ends cleanly when results is closed and empty.",
          "focus": [
            "for r := range results {",
            "fmt.Println(\"total:\", total)"
          ],
    ...
    },
    {
      ...
      "walkthrough": [
        {
          "title": "Launch producers",
          "caption": "An unbuffered channel is created and three worker goroutines are spawned, all blocked trying to send on out because no receiver is ready yet.",
          "focus": [
            "out := make(chan int)",
            "go worker(ctx, id, out)"
          ],
        ...
        },
        {
          ...
          "caption": "The main loop's receive synchronizes with one worker's send, handing over a single value with no data race because the channel provides happens-before ordering.",
          "focus": [
            "case v := <-out:",
            "case out <- id*100 + i:"
          ],
        ...
        },
        {
          ...
          "caption": "The loop repeats until exactly five values are collected; which worker wins each send is nondeterministic, but every handoff is race-free.",
          "focus": [
            "for len(got) < 5 {",
            "got = append(got, v)"
          ],
        ...
        },
        {
          ...
          "caption": "After the loop ends, cancel() closes ctx.Done(); each worker's select then takes the ctx.Done() branch and returns instead of blocking forever on a send nobody reads.",
          "focus": [
            "cancel()",
            "case <-ctx.Done():",
            "return"
        ...
        },
        {
          ...
          "caption": "Without the ctx.Done() case, a worker whose send has no receiver would park permanently on out <- ... — a goroutine leak the runtime never reports because the goroutine is merely blocked, not deadlocked.",
          "focus": [
            "case out <- id*100 + i:"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "The runtime panics 'all goroutines are asleep - deadlock!' only when EVERY goroutine is blocked; here main keeps running and workers can exit, so this never triggers.",
          "focus": [
            "select {",
            "case <-time.After(time.Second):"
          ],
        ...
        },
        {
          ...
          "caption": "This program never closes out, avoiding the send-on-closed-channel and double-close panics — the two hazards the runtime always catches deterministically, unlike races and leaks.",
          "focus": [
            "out := make(chan int)"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "Main prints 5 and the deferred cancel runs (idempotent alongside the explicit cancel), guaranteeing every worker has a shutdown signal as the program exits.",
          "focus": [
            "fmt.Println(len(got))",
            "defer cancel()"
          ],
      ...
      "walkthrough": [
        {
          ...
          "caption": "The main goroutine atomically stores a pointer to the initial immutable Config, so any later Load observes a fully-constructed snapshot.",
          "focus": [
            "var current atomic.Pointer[Config]",
            "current.Store(&Config{Version: 1, Timeout: 30})"
          ],
        ...
        },
        {
          ...
          "caption": "An atomic.Int64 is declared; its zero value is a ready-to-use atomic that will accumulate hits indivisibly across all goroutines.",
          "focus": [
            "var hits atomic.Int64",
            "var wg sync.WaitGroup"
          ],
        ...
        },
        {
          ...
          "caption": "Eight reader goroutines are spawned, each registered on the WaitGroup, all racing to Load the same atomic pointer concurrently.",
          "focus": [
            "for i := 0; i < 8; i++ {",
            "go func() {"
          ],
        ...
        },
        {
          ...
          "caption": "Each reader calls Load to grab a consistent Config pointer without a mutex, then dereferences it safely because the pointee is never mutated after publication.",
          "focus": [
            "cfg := current.Load()",
            "if cfg.Timeout > 0 {"
          ],
        ...
        },
        {
          ...
          "caption": "hits.Add(1) performs a sequentially-consistent read-modify-write, so concurrent increments from all 8 readers never lose an update.",
          "focus": [
            "hits.Add(1)"
          ],
          "state": [
        ...
        },
        {
          ...
          "caption": "The single writer publishes four new immutable Configs by Storing fresh pointers; readers atomically flip between whole snapshots with no torn reads.",
          "focus": [
            "for v := 2; v <= 5; v++ {",
            "current.Store(&Config{Version: v, Timeout: 30 + v})"
          ],
        ...
        },
        {
          ...
          "caption": "The atomic makes each pointer Store/Load indivisible, but a read-then-act sequence spanning multiple Loads is NOT a critical section; also, atomic.Pointer must never be copied after first use, and go vet's copylocks check flags such copies via its embedded noCopy.",
          "focus": [
            "cfg := current.Load()",
            "hits.Add(1)"
          ],
        ...
        {
          "title": "Join and report",
          "caption": "wg.Wait blocks the main goroutine until all 9 tracked goroutines (8 readers + 1 writer) finish, then the final Load reads the last-published version (5) and hits totals 8000 with no data race.",
          "focus": [
            "wg.Wait()",
            "fmt.Println(\"reads:\", hits.Load())"
          ],
