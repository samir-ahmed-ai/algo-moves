      ...
      quiz: [
        {
          id: 'gmp-letters',
          prompt:
            "In Go's GMP model, what do G, M, and P denote, and which one does GOMAXPROCS bound?",
          choices: [
            {
          ...
          explain:
            "P is the scheduling context (a run-queue plus resources) needed to execute Go code; GOMAXPROCS sets the number of P's, capping goroutines running Go code simultaneously — not the total M count.",
        },
        {
          id: 'gmp-threadcount',
          prompt:
            "With GOMAXPROCS=4, why can a program still show far more than 4 OS threads (M's) alive?",
          choices: [
            {
          ...
          explain:
            'When a G enters a blocking syscall, its M blocks with it and hands off P; the runtime creates or wakes another M to keep P busy, so live M count can exceed GOMAXPROCS.',
        },
        {
          id: 'gmp-preempt',
          prompt:
            'A tight loop with no function calls once starved the scheduler. What lets Go 1.26 preempt it?',
          choices: [
            {
          ...
          explain:
            'Since Go 1.14, signal-based (SIGURG) asynchronous preemption interrupts long-running loops at safe points, so a call-free tight loop no longer monopolizes its P.',
        },
        {
          id: 'gmp-steal',
          prompt:
            'What does an idle P do under work-stealing, and from where does it primarily take work?',
          choices: [
            {
          ...
          explain:
            "An idle P first checks the global queue and netpoller, then steals roughly half of a randomly chosen victim P's local runqueue to amortize stealing cost and rebalance load.",
        },
        {
          id: 'gmp-cheap',
          ...
          explain:
            'Goroutines begin with tiny (~2KB) contiguous stacks that grow by copying to a larger segment when needed, and user-space scheduling avoids kernel context-switch cost.',
        },
        {
          id: 'gmp-blocking',
          prompt:
            'A goroutine blocks on a channel receive. How does this differ from blocking in a raw OS syscall for the scheduler?',
          choices: [
            {
      ...
      quiz: [
        {
          id: 'recv-from-closed',
          prompt:
            'A goroutine executes `v, ok := <-ch` on a channel `ch` that has already been closed AND fully drained (buffer empty). What happens?',
          choices: [
            {
          ...
          explain:
            "Receiving from a closed, drained channel never blocks: it yields the element type's zero value with ok=false. Only sending on a closed channel panics; receiving is always safe.",
        },
        {
          id: 'nil-recv',
          prompt:
            'What is the behavior of a receive `<-ch` when `ch` is a nil channel (declared `var ch chan int` and never made)?',
          choices: [
            {
          ...
          explain:
            'Both send and receive on a nil channel block forever. This is deliberate and exploited in select to dynamically disable a case by setting its channel to nil.',
        },
        {
          id: 'close-nil',
          ...
          explain:
            "close(nil) panics with 'close of nil channel', just as closing an already-closed channel panics with 'close of closed channel'. close never returns a value, so it cannot report an error.",
        },
        {
          id: 'buffered-full-send',
          prompt:
            'Given `ch := make(chan int, 2)` with two values already buffered and no active receiver, what does a third `ch <- 3` do?',
          choices: [
            {
          ...
          explain:
            "A buffered channel's cap is fixed; sending when len == cap blocks until a receive frees a slot. Channels never drop values or grow their buffer, and a full send is not a panic.",
        },
        {
          id: 'send-on-closed',
          prompt:
            'After `close(ch)`, another goroutine still holds a reference and executes `ch <- 1`. What happens?',
          choices: [
            {
          ...
          explain:
            "Sending on a closed channel always panics ('send on closed channel'), regardless of buffering. This is why the close-signaling convention is that only the sole sender closes, never receivers.",
        },
        {
          id: 'range-close',
          prompt:
            'A `for v := range ch` loop reads a buffered channel that senders fill and then never close. What is the outcome once all buffered values are consumed?',
          choices: [
            {
          ...
          explain:
            'range over a channel terminates only when the channel is closed and drained. If it is never closed, the loop blocks on the empty channel — a classic goroutine leak / deadlock source.',
        },
      ],
      design: {
        prompt:
          "You are designing a fan-in pipeline where multiple producer goroutines feed one consumer over a shared channel, and the pipeline must support graceful shutdown on a cancellation signal. Who should close the channel, and how do you avoid both 'send on closed channel' panics and goroutine leaks?",
        answer:
      ...
      quiz: [
        {
          id: 'random-ready',
          prompt:
            'Two cases in a select are both ready when the statement executes. How does the runtime decide which one runs?',
          choices: [
            {
          ...
          explain:
            'When multiple cases can proceed, select chooses one uniformly at random (a pseudo-random permutation) to avoid starvation; source order does not affect which ready case wins.',
        },
        {
          id: 'nil-case',
          prompt:
            'Inside a for-select loop you set a channel variable to nil after it closes. What is the effect on that case?',
          choices: [
            {
          ...
          explain:
            'A receive from a nil channel blocks forever, so its select case is never selectable; niling out a channel is the idiom to permanently disable its case.',
        },
        {
          id: 'default-nonblock',
          prompt:
            'A select has one send case on an unbuffered channel with no waiting receiver, plus a default. What happens?',
          choices: [
            {
          ...
          explain:
            'With a default present, select never blocks: an unbuffered send cannot proceed without a ready receiver, so the default case executes immediately.',
        },
        {
          id: 'time-after-leak',
          prompt:
            'You write `case <-time.After(d)` inside a hot for-select loop that mostly takes the other case. What is the main hazard?',
          choices: [
            {
          ...
          explain:
            'Each time.After call allocates a new Timer that stays alive until it fires after d; in a tight loop these accumulate, wasting memory and timer-heap slots — reuse one time.Timer with Reset instead.',
        },
        {
          id: 'empty-select',
          ...
          explain:
            '`select {}` has no runnable case and no default, so it blocks the goroutine permanently (the runtime reports a deadlock if every goroutine is blocked).',
        },
        {
          id: 'eval-order',
      ...
      quiz: [
        {
          id: 'cancel-leak',
          prompt:
            'You call ctx, _ := context.WithTimeout(parent, time.Second) and discard the cancel func. What is the concrete consequence even if the timeout eventually fires?',
          choices: [
            {
          ...
          explain:
            "Discarding cancel keeps the child registered in the parent's children set and its timer armed until the deadline expires, leaking those resources until then; go vet's lostcancel check flags it. Calling cancel releases the timer and detaches the child immediately.",
        },
        {
          id: 'deadline-err',
          prompt:
            'A context created with WithTimeout expires. What does ctx.Err() return, and how should callers test it?',
          choices: [
            {
          ...
          explain:
            'Timeout/deadline expiry sets Err to context.DeadlineExceeded; explicit cancel sets context.Canceled. context.DeadlineExceeded is its own sentinel (distinct from os.ErrDeadlineExceeded), and callers should prefer errors.Is since wrapped errors may not compare equal with ==.',
        },
        {
          id: 'values-misuse',
          prompt:
            'Which use of context.WithValue is the documented anti-pattern that seniors should reject in review?',
          choices: [
            {
          ...
          explain:
            'Context values are for request-scoped data that transits API boundaries (trace IDs, request-scoped identity, loggers), not for passing mandatory arguments a function needs to operate. Using them to smuggle required params makes signatures dishonest and untyped.',
        },
        {
          id: 'parent-propagate',
          prompt:
            "Parent context P has child C (via WithCancel) and grandchild G (derived from C). You call C's cancel func. What happens to P and G?",
          choices: [
            {
          ...
          explain:
            "Cancellation propagates from a context to all contexts derived from it, but never upward to ancestors. Cancelling C closes G's Done() while leaving P live.",
        },
        {
          id: 'done-nil',
          prompt:
            'You call ctx.Done() on a context.Background(). What does the returned channel do in a select?',
          choices: [
            {
          ...
          explain:
            'Background/TODO can never be cancelled, so Done() returns a nil channel; a receive on a nil channel blocks forever, so that single select case is effectively disabled while the other cases still run normally.',
        },
        {
          id: 'deadline-shorten',
          prompt:
            "parent has a deadline 10s from now. You call WithTimeout(parent, 30*time.Second). What is the child's effective deadline?",
          choices: [
            {
          ...
          explain:
            "A derived context can only tighten, never loosen, an ancestor's deadline; WithTimeout effectively records min(parent deadline, requested). The child fires at the parent's 10s mark.",
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          id: 'copy-after-use',
          prompt:
            'A function takes a struct containing a sync.Mutex by value, locks it, and mutates a field. What is the core problem?',
          choices: [
            {
          ...
          explain:
            "A sync.Mutex must not be copied after first use; copying duplicates the internal state, so the copy's lock protects nothing the original sees. go vet flags this via copylocks, but it is not a compile error.",
        },
        {
          id: 'wg-add-placement',
          prompt:
            'Why must wg.Add(1) be called before launching the goroutine rather than inside it?',
          choices: [
            {
          ...
          explain:
            'If Add runs inside the goroutine, Wait in the parent may observe a zero counter and return before the goroutine even increments it, defeating the barrier. Add must happen-before the corresponding Wait.',
        },
        {
          id: 'rwmutex-tradeoff',
          ...
          explain:
            'RWMutex has higher per-operation overhead than Mutex; for tiny critical sections that overhead outweighs any read-parallelism benefit, so a plain Mutex is often faster unless reads are frequent and non-trivial.',
        },
        {
          id: 'once-panic',
          prompt:
            'Inside sync.Once.Do, the function panics. What happens on a later Do call with the same function?',
          choices: [
            {
          ...
          explain:
            'Once.Do sets its done flag via defer, so the call counts as complete even if f panics; it will never invoke f again. If initialization can fail and must retry, Once is the wrong tool.',
        },
        {
          id: 'rlock-reentrant',
          prompt:
            'A goroutine holds RLock and, while holding it, calls RLock again on the same RWMutex. Is this safe?',
          choices: [
            {
          ...
          explain:
            "Go's RWMutex is not reentrant. If a writer is blocked waiting between the two RLock calls, the second RLock blocks to avoid writer starvation, causing a self-deadlock.",
        },
        {
          id: 'zero-value',
      ...
      quiz: [
        {
          ...
          prompt:
            'In a fan-out/fan-in pool where N workers all send to a single unbuffered results channel, which strategy correctly closes results exactly once with no panic and no lost values?',
          choices: [
            {
          ...
          explain:
            'A channel must be closed exactly once by a party that knows all senders are done. Only a dedicated goroutine that does wg.Wait() (after every worker returns) can safely close(results). Each worker closing panics on the second close; the producer closing early would race with in-flight sends.',
        },
        {
          id: 'deadlock-unbuffered',
          prompt:
            'You launch the workers and then, in main, run the producer loop `for j := ...{ jobs <- j }` inline (not in a goroutine) BEFORE starting the results range loop, with unbuffered jobs and results channels. What happens?',
          choices: [
            {
          ...
          explain:
            'Workers finish a job and block on `results <- ...` because nobody is receiving yet. Once N workers are blocked on results, they stop ranging jobs, so the producer blocks on `jobs <- j`, and no goroutine can make progress: deadlock. The fix is to consume results concurrently (or run the producer in its own goroutine).',
        },
        {
          id: 'forgot-close-jobs',
          prompt:
            "A pool works correctly except the producer forgets to `close(jobs)` after sending all work. All results are still received by main's range loop. What is the runtime outcome?",
          choices: [
            {
          ...
          explain:
            "`for j := range jobs` only ends when jobs is closed. Without close, workers block forever waiting for more jobs, so wg.Done() is never called, the closer's wg.Wait() never returns, results is never closed, and main's range loop blocks after the last value: the runtime detects all goroutines asleep and reports deadlock.",
        },
        {
          id: 'done-channel-cancel',
          prompt:
            'To let a worker pool abandon remaining work on cancellation, you add a `done` channel and use `select { case results <- r: case <-done: return }` in workers. What is the essential property of the done channel that makes this a correct broadcast to all workers?',
          choices: [
            {
        ...
        },
        {
          ...
          explain:
            'Concurrency is bounded by how many goroutines consume jobs, which is exactly N. The jobs buffer only decouples producer/consumer timing, not concurrency. GOMAXPROCS caps OS-thread parallelism for running goroutines, not how many are logically in-flight (many can be blocked on I/O).',
        },
        {
          id: 'waitgroup-add-placement',
          prompt:
            "A developer moves `wg.Add(1)` from the launching loop into the top of each worker goroutine, keeping the closer goroutine's `wg.Wait(); close(results)`. Why is this a bug?",
          choices: [
            {
          ...
          explain:
            "wg.Add must happen-before the corresponding wg.Wait. If Add(1) runs inside the worker goroutine, the closer's Wait() can run before any worker has scheduled its Add, observe a counter of 0, return immediately, and close(results) while workers still send — causing a send-on-closed panic. Add must be called before starting the goroutine (or before Wait can run).",
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          ...
          prompt:
            'A team runs their full suite under `go test -race` and it passes green. What can they correctly conclude about data races in the code?',
          choices: [
            {
          ...
          explain:
            'The race detector is a dynamic, happens-before analyzer: it flags races only on memory accesses that actually occur during the run. A clean run proves nothing about code paths or goroutine interleavings never exercised, so it cannot prove absence.',
        },
        {
          id: 'closed-channel-send',
          prompt:
            'Consider these four channel operations. Exactly one of the statements below is a TRUE description of an operation that is guaranteed to panic at runtime. Which is it?',
          choices: [
            {
          ...
          explain:
            "Closing an already-closed channel panics with 'close of closed channel', so this statement is both true and describes a guaranteed panic. Receiving from a closed channel returns the zero value with ok=false (no panic); closing a nil channel actually panics rather than blocking, so that statement is false; sending on a full buffered channel simply blocks.",
        },
        {
          id: 'leak-unreceived',
          prompt:
            'A helper does `ch := make(chan int); go func(){ ch <- compute() }()` then returns after a `select` picks a timeout branch instead of `<-ch`. What happens to the goroutine?',
          choices: [
            {
          ...
          explain:
            "The goroutine blocks permanently on the unbuffered send because no receiver will ever arrive; goroutines are never GC'd while runnable or blocked, so it leaks. A buffered channel of size 1 or a context-aware select is the standard fix.",
        },
        {
          id: 'fatal-deadlock-detect',
          prompt:
            'When does the Go runtime report `fatal error: all goroutines are asleep - deadlock!`?',
          choices: [
            {
          ...
          explain:
            "The runtime's deadlock detector fires only when NO goroutine is runnable — every one is blocked on channels, mutexes, or other sync primitives. A partial deadlock where a background timer, netpoller, or one live goroutine still runs is invisible to it.",
        },
        {
          id: 'single-writer-race',
          prompt:
            'One goroutine only writes an `int` field and another only reads it, with no synchronization. Is this a data race under the Go memory model?',
          choices: [
            {
          ...
          explain:
            'The Go memory model defines a data race as a read and a write (or two writes) to the same location with no happens-before ordering, regardless of hardware atomicity of the store. It is undefined behavior even with a single writer; use atomic.Int64 or a mutex.',
        },
        {
          id: 'range-close-leak',
          prompt:
            'A consumer does `for v := range ch { ... }` but the producers finish and never `close(ch)`. What is the runtime outcome for the consumer?',
          choices: [
            {
          ...
          explain:
            '`range` over a channel terminates only when the channel is closed; if producers exit without closing, the consumer blocks forever on receive and leaks — or triggers a fatal deadlock if it is the last runnable goroutine.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          id: 'seqcst-ordering',
          prompt:
            'Under the Go memory model, what ordering guarantee do the sync/atomic operations (e.g. atomic.Int64.Add, Pointer.Store/Load) provide?',
          choices: [
            {
          ...
          explain:
            "Since Go 1.19 the memory model specifies that atomic operations behave as if executed in some single, total order consistent with each goroutine's program order — i.e. sequentially consistent. Go exposes no relaxed or acquire/release tiers like C++.",
        },
        {
          id: 'racy-vs-atomic',
          prompt:
            'A field is written by one goroutine with atomic.StoreInt64 but read elsewhere with a plain `x := v` (non-atomic). Under Go 1.26 the race detector flags this. Why is the plain read undefined behavior?',
          choices: [
            {
          ...
          explain:
            'The memory model requires that if any concurrent access to a location is a write, all such accesses must be atomic. Mixing an atomic write with a plain read is a data race with undefined behavior regardless of architecture or word size.',
        },
        {
          id: 'copy-atomic',
          prompt:
            'You have `var n atomic.Int64` and write `m := n` to snapshot it. What is the correct assessment?',
          choices: [
            {
          ...
          explain:
            'atomic.Int64 embeds a noCopy marker, so copying it after first use is a bug that go vet reports. It still compiles, but the copy is not a valid atomic. To read the value use n.Load().',
        },
        {
          id: 'atomics-vs-mutex',
          ...
          explain:
            'Atomics excel when the entire operation is one machine word (a counter, a flag, a pointer swap): a single LOCK-prefixed instruction with no blocking or context switch. They cannot protect a multi-word invariant, which still needs a mutex.',
        },
        {
          id: 'add-return',
          prompt:
            "Two goroutines each call `atomic.AddInt64(&c, 1)` 1000 times on a zero-valued c. What can you rely on for the final value and each call's return?",
          choices: [
            {
          ...
          explain:
            'AddInt64 is an atomic read-modify-write returning the new (post-add) value; no updates are lost so the sum is exactly 2000. Because each add is a distinct RMW step, every return value is unique — no two calls can return the same number, so returns can never repeat.',
        },
        {
          id: 'pointer-publish',
          prompt:
            "A writer builds a fully-initialized struct then does `p.Store(ptr)` (atomic.Pointer); a reader does `cfg := p.Load()` and dereferences it. Are the struct's fields safely visible to the reader?",
          choices: [
            {
          ...
          explain:
            'An atomic Store observed by an atomic Load establishes a happens-before edge, so all writes sequenced before the Store (the struct initialization) are visible after the Load. This is the standard lock-free immutable-snapshot publication pattern.',
        },
      ],
      design: {
        prompt:
          'You maintain a service whose configuration (dozens of fields) is reloaded occasionally but read on every request across thousands of goroutines. Compare using an atomic.Pointer[Config] snapshot versus a sync.RWMutex guarding a mutable Config. Which do you recommend and what are the tradeoffs?',
        answer:
