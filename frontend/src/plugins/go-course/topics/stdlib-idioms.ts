      ...
      quiz: [
        {
          id: 'read-n-and-err',
          prompt:
            'A correct io.Reader implementation calls into an underlying source. Which behavior must a caller of Read be prepared to handle per the io.Reader contract?',
          choices: [
            {
          ...
          explain:
            "The io.Reader contract explicitly permits returning n>0 bytes AND a non-nil err (including io.EOF) in the same call; callers must consume the n bytes first, then handle err. (0, nil) is discouraged but not forbidden, so 'must never happen' is wrong.",
        },
        {
          id: 'copy-eof',
          prompt:
            'io.Copy(dst, src) returns after the source is exhausted. What does its returned error value reflect on a normal successful completion?',
          choices: [
            {
          ...
          explain:
            'io.Copy treats io.EOF as the normal termination signal and returns nil on success; it only returns a non-nil error for real read/write failures or a short write.',
        },
        {
          id: 'bufio-flush',
          prompt:
            'You wrap a net.Conn with bufio.NewWriter and issue several Write calls followed by returning from the handler. Why might the peer never receive the final bytes?',
          choices: [
            {
          ...
          explain:
            'bufio.Writer accumulates data in memory and only pushes to the underlying writer when the buffer fills or Flush/Close is called; forgetting Flush leaves the tail buffered and never sent.',
        },
        {
          id: 'scanner-token-limit',
          prompt:
            "bufio.Scanner reads a stream where one 'line' is 128 KB with no newline. With default settings, what happens?",
          choices: [
            {
          ...
          explain:
            'bufio.Scanner has a default max token size of bufio.MaxScanTokenSize (64 KB); a longer token makes Scan stop and return false, with Err reporting bufio.ErrTooLong unless you raise the limit via Buffer().',
        },
        {
          id: 'multiwriter-error',
          prompt:
            'io.MultiWriter(a, b, c) is used as an io.Copy destination. Writer b returns an error mid-stream. What happens?',
          choices: [
            {
          ...
          explain:
            'MultiWriter writes to each writer in order and returns on the first error without calling the remaining writers, so c is not written for that call and Copy aborts.',
        },
        {
          id: 'limitreader-eof',
          prompt:
            'You wrap a body with io.LimitReader(r, 1024) to cap reads. The underlying r actually holds 5000 bytes. After reading 1024 bytes, what does the next Read return?',
          choices: [
            {
          ...
          explain:
            'io.LimitReader returns io.EOF once N bytes have been read regardless of what remains underneath; when its internal limit hits zero it returns EOF without calling the underlying reader, and it does not close or drain it.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          ...
          prompt:
            'Unmarshaling `{"n": 42}` into a variable of type `interface{}` yields a map whose `"n"` value has what dynamic type?',
          choices: [
            {
          ...
          explain:
            'With no concrete target type, JSON numbers always decode into float64; json.Number requires opting in via Decoder.UseNumber().',
        },
        {
          id: 'unexported-skip',
          prompt:
            'A struct has a field `tag string` (lowercase). What does `json.Marshal` do with it?',
          choices: [
            {
          ...
          explain:
            'encoding/json uses reflection, which cannot read unexported fields, so they are silently skipped in both directions regardless of any tag.',
        },
        {
          id: 'omitempty-zero',
          prompt:
            'For a field `Count int `json:"count,omitempty"``, when is `count` omitted from Marshal output?',
          choices: [
            {
          ...
          explain:
            "omitempty drops a field when it equals its type's zero value; for int that is 0. It cannot distinguish an explicit 0 from an unset field.",
        },
        {
          id: 'omitempty-struct',
          prompt:
            'A field `Meta Info `json:"meta,omitempty"`` where Info is a struct type and the value is the zero struct. What appears in the output?',
          choices: [
            {
          ...
          explain:
            "omitempty only recognizes false, 0, nil pointer/interface, and empty array/slice/map/string; a zero struct is never 'empty', so the object is always emitted. To skip zero structs in Go 1.24+ use the omitzero tag option instead.",
        },
        {
          id: 'embed-promotion',
          prompt:
            "An embedded struct `Base` (no json tag) with tagged fields is marshaled inside an outer struct. How do Base's fields appear?",
          choices: [
            {
          ...
          explain:
            'An anonymous (embedded) struct field without a json tag has its exported fields promoted and flattened into the parent object, as if declared inline. Adding a tag turns it into a nested object.',
        },
        {
          id: 'rawmessage-defer',
      ...
      quiz: [
        {
          id: 'since-ntp-step',
          prompt:
            'You measure a duration with start := time.Now() then time.Since(start). Midway, an NTP daemon steps the wall clock backward by 5 seconds. What does time.Since(start) report?',
          choices: [
            {
          ...
          explain:
            'time.Now() records a monotonic reading alongside the wall clock, and Sub/Since use the monotonic delta when both operands carry it, so NTP steps, DST, and manual clock sets do not corrupt the measurement.',
        },
        {
          id: 'time-after-loop-leak',
          prompt:
            'Inside a hot for-loop compiled with a Go 1.23+ module, each iteration does `select { case <-work: ...; case <-time.After(d): ... }`. What is the senior-level concern under current Go?',
          choices: [
            {
          ...
          explain:
            'Since Go 1.23 an unreferenced, unstopped timer is eligible for garbage collection immediately, so time.After no longer leaks until it fires; the remaining cost in a tight loop is the per-iteration allocation and runtime-timer scheduling overhead. Reusing one time.Timer with Reset, or a Ticker, avoids that churn.',
        },
        {
          id: 'ticker-stop-drain',
          prompt:
            'On a Go 1.23+ module you call ticker.Stop(), then immediately do a non-blocking `select { case <-ticker.C: ... default: }`. Which statement is true?',
          choices: [
            {
          ...
          explain:
            'Stop halts future ticks but never closes C, so you cannot range over it waiting for a close. Since Go 1.23 the channel is unbuffered (cap 0) and Stop guarantees no value prepared before the call is delivered afterward, so the select falls to default rather than reading a stale tick.',
        },
        {
          id: 'round-zero-strip',
          prompt:
            "Two time.Time values come from time.Now(); one is round-tripped through JSON (RFC3339) and back. Comparing with == unexpectedly returns false even for the 'same' instant. Best fix?",
          choices: [
            {
          ...
          explain:
            '== compares struct fields including the monotonic reading, which marshaling drops, so a stamped value never equals its round-tripped copy; use t.Equal(u), which ignores monotonic, or strip it via Round(0). UTC() only changes the location and does not touch the monotonic reading.',
        },
        {
          id: 'timer-vs-ticker',
          prompt:
            'You need a callback to fire repeatedly at a fixed cadence. Which primitive and why?',
          choices: [
            {
          ...
          explain:
            "A Ticker delivers on a schedule and, when the receiver is slow, coalesces missed ticks rather than queueing them. A Timer fires exactly once, and Sleep-in-a-loop drifts because it adds the work's own runtime to every interval.",
        },
        {
          id: 'timer-reset-race',
          prompt:
            'On a Go 1.23+ module you reuse a single timer, calling t.Stop() then t.Reset(d). What is the correct expectation about the classic stale-value hazard?',
          choices: [
            {
          ...
          explain:
            'Since Go 1.23 timer channels are unbuffered (cap 0) and Go guarantees no value prepared before a Stop or Reset is sent or received after it, so the pre-1.23 drain-guard dance is unnecessary. Reset re-arms a stopped timer without panicking, and Stop never closes C.',
        },
      ],
      design: {
        prompt:
          "Design a resilient periodic scheduler that runs jobs every N seconds across a fleet of long-lived worker goroutines. Discuss how you'd handle slow jobs, clock changes, graceful shutdown, and avoiding goroutine/timer leaks under current (Go 1.23+) timer semantics.",
        answer:
          'Use a single time.Ticker per worker rather than time.After-in-a-loop; even though Go 1.23+ garbage-collects unreferenced timers immediately, After-per-iteration still churns allocations and runtime-timer bookkeeping, so a reused Ticker is cheaper and clearer. Always defer ticker.Stop() to halt future ticks promptly and pair the ticker with a context or done channel in the same select so shutdown is prompt; note that Stop is no longer required merely to let the GC reclaim the ticker. For elapsed-time and deadline math rely on time.Since / time.Sub, which use the monotonic clock and are therefore immune to NTP steps, DST, and manual wall-clock changes; reserve wall time for human-facing timestamps and persist them with Round(0) or compare with Equal to avoid monotonic-reading surprises across marshaling. Handle slow jobs by deciding cadence semantics explicitly: a Ticker naturally coalesces missed ticks (fixed-rate, drop overruns), whereas re-arming only after the previous run finishes (Reset after completion) gives fixed-delay with no overlap; overlapping runs need a worker pool or a skip-if-busy guard. The main correctness pitfall is treating Ticker.C as closeable: it is never closed, so never range over it and always select on Done alongside it. On Go 1.23+ the channel is unbuffered (cap 0) and Stop/Reset guarantee no stale tick survives, so the old drain-before-Reset dance is obsolete; just avoid len/cap polling, which now returns 0. Recommendation: one Ticker + context per worker with defer Stop(), monotonic-based timing, and fixed-delay Reset semantics when jobs can outlast the interval.',
      },
      keyPoints: [
      ...
      quiz: [
        {
          ...
          prompt:
            'A function has signature `func f() (err error)` and its body does `defer func() { err = wrap(err) }(); return doWork()`. When does the deferred closure observe/modify `err`?',
          choices: [
            {
          ...
          explain:
            'A `return x` statement first assigns the named result (err = the error from doWork()), then runs deferred calls, then transfers control. Because the result is a named variable, the deferred closure can read and overwrite it, so wrap(err) takes effect on the value actually returned.',
        },
        {
          id: 'defer-args-eval',
          prompt:
            '`i := 0; defer fmt.Println(i); i = 42` — what does the deferred call print, and why?',
          choices: [
            {
          ...
          explain:
            "Deferred call arguments are evaluated immediately when the defer statement executes, not when the deferred function later runs. So i's value 0 is captured; the subsequent i = 42 has no effect. Wrapping the call in a closure `defer func(){ fmt.Println(i) }()` would instead read i at run time and print 42.",
        },
        {
          id: 'defer-in-loop',
          prompt:
            'Inside a single function you loop over 10000 files and write `f, _ := os.Open(path); defer f.Close()` in the loop body. What is the primary defect?',
          choices: [
            {
        ...
        },
        {
          id: 'loopvar-1-22',
          prompt:
            'A function loops `for _, v := range items { defer fmt.Println(v) }` where items = ["a","b","c"]. What prints when the function returns, under both Go 1.22+ and pre-1.22 semantics?',
          choices: [
            {
        ...
        },
        {
          ...
          prompt:
            'Two defers are registered: first `defer a()`, then `defer b()`. The function body panics. Which runs first, and can it stop the panic?',
          choices: [
            {
          ...
          explain:
            'Defers always unwind last-registered-first, so b() runs before a(), even during a panic. A recover() called directly inside b() stops the panicking sequence, after which a() runs normally as unwinding continues and the function returns without re-panicking.',
        },
        {
          id: 'defer-cost',
          prompt:
            "A senior engineer claims 'defer is essentially free in modern Go, so use it everywhere including hot inner loops.' What is the most accurate correction under Go 1.26?",
          choices: [
            {
          ...
          explain:
            'Since Go 1.14 the compiler open-codes defers (a statically known count, up to 8 per function) making them nearly as cheap as a direct call, but defers placed inside loops or with a dynamic count fall back to the slower runtime.deferproc path that can allocate. So the nuance is: cheap in straight-line code, potentially costly in loops or dynamic sites.',
        },
      ],
      design: {
        prompt:
