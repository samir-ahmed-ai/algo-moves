      ...
      quiz: [
        {
          id: 'parallel-loopvar',
          prompt:
            'In Go 1.26, you write a table test where each subtest calls t.Parallel() as its first line, capturing tc from `for _, tc := range cases`. Compared to Go 1.21, what is the observable effect?',
          choices: [
            {
          ...
          explain:
            'Since Go 1.22 the loop variable is scoped per-iteration, so the classic `tc := tc` shadow is unnecessary and every parallel subtest sees its own row. Pre-1.22 all parallel subtests would read the single shared tc after the loop advanced, so they observed the last row (a logic bug).',
        },
        {
          id: 'parallel-run-timing',
          prompt:
            'A parent test calls t.Run("group", ...) whose body launches subtests that each call t.Parallel(). When does code *after* that t.Run("group", ...) call execute relative to those parallel subtests?',
          choices: [
            {
          ...
          explain:
            'A parallel subtest pauses at t.Parallel() until its enclosing t.Run function returns, then the paused siblings run together; the outer t.Run("group") call does not return until all of them complete, so code after it sees them done. This is the standard idiom for grouped parallel cleanup.',
        },
        {
          id: 'cleanup-vs-defer',
          prompt:
            'You register resource teardown inside a helper that a subtest calls. Why prefer t.Cleanup(fn) over `defer fn()` written in that helper?',
          choices: [
            {
          ...
          explain:
            'A defer in the helper fires when the helper returns, often before the subtest body finishes using the resource; t.Cleanup defers teardown to the end of that test/subtest scope. Both run in LIFO order, and neither survives os.Exit.',
        },
        {
          id: 'cmp-vs-reflect',
          prompt:
            'Comparing want/got structs that contain a time.Time and an unexported field, why is cmp.Diff (go-cmp) generally preferred over reflect.DeepEqual for assertions?',
          choices: [
            {
        ...
        },
        {
          ...
          explain:
            'Regenerating golden files without reviewing the diff can silently enshrine a bug as the new expected output. The go tool skips the testdata directory for building and vetting, but test code still reads it at runtime; golden files are arbitrary bytes, not Go source.',
        },
        {
          id: 'subtest-naming',
          prompt:
            'Two table rows have names "add x" and "add/x". How does the testing package expose these for `go test -run`?',
          choices: [
            {
          ...
          explain:
            'testing rewrites spaces to underscores, so "add x" becomes add_x, while a literal slash in a name creates a nested subtest path (add/x). Identical resulting names get a #NN suffix rather than panicking, so these two rows stay distinct anyway.',
        },
      ],
      design: {
        prompt:
      ...
      quiz: [
        {
          id: 'scheduling-order',
          prompt:
            "A parent test calls t.Run for three subtests, each calling t.Parallel() as its first line, and the parent body then executes a log statement after the loop. When does that final log statement run relative to the parallel subtests' bodies?",
          choices: [
            {
          ...
          explain:
            't.Parallel() pauses the subtest and returns control to the parent. The parent runs to completion (including the post-loop log); only when the parent function returns does the framework resume the paused parallel subtests concurrently.',
        },
        {
          id: 'loop-capture',
          prompt:
            "In Go 1.26, the code sample's loop uses `tc` directly inside the parallel closure without a shadow copy. Why is this safe, unlike in Go 1.21?",
          choices: [
            {
          ...
          explain:
            'Go 1.22 changed for-loop semantics so the loop variable is scoped per iteration (a fresh variable each pass). Before that, all closures shared one `tc`, so parallel subtests observed the final value. This is a language change, not a testing/framework behavior.',
        },
        {
          id: 'cleanup-timing',
          prompt:
            'The parent registers t.Cleanup checking that all parallel subtests ran, and the subtests populate `seen` under a mutex. Why does this Cleanup reliably observe a complete map?',
          choices: [
            {
          ...
          explain:
            "A parent's registered Cleanup functions run only after all of its subtests (including parallel ones, which resume after the parent returns) have completed. So by cleanup time every parallel child has updated `seen`. The mutex is needed for the concurrent writes, not for ordering.",
        },
        {
          id: 'helper-line',
          prompt:
            'writeConfig calls t.Helper() and then t.Fatalf on write failure. What is the effect of t.Helper() on that failure report?',
          choices: [
            {
          ...
          explain:
            't.Helper() marks the function so the test framework skips it when computing the file:line shown in failure messages, pointing at the calling test line instead. It does not change control flow; t.Fatalf works in any function whose t belongs to the running goroutine.',
        },
        {
          id: 'tempdir-parallel',
          prompt:
            'Both parallel subtests call writeConfig, which calls t.TempDir(). Can these two parallel subtests collide on the same directory or clash on cleanup?',
          choices: [
            {
          ...
          explain:
            't.TempDir() creates a unique directory per calling *testing.T and auto-removes it when that test (and its cleanups) finish. Each subtest has its own t, so the two parallel subtests get distinct directories with independent, correctly-ordered removal.',
        },
        {
          id: 'fatal-goroutine',
          prompt:
            'Inside a parallel subtest, a developer spawns a goroutine that calls t.Fatalf on error and does not wait for it. What is the primary correctness problem?',
          choices: [
            {
      ...
      quiz: [
        {
          id: 'seed-vs-mutation',
          prompt:
            'During `go test -fuzz=FuzzRoundTrip`, what role do the arguments passed to `f.Add` play?',
          choices: [
            {
          ...
          explain:
            'f.Add supplies the seed corpus: concrete inputs that are run as-is and then used as starting points for coverage-guided mutation. They are not assertions and are not discarded before mutation.',
        },
        {
          id: 'add-type-match',
          prompt:
            'What constraint does `f.Add` impose relative to the `f.Fuzz` callback signature?',
          choices: [
            {
          ...
          explain:
            'Each f.Add call must pass values whose types and order exactly match the fuzzed parameters of the f.Fuzz callback (the parameters after *testing.T); a mismatch panics at run time when the target executes. Multiple f.Add calls are allowed and encouraged.',
        },
        {
          id: 'corpus-persist',
          prompt:
            'A fuzz run discovers an input that makes `t.Fatal` fire. Where does Go persist it?',
          choices: [
            {
          ...
          explain:
            'Failing inputs are minimized and written to testdata/fuzz/FuzzXxx/ inside the package so they can be committed and replayed as ordinary regression tests. The engine-generated (non-failing) corpus lives under $GOCACHE/fuzz, but crashers go to testdata.',
        },
        {
          id: 'no-fuzz-flag',
          prompt:
            'You run `go test` WITHOUT the `-fuzz` flag on a package containing `FuzzRoundTrip`. What happens?',
          choices: [
            {
          ...
          explain:
            'Without -fuzz, the target still runs its seed corpus (f.Add inputs plus any persisted testdata) deterministically, so crashers stay covered on every CI run. Mutation only begins when -fuzz matches the target.',
        },
        {
          id: 'fuzz-arg-types',
          ...
          explain:
            'The fuzzing engine supports only a fixed set of types: []byte, string, bool, the sized int/uint families, byte, rune, float32, and float64. Maps, structs, and interfaces (including any) are not fuzzable arguments.',
        },
        {
          id: 'fuzz-terminal',
          prompt:
            'Why is calling `f.Add` after `f.Fuzz` has run considered a misuse of the fuzz API?',
          choices: [
            {
      ...
      quiz: [
        {
          ...
          prompt:
            "You want to test an http.Handler's routing, status codes, and written body in memory, without paying for real TCP. What is the idiomatic tool?",
          choices: [
            {
          ...
          explain:
            "ResponseRecorder implements http.ResponseWriter in memory; you invoke the handler's ServeHTTP directly and read the recorded status/body/headers with no socket. httptest.NewServer is for testing clients against a real transport, not for exercising a handler in isolation.",
        },
        {
          id: 'fake-vs-mock',
          prompt:
            'For an interface with 12 methods used across many tests, which double minimizes test brittleness while preserving realistic behavior?',
          choices: [
            {
          ...
          explain:
            'A fake is a real, simplified implementation (e.g. an in-memory store) that behaves correctly, so tests assert on outcomes rather than interaction details. Strict mocks couple tests to call sequences, making unrelated refactors break tests.',
        },
        {
          id: 'interface-seam-location',
          prompt:
            'Where should the RateFetcher interface be defined for the cleanest seam under Go conventions?',
          choices: [
            {
          ...
          explain:
            "Go idiom is 'accept interfaces, return structs' and define the interface where it is consumed, keeping it small and letting the producer stay a plain struct. Producer-defined interfaces tend to be wide and force consumers to depend on the whole contract.",
        },
        {
          id: 'httptest-client-tls',
          prompt:
            'You start an httptest.NewTLSServer. Why must the client use srv.Client() rather than http.DefaultClient?',
          choices: [
            {
          ...
          explain:
            'NewTLSServer generates a self-signed certificate; srv.Client() returns a client whose transport is configured to trust exactly that cert. http.DefaultClient would reject it with an x509 verification error.',
        },
        {
          id: 'avoid-globals-di',
          prompt:
            'A package calls http.Get(url) directly via the global default client. What is the primary testability defect?',
          choices: [
            {
          ...
          explain:
            'Reaching for the global default client hard-wires the dependency, leaving no injection point to substitute an httptest server or fake. Depending on an injected *http.Client or a small interface restores the seam. (The default client is in fact concurrency-safe.)',
        },
        {
          id: 'recorder-result-body',
          prompt:
            'After handler.ServeHTTP(rec, req), which expression yields an *http.Response whose Body you read and then Close?',
          choices: [
            {
