...
export const quiz: QuizQuestion[] = [
  ...
  {
    id: 'find-returns',
    prompt: 'What does the find operation return?',
    choices: [
  ...
  {
    id: 'union-does',
    prompt: 'What does the union operation actually do?',
    choices: [
      { label: 'Link roots — merge if different', correct: true },
      { label: 'Point one node straight at the other — skips find' },
  ...
  {
    id: 'cycle-detect',
    prompt: 'How does Kruskal detect that an edge would create a cycle?',
    choices: [
      { label: 'Same representative — already connected', correct: true },
...
export const codePieces: CodePiece[] = [
  ...
  {
    id: 'new-sig',
    code: 'func newDSU(n int) *DSU {',
    role: 'constructor — every node starts in its own singleton set',
  },
  {
    id: 'new-body',
    code: '\tp := make([]int, n)\n\tfor i := range p {\n\t\tp[i] = i\n\t}\n\treturn &DSU{parent: p, rank: make([]int, n)}',
    role: 'parent[i] = i means i is its own root; ranks start at 0',
  },
  {
    id: 'find-sig',
    code: '}\n\nfunc (d *DSU) find(x int) int {',
    role: 'find: return the representative root of x’s set',
  },
  {
    id: 'find-loop',
    code: '\tfor d.parent[x] != x {\n\t\td.parent[x] = d.parent[d.parent[x]] // path compression\n\t\tx = d.parent[x]\n\t}\n\treturn x',
    role: 'walk to the root, halving the path so future finds are faster',
  },
  {
    id: 'union-sig',
    code: '}\n\nfunc (d *DSU) union(a, b int) bool {',
    role: 'union: merge two sets, or report a cycle',
  },
  {
    id: 'union-roots',
    code: '\tra, rb := d.find(a), d.find(b)\n\tif ra == rb {\n\t\treturn false // same set → would form a cycle\n\t}',
    role: 'equal roots ⇒ already connected ⇒ adding this edge makes a cycle',
  },
  {
    id: 'union-rank',
    code: '\tif d.rank[ra] < d.rank[rb] {\n\t\tra, rb = rb, ra\n\t}\n\td.parent[rb] = ra\n\tif d.rank[ra] == d.rank[rb] {\n\t\td.rank[ra]++\n\t}\n\treturn true',
    role: 'union by rank: hang the shorter tree under the taller root',
  },
  {
    id: 'kruskal-sig',
    code: '}\n\n// edges: each {u, v, w}\nfunc kruskal(n int, edges [][3]int) (mst [][3]int, total int) {',
    role: 'Kruskal driver over a disjoint-set',
  },
  {
    id: 'kruskal-sort',
    code: '\tsort.Slice(edges, func(i, j int) bool { return edges[i][2] < edges[j][2] })\n\tdsu := newDSU(n)',
    role: 'sort edges by weight ascending; start with n disjoint sets',
  },
  {
    id: 'kruskal-loop',
    code: '\tfor _, e := range edges {\n\t\tif len(mst) == n-1 {\n\t\t\tbreak\n\t\t}',
    role: 'consider edges cheapest-first; stop once the tree has n-1 edges',
  },
  {
    id: 'kruskal-take',
    code: '\t\tif dsu.union(e[0], e[1]) {\n\t\t\tmst = append(mst, e)\n\t\t\ttotal += e[2]\n\t\t}\n\t}\n\treturn mst, total',
    role: 'union succeeded ⇒ no cycle ⇒ keep the edge and add its weight',
  },
];
