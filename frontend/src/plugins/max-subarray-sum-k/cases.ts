...
export const goodCases: WorkedCase<WindowInput>[] = [
  {
    ...
    answer:
      'Subtract the element leaving the left edge and add the new right element — O(1) per slide instead of recomputing k terms each time.',
  },
];
...
export const badCases: WorkedCase<WindowInput>[] = [
  {
    ...
    answer:
      'Exactly one window — indices 0..k-1. A common bug is sliding before the window is full; here the first valid window is also the last.',
  },
];

export const intro =
  'Fixed-size sliding window: maintain running sum while advancing left and right together.';
