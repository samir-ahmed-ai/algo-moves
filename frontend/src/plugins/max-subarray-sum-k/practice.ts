...
export const quiz: QuizQuestion[] = [
  {
    ...
    choices: [
      { label: 'Add entering, subtract leaving — O(1) update', correct: true },
    ...
    explain:
      'Only one element enters and one leaves per slide — subtract the outgoing value and add the incoming value.',
  },
];
...
export const codePieces: CodePiece[] = [
  {
    ...
    role: 'maximum sum among all contiguous subarrays of length k',
  },
  {
    id: 'init',
    code: '\tsum, best := 0, 0\n\tfor i := 0; i < k; i++ { sum += nums[i] }',
    role: 'seed the first window sum',
  },
  { id: 'best', code: '\tbest = sum', role: 'track the best sum seen' },
  {
    id: 'slide',
    code: '\tfor right := k; right < len(nums); right++ {',
    role: 'slide window: new right index, left = right-k',
  },
