...
export const codePieces: CodePiece[] = [
  {
    ...
    role: 'signature — return the index of target, or -1 if absent',
  },
  {
    id: 'window',
    code: '\tlo, hi := 0, len(nums)-1',
    role: 'window: the answer can only live in [lo, hi]',
  },
  {
    id: 'loop',
    code: '\tfor lo <= hi {',
    role: 'keep going while the window is non-empty (lo == hi still holds one cell)',
  },
  ...
  {
    id: 'hit',
    code: '\t\tcase nums[mid] == target:\n\t\t\treturn mid',
    role: 'exact match → return the index',
  },
  {
    id: 'right',
    code: '\t\tcase nums[mid] < target:\n\t\t\tlo = mid + 1',
    role: 'mid too small → discard the left half, search right',
  },
  {
    id: 'left',
    code: '\t\tdefault:\n\t\t\thi = mid - 1',
    role: 'mid too big → discard the right half, search left',
  },
