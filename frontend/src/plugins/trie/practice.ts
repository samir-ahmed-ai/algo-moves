...
export const quiz: QuizQuestion[] = [
  ...
  },
  {
    id: 'why-over-hashset',
    prompt:
      'Why use a trie instead of a hash set for prefix queries like "does any word start with car"?',
    choices: [
  ...
  },
  {
    id: 'complexity',
    prompt:
      'Why is searching a trie O(L) in the word length, independent of how many words are stored?',
    choices: [
...
export const codePieces: CodePiece[] = [
  ...
  {
    id: 'newtrie',
    code: 'func NewTrie() *Trie { return &Trie{root: newNode()} }',
    role: 'start with an empty root',
  },
  {
    id: 'insert-sig',
    code: 'func (t *Trie) Insert(word string) {\n\tcur := t.root',
    role: 'insert: begin the walk at the root',
  },
  {
    id: 'insert-loop',
    code: '\tfor _, ch := range word {\n\t\tnext, ok := cur.children[ch]',
    role: 'for each character, look for an existing child',
  },
  {
    id: 'insert-create',
  ...
  {
    id: 'search-sig',
    code: 'func (t *Trie) Search(word string) bool {\n\tcur := t.root',
    role: 'search: begin the walk at the root',
  },
  {
    id: 'search-loop',
    code: '\tfor _, ch := range word {\n\t\tnext, ok := cur.children[ch]',
    role: 'follow one child per character',
  },
  {
    id: 'search-miss',
    code: '\t\tif !ok {\n\t\t\treturn false\n\t\t}\n\t\tcur = next\n\t}',
