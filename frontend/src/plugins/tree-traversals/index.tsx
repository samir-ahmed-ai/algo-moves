...
function record({ tree, order }: TreeInput): Frame<TreeState>[] {
  ...
  const intro: Record<Order, string> = {
    ...
    inorder:
      'In-order DFS: visit the left subtree, then the node, then the right — a BST yields sorted output.',
    postorder:
      'Post-order DFS: visit both subtrees first, then the node — useful for deletes and bottom-up sums.',
  ...
  emit(
    'DONE',
    `[${output.join(',')}]`,
    `Traversal complete. ${order} = [${output.join(', ')}].`,
    null,
    [],
    'good',
  );
...
const inputs: SampleInput<TreeInput>[] = [
  ...
  {
    id: 'post',
    label: 'Post-order (L → R → node)',
    value: { tree: sampleTree, order: 'postorder' },
  },
  { id: 'level', label: 'Level-order (BFS)', value: { tree: sampleTree, order: 'levelorder' } },
];

const verdict = (frames: Frame<TreeState>[]) => ({
  ok: true,
  label: frames[0]?.state.order ?? 'traversal',
});
