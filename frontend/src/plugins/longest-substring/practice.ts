...
export const quiz: QuizQuestion[] = [
  {
    id: 'shrink',
    prompt: 'When the incoming character already appears in the current window, what must happen?',
    choices: [
    ...
    explain:
      'The window invariant is "all characters unique". The left edge jumps just past the earlier copy so the duplicate is evicted.',
  },
];
...
export const codePieces: CodePiece[] = [
  {
    ...
    role: 'length of longest substring without repeating characters',
  },
  ...
  { id: 'init', code: '\tleft, best := 0, 0', role: 'window left edge and best length' },
  {
    ...
    role: 'expand window with right pointer',
  },
  {
    id: 'jump',
    code: '\t\tif i, ok := last[s[right]]; ok && i >= left {\n\t\t\tleft = i + 1\n\t\t}',
    role: 'shrink left past previous duplicate',
  },
  { id: 'record', code: '\t\tlast[s[right]] = right', role: 'update last seen index' },
  {
    id: 'best',
    code: '\t\tif right-left+1 > best { best = right - left + 1 }',
    role: 'track maximum window length',
  },
