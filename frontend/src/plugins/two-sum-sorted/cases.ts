...
export const goodCases: WorkedCase<TwoSumInput>[] = [
  {
    ...
    answer:
      'The array is sorted, so the sum at the ends is the largest possible. If it is too big, shrink from the right; if too small, grow from the left — each move discards a whole line of impossible pairs.',
  },
  {
    id: 'ends',
    ...
    answer:
      'With sorted order, the first valid hit from the two-pointer walk is guaranteed to be correct — you never need to backtrack because moving either pointer only monotonically changes the sum.',
  },
];
...
export const badCases: WorkedCase<TwoSumInput>[] = [
  {
    ...
    answer:
      'Every smaller candidate was skipped by moving left rightward; every larger sum was pruned by moving right leftward. When left >= right, every pair was implicitly checked.',
  },
];
