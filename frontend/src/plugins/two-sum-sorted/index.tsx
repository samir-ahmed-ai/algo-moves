...
function record({ values, target }: TwoSumInput): Frame<TwoSumState>[] {
  ...
  emit(
    'INIT',
    `t=${target}`,
    `Array is sorted, so put one pointer at each end. Their sum tells us which way to move: too big → pull right in, too small → push left out.`,
    null,
  );
  ...
  emit(
    'MISS',
    'no pair',
    `The pointers met without hitting ${target}. No two numbers sum to the target.`,
    null,
    'bad',
  );
  return frames;
...
function Inspector({ frame }: InspectorProps<TwoSumState>) {
  ...
  return (
    <VizInspector>
      ...
      <InspectorRow
        k="result"
        v={s.found ? `[${s.found[0]}, ${s.found[1]}]` : s.done ? 'none' : '…'}
      />
    </VizInspector>
  );
}
...
const teaching = wireTeachingStack({
  ...
  practice: {
    quiz,
    codePieces,
    cases: { good: goodCases, bad: badCases, intro, goodLabel: 'two-pointer moves' },
