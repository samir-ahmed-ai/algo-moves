      ...
      const existing: PanelFlowNode[] = [
        {
          ...
          type: 'panel',
          position: { x: 0, y: 0 },
          data: { kind: 'whiteboard', title: 'Whiteboard', runState: 'paused' },
        },
      ];
      ...
      const existing: PanelFlowNode[] = [
        {
          ...
          type: 'panel',
          position: { x: 10, y: 10 },
          data: { kind: 'viz', title: 'Viz', runState: 'paused' },
        },
      ];
      ...
      const existing: PanelFlowNode[] = [
        {
          ...
          type: 'panel',
          position: { x: 0, y: 0 },
          data: { kind: 'whiteboard', title: 'WB', runState: 'paused' },
        },
        {
          id: 'cc-1',
          type: 'panel',
          position: { x: 0, y: 0 },
          data: { kind: 'collab-code', title: 'CC', runState: 'paused' },
        },
      ];
      const incoming = buildInterviewBoardNodes(false);
      const merged = mergeInterviewNodes(existing, incoming);
