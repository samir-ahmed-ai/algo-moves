...
function hasElements(canvas: unknown): canvas is WhiteboardPayload {
  return (
    !!canvas &&
    Array.isArray((canvas as WhiteboardPayload).elements) &&
    (canvas as WhiteboardPayload).elements.length > 0
  );
}
          ...
          return merged.map((n) =>
            (n.data as { kind?: string })?.kind === 'whiteboard'
              ? {
                  ...
                  data: {
                    ...n.data,
                    subDoc: snapshotFromPayload(n.id, 'whiteboard', 1, savedBoard),
                  },
                }
              : n,
          );
