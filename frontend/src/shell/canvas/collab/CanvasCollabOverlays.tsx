...
function DragGhost({ x, y, color }: { x: number; y: number; color: string }) {
  return (
    <div
      ...
      style={{ left: x, top: y, width: 10, height: 10, background: color, opacity: 0.5, zIndex: 25 }}
    />
  );
}
        ...
        >
          <div className="flex flex-col gap-1.5 p-2.5">
            <div className="flex items-baseline justify-between gap-2">
              <span className={cn(chromeText.sm, 'font-semibold text-ink')}>{comment.authorName}</span>
              {comment.resolved && (
              ...
              <div className="mt-1 flex flex-col gap-1.5 border-t border-edge pt-1.5">
                {comment.replies.map((r) => (
                  <div key={r.id} className="flex flex-col gap-0.5">
                    <span className={cn(chromeText.tight, 'font-medium text-ink3')}>{r.authorName}</span>
          ...
          <div className="flex items-center gap-1 border-t border-edge bg-panel2 p-1.5">
            ...
            <button
              ...
              onClick={submitReply}
              title="Send reply"
              className={cn('grid h-6 w-6 shrink-0 place-items-center text-ink3 transition-colors hover:text-accent', RADIUS_CTRL)}
            >
          ...
          <div className="flex items-center justify-end gap-1 border-t border-edge px-1.5 py-1">
            <button
              ...
              className={cn('inline-flex items-center gap-1 px-1.5 py-0.5 text-ink3 transition-colors hover:text-good', chromeText.tight, RADIUS_CTRL)}
            >
            ...
            {isAuthor && (
              <button
                ...
                className={cn('inline-flex items-center gap-1 px-1.5 py-0.5 text-ink3 transition-colors hover:text-bad', chromeText.tight, RADIUS_CTRL)}
              >
