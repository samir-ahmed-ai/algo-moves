...
export function docSignature(
  ...
  const c = comments
    .map((cm) => `${cm.id}:${cm.resolved ? 1 : 0}:${cm.replies.length}:${round(cm.x)},${round(cm.y)}`)
    .join('|');
  return `${n}#${e}#${c}`;
}
...
function preserveLocalData(data: PanelNodeData): Partial<PanelNodeData> {
  ...
  for (const k of Object.keys(data)) {
    if (!(COLLAB_DATA_FIELDS as readonly string[]).includes(k)) out[k] = (data as Record<string, unknown>)[k];
  }
...
export function isCanvasDoc(value: unknown): value is CanvasDoc {
  ...
  return !!d && d.v === 1 && Array.isArray(d.nodes) && Array.isArray(d.edges) && Array.isArray(d.comments);
}
