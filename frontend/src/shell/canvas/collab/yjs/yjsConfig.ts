 ...
 *
 * Transport defaults ON when `VITE_HOCUSPOCUS_URL` is set (opt-out with
 * `VITE_YJS_TRANSPORT=false`). Subdoc transport defaults ON with transport
 * (opt-out with `VITE_YJS_SUBDOC_TRANSPORT=false`).
 *
...
export function isYjsTransportEnabled(): boolean {
  const url = hocuspocusUrl();
  if (!url) return false;
  if (envExplicitFalse('VITE_YJS_TRANSPORT')) return false;
  return true;
}
