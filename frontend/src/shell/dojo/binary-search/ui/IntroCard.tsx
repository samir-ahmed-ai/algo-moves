...
import { LEVEL_IDS } from '../engine/search';
import { useBisectGame } from '../BisectGameProvider';

const KEY_LEGEND: [string, string][] = [
  ['m', 'flip the MIDDLE card of the active range (also ↵ / space)'],
  ['l', 'discard the left half — only after a flip shows a value below the target'],
  ['h', 'discard the right half — only after a flip shows a value above the target'],
  ['x', 'declare "not here" — only once the active range is empty'],
  ['r', 'restart the level'],
];

export function IntroCard() {
  const { level, showIntro, complete, dismissIntro } = useBisectGame();
