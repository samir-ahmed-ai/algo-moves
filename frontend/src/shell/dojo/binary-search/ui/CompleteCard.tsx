...
export function CompleteCard() {
  const { complete, level, actions, progress, nextId, selectLevel, resetLevel } = useBisectGame();
      ...
      >
        ...
        {stars < 3 ? (
          <p className="mt-2.5 text-center text-sm text-ink2">
            Finish in {level.par} actions or fewer for three stars — probe first, let the comparison
            pick the half, and never discard against it.
          </p>
        ) : null}
        {!nextId ? (
          <p className="mt-2.5 flex items-center justify-center gap-1.5 text-center text-sm text-good">
            <Trophy className="h-4 w-4 shrink-0" aria-hidden />
            Dojo complete — you can halve any haystack in log₂(n) probes now!
          </p>
        ) : null}
