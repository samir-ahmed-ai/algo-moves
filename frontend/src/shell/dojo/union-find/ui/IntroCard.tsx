...
import { LEVEL_IDS, levelHasOpType, type UfLevel } from '../engine/dsu';
import { useBridgeGame } from '../BridgeGameProvider';

function keyLegend(level: UfLevel): [string, string][] {
  const legend: [string, string][] = [];
  if (levelHasOpType(level, 'connected')) {
    legend.push(
      ['y', 'yes — the two islands climb to the same root'],
      ['n', 'no — their roots differ: separate trees'],
    );
  }
  if (levelHasOpType(level, 'union')) {
    legend.push(['1–9', "press the winning root — the LARGER tree's root adopts"]);
  }
  if (levelHasOpType(level, 'edge')) {
    legend.push(
      ['u', 'build the bridge — different trees, union by size'],
      ['c', 'cycle — already connected, Kruskal skips it'],
    );
  }
  legend.push(['r', 'restart the level']);
  return legend;
}

export function IntroCard() {
  const { level, showIntro, complete, dismissIntro } = useBridgeGame();
