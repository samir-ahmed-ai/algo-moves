...
const STAR_TITLES: Record<1 | 2 | 3, string> = {
  3: 'Harbor master!',
  2: 'Bridges well built',
  1: 'Level complete',
};

export function CompleteCard() {
  const { complete, level, actions, progress, nextId, selectLevel, resetLevel } = useBridgeGame();
      ...
      >
        ...
        {stars < 3 ? (
          <p className="mt-2.5 text-center text-sm text-ink2">
            Finish in {level.par} actions or fewer for three stars — climb to each root before you
            answer, and always hang the smaller tree under the bigger root.
          </p>
        ) : null}
        {!nextId ? (
          <p className="mt-2.5 flex items-center justify-center gap-1.5 text-center text-sm text-good">
            <Trophy className="h-4 w-4 shrink-0" aria-hidden />
            Dojo complete — union–find is in your hands: roots, ranks, and redundant bridges!
          </p>
        ) : null}
