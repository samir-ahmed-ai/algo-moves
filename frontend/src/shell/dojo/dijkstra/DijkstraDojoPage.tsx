...
import { LEVEL_IDS } from './engine/signal';
import { SignalGameProvider, useSignalGame } from './SignalGameProvider';
import { SignalBoard } from './ui/SignalBoard';
...
function StatusBar() {
  const { level, state, actions, message, error } = useSignalGame();
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="flex flex-wrap items-center justify-center gap-1.5 tabular-nums">
        ...
        <DojoBadge tone="default">
          settled {state.settled.length}/{level.nodes.length}
        </DojoBadge>
      </div>
      <p
        className={cn(
...
function DijkstraDojoPageBody() {
  const { completedCount } = useSignalGame();
  useGameKeyboard();
...
export function DijkstraDojoPage() {
  return (
    <SignalGameProvider>
      <DijkstraDojoPageBody />
    </SignalGameProvider>
  );
}
