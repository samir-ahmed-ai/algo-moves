...
export function IntroCard() {
  const { level, showIntro, complete, dismissIntro } = useForgeGame();
  if (!showIntro || complete) return null;

  const levelNum = LEVEL_IDS.indexOf(level.id) + 1;
  const legend: [string, string][] = [
    ...level.coins.map((coin): [string, string] => [
      String(coin),
      `forge dp[i] via the ${coin}¢ coin — reads dp[i − ${coin}], only when it's a minimum`,
    ]),
    ['x', 'stamp ∞ — only when no coin reaches the cell'],
    ['r', 'restart the level'],
  ];
