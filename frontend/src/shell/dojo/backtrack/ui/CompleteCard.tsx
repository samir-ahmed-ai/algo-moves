...
export function CompleteCard() {
  const { complete, level, actions, progress, nextId, selectLevel, resetLevel } =
    useBacktrackGame();
      ...
      >
        ...
        {stars < 3 ? (
          <p className="mt-2.5 text-center text-sm text-ink2">
            Finish in {level.par} actions or fewer for three stars — foresee dead ends before you
            place.
          </p>
        ) : null}
        {!nextId ? (
          <p className="mt-2.5 flex items-center justify-center gap-1.5 text-center text-sm text-good">
            ...
            Dojo complete — you just lived the backtracking algorithm!
          </p>
        ) : null}
