...
export function CompleteCard() {
  const { complete, level, actions, booked, progress, nextId, selectLevel, resetLevel } =
    useScheduleGame();
    ...
    <div className="vim-overlay">
      ...
      >
        ...
        <p className="mt-1.5 text-center text-sm text-ink2">
          {booked.length} bookings — the true maximum for this day. You matched the optimum.
        </p>
        ...
        {stars < 3 ? (
          <p className="mt-2.5 text-center text-sm text-ink2">
            Finish in {level.par} actions or fewer for three stars — among everything that still
            fits, always book the meeting that ends earliest; every mistake costs an action.
          </p>
        ) : null}
        {!nextId ? (
          <p className="mt-2.5 flex items-center justify-center gap-1.5 text-center text-sm text-good">
            <Trophy className="h-4 w-4 shrink-0" aria-hidden />
            Dojo complete — earliest-end-first greedy is in your calendar for good!
          </p>
        ) : null}
