...
const KEY_LEGEND: [string, string][] = [
  ['p', 'pop the stack top — only when it is strictly shorter than the incoming bar'],
  ['↵', 'push the incoming bar — only when the top is tall enough (or the stack is empty)'],
  ['r', 'restart the level'],
];

export function IntroCard() {
  const { level, showIntro, complete, dismissIntro } = useSkylineGame();
