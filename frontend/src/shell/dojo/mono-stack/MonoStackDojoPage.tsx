...
import { SkylineGameProvider, useSkylineGame } from './SkylineGameProvider';
import { SkylineBoard } from './ui/SkylineBoard';
...
function useGameKeyboard() {
  const { handleKey } = useSkylineGame();
...
function StatusBar() {
  const { level, actions, message, error } = useSkylineGame();
...
function MonoStackDojoPageBody() {
  const { completedCount } = useSkylineGame();
  useGameKeyboard();
