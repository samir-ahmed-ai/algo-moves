...
const KEY_LEGEND: [string, string][] = [
  ['l', 'walk L right — only when the sum is too small'],
  ['h', 'walk R left — only when the sum is too big'],
  ['↵', 'claim the pair — only when the sum hits the target'],
  ['x', 'declare no pair exists — only when L and R meet'],
  ['r', 'restart the level'],
];

export function IntroCard() {
  const { level, showIntro, complete, dismissIntro } = usePincerGame();
