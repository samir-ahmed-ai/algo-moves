...
export function CompleteCard() {
  const { complete, level, actions, message, progress, nextId, selectLevel, resetLevel } =
    useFloodGame();
  ...
  return (
    <div className="vim-overlay">
      ...
      >
        ...
        {message ? <p className="mt-2.5 text-center text-sm text-ink2">{message}</p> : null}
        {stars < 3 ? (
          <p className="mt-2.5 text-center text-sm text-ink2">
            Finish in {level.par} actions or fewer for three stars — every mistake costs an action,
            so always serve the head of the queue: position 1, the oldest discovery.
          </p>
        ) : null}
        {!nextId ? (
          <p className="mt-2.5 flex items-center justify-center gap-1.5 text-center text-sm text-good">
            <Trophy className="h-4 w-4 shrink-0" aria-hidden />
            Dojo complete — the FIFO flood is in your hands now!
          </p>
        ) : null}
