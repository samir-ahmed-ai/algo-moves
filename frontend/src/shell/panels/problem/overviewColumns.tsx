...
export function ProblemStatementColumn({
  ...
  return (
    <aside
      className={cn(
        'problem-statement-column ws-scroll h-full overflow-y-auto bg-panel/25',
        className,
      )}
    >
      <div className="problem-statement-column__inner flex h-full min-h-0 flex-col p-2">
        <div className="problem-study-card relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-edge bg-panel/85 p-[var(--pad)] shadow-[var(--shadow-sm)]">
          {collapse && (
            <button
              ...
              className="problem-collapse-btn absolute right-[var(--gap)] top-[var(--gap)] z-10 grid h-[var(--row)] w-[var(--row)] place-items-center rounded-md border border-edge bg-panel2 text-ink3 transition-colors hover:border-accent/40 hover:bg-panel2 hover:text-ink"
            >
              <ChevronDown className="h-3.5 w-3.5 -rotate-90" />
            </button>
          )}
          <div className="problem-study-card__body ws-scroll min-h-0 flex-1 overflow-y-auto pt-1">
            {children}
          </div>
          {toolbar}
          {footer}
