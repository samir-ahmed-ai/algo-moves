    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <header className="sticky top-0 z-10 flex items-center gap-1 border-b border-edge bg-bg/90 px-3 py-2 backdrop-blur">
          <TabButton active={tab === 'profile'} onClick={() => setTab('profile')} icon={<UserIcon className="h-4 w-4" />}>
            {t.profile.title}
          </TabButton>
          <TabButton active={tab === 'leaderboard'} onClick={() => setTab('leaderboard')} icon={<Trophy className="h-4 w-4" />}>
            {t.leaderboard.title}
          </TabButton>
          <button
            ref={closeRef}
      ...
      {stats.length > 0 ? (
        <ul className="flex flex-col gap-1.5">
          {stats.map((s) => {
            ...
            return (
              <li key={s.game_id} className="flex items-center justify-between rounded-md border border-edge bg-panel/50 px-3 py-2 text-sm">
                <span className="font-medium text-ink">{game ? localizedGameMeta(game, locale).title : s.game_id}</span>
              ...
              return (
                <span
                  ...
                  className={cn(
                    ...
                    has ? 'border-good/50 bg-good/10 text-good' : 'border-edge bg-panel2 text-ink3 opacity-60',
                  )}
                >
                  {a.title}
                </span>
        ...
        {history.length === 0 ? (
          ...
          <ul className="flex flex-col gap-1">
            {history.map((h) => {
              ...
              return (
                <li key={h.id} className="flex items-center justify-between text-sm">
                  <span className="text-ink2">{game ? localizedGameMeta(game, locale).title : h.match.game_id}</span>
                  <span className={won ? 'font-semibold text-good' : 'text-ink3'}>
                    {won ? t.profile.wins : t.profile.losses}
                  </span>
                </li>
...
function StatCell({ label, value, tone }: { label: string; value: number; tone: 'good' | 'bad' | 'accent' }) {
  return (
    ...
    <div className="flex flex-col gap-3">
      <div className="flex gap-1 rounded-[var(--radius)] border border-edge bg-panel2 p-1 text-sm">
        {(['all', 'week', 'today'] as Period[]).map((p) => (
          ...
          >
            {p === 'all' ? t.leaderboard.allTime : p === 'week' ? t.leaderboard.thisWeek : t.leaderboard.today}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <ol className="flex flex-col gap-1">
          {rows.map((r) => (
            <li key={r.profile_id} className="flex items-center gap-3 rounded-md border border-edge bg-panel/50 px-3 py-2">
              <span className="w-6 text-center text-sm font-bold tabular-nums text-ink3">{r.rank}</span>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'border-accent bg-accentbg text-accent' : 'border-edge bg-panel text-ink3 hover:text-ink',
      )}
    >
      {children}
    </button>
