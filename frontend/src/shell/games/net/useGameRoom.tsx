...
import { gameServerWsUrl, makePlayerId } from './gameServer';
...
export function GameRoomProvider({ children }: { children: ReactNode }) {
  ...
  const targetRef = useRef<{ room: string; name: string; pid: string } | null>(null);
  ...
  const openSocket = useCallback((roomCode: string, name: string, pid: string) => {
    ...
    let ws: WebSocket;
    try {
      ws = new WebSocket(gameServerWsUrl(roomCode, name, pid));
    } catch {
      setStatus('error');
    ...
    ws.onclose = () => {
      ...
      if (shouldReconnect.current && retriesRef.current < MAX_RETRIES) {
        // Keep self/peers so the in-progress game stays mounted across a transient
        // blip; a fresh 'welcome' will refresh them on reconnect. Only clear on a
        // terminal give-up (below) or an explicit disconnect().
        retriesRef.current += 1;
        setStatus('connecting');
        const delay = Math.min(800 * retriesRef.current, 4000);
        retryTimer.current = setTimeout(() => {
          const t = targetRef.current;
          if (t && shouldReconnect.current) openSocket(t.room, t.name, t.pid);
        }, delay);
  ...
  const connect = useCallback(
    (roomCode: string, name: string) => {
      ...
      // hands us back the same slot/role.
      const pid = makePlayerId();
      targetRef.current = { room: code, name, pid };
      shouldReconnect.current = true;
      retriesRef.current = 0;
      pendingRelay.current = [];
      pendingState.current = null;
      setRoom(code);
      openSocket(code, name, pid);
    },
    [openSocket],
  );
