...
export function ExplorerSheet({
  ...
  const problemsTitle =
    browseCrumb.category?.title ?? catalog.breadcrumb(activeItemId).topic?.title ?? 'Problems';
  ...
  const filteredAddable = searching
    ? addableKinds.filter((k) => matchesQuery(sidebarSearch, k.title, k.id, nodeCategory(k.id)))
    : addableKinds;
  ...
  const tabs = [
    ...(essentials.length
      ? [{ key: 'Essentials', icon: <Star className="h-3.5 w-3.5" />, items: essentials }]
      : []),
    ...catTabs,
  ];
  ...
  const filteredProblems =
    showProblems && searching
      ? siblingItems.filter((it) => matchesQuery(sidebarSearch, it.title, it.id, it.difficulty))
      : siblingItems;
                    ...
                    return (
                      ...
                      >
                        <span
                          ...
                          title={it.status}
                        />
                        <span className={cn('min-w-0 flex-1 truncate', chromeText.sm)}>
                          {it.title}
                        </span>
                  ...
                  {!currentTab && !searching ? (
                    <div className="explorer-sheet__empty flex flex-col items-center gap-1.5 px-2 py-4 text-center">
                      ...
                      <p className={cn('font-medium text-ink2', chromeText.sm)}>
                        All panels on canvas
                      </p>
                      <p className={cn('leading-snug text-ink3', chromeText.sm)}>
                        Remove a panel to re-add it here.
                      </p>
                    </div>
                  ) : searching ? (
                    groupedPanelResults!.length === 0 ? (
                      <div className={cn('py-2 leading-snug text-ink3', chromeText.sm)}>
                        No matching panels
                      </div>
                    ) : (
