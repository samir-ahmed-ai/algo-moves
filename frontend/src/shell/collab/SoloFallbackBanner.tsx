  ...
  return (
    <div
      className={cn(
        ...
        'solo-fallback-banner',
        RADIUS_CTRL,
      )}
      role="status"
    >
      <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" aria-hidden />
      <p className={cn('solo-fallback-banner__message min-w-0 flex-1 leading-snug', chromeText.sm)}>
        {message}
      </p>
      <button
        type="button"
        onClick={onDismiss}
        aria-label="Dismiss"
        className="solo-fallback-banner__dismiss grid h-5 w-5 shrink-0 place-items-center rounded text-amber-700/80 hover:bg-amber-500/15"
      >
        <X className="h-3 w-3" />
      </button>
    </div>
