...
export function SessionBody() {
  ...
  const inviteUrl = (r: string) =>
    isInterview
      ? buildInterviewInviteUrl(shareBase, r, session.guestToken)
      : buildInviteUrl(shareBase, r);
      ...
      {showQr && room ? (
        ...
        >
          <QRCodeSVG
            ...
            className="text-ink"
          />
          <span className={cn('text-center text-ink3', chromeText.xs)}>
          ...
          <div className="collab-comment-card__actions flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                chromeText.xs,
                c.resolved
                  ? 'text-ink3 hover:bg-panel2 hover:text-ink'
                  : 'text-good hover:bg-goodbg',
              )}
            >
