  ...
  return (
    <button
      ...
      className={cn(
        ...
        isMastered && 'problem-card--mastered',
        isBuilding && inPlan && 'problem-card--in-plan border-accent/60 ring-1 ring-accent/20',
      )}
    >
      <span
        aria-hidden
        className="problem-card__rail pointer-events-none absolute inset-x-0 top-0 h-[3px]"
      ...
      {isMastered && (
        <>
          ...
          />
          <Trophy
            className="problem-card__trophy absolute right-2 top-2.5 h-3.5 w-3.5 text-good"
            aria-hidden
          />
        </>
      )}
      ...
      <span
        aria-hidden
        className="problem-card__corner problem-card__corner--tl pointer-events-none absolute left-1.5 top-2 h-2 w-2 border-l border-t border-edge2 opacity-60"
      />
      <span
        aria-hidden
        className="problem-card__corner problem-card__corner--br pointer-events-none absolute bottom-1.5 right-1.5 h-2 w-2 border-b border-r border-edge2 opacity-60"
      />

      <div className="problem-card__body relative flex flex-1 flex-col items-center justify-center px-3 pb-3 pt-4">
        <div className="problem-card__glyph-wrap relative grid flex-1 place-items-center">
          <span
            aria-hidden
            className="problem-card__glyph-grid pointer-events-none absolute inset-1 opacity-[0.1]"
            style={{ ...gridStyle, backgroundSize: '10px 10px' }}
          />
          <svg
            viewBox="0 0 48 48"
            className="problem-card__glyph relative h-[60px] w-[60px] text-ink2 transition-all duration-200 group-hover:scale-110 group-hover:text-accent group-hover:drop-shadow-[0_0_8px_var(--accent-bg)]"
            fill="none"
            stroke="currentColor"
            strokeWidth={1.7}
            strokeLinecap="round"
        ...
        <div
          className={cn(
            'problem-card__title mt-2 line-clamp-1 font-medium leading-snug text-ink2 group-hover:text-ink',
            chromeText.base,
          )}
        >
          {item.title}
        ...
        {item.summary && (
          <p
            className={cn(
              'problem-card__summary mt-1 line-clamp-2 leading-snug text-ink3',
              chromeText.sm,
            )}
          >
            {item.summary}
        ...
        {st.attempts > 0 && (
          <div className="problem-card__progress mt-1.5 w-full px-1">
            <Meter
              value={st.correct}
              max={st.attempts}
...
export function ProblemGrid({
  ...
  return (
    <div className="problem-grid-shell">
      {showFilters && (
        <div className="problem-grid-filters mb-2 flex flex-wrap items-center gap-[var(--gap)]">
          <span className={cn('problem-grid-filters__label mr-0.5 text-ink3', chromeText.sm)}>
            Filter:
          </span>
          {DIFFS.map((d) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'problem-grid-filters__button transition-opacity',
                  active && 'problem-grid-filters__button--active',
                  !active && diffFilter.size > 0 && 'opacity-40',
                )}
              >
                <Chip
          ...
          {diffFilter.size > 0 && (
            <button
              ...
              className={cn(
                'problem-grid-filters__clear text-accent hover:underline',
                chromeText.sm,
              )}
            >
              Clear
