...
func (h *Hub) Join(code string, c Sender) (Role, bool) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
// SetState stores the shared room state and echoes it to the other player(s).
// New joiners receive the latest state in their welcome message.
