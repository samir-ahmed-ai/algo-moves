...
export function ReactionDuel() {
  ...
  // Host kicks off round 0 once connected, and re-arms via the advance effect.
  useEffect(() => {
    if (!connected || !amHost) return;
    if (phase === 'idle' && myMs === null) armRound(round);
  ...
  // Once both taps for the round are in, score it and show the result.
  useEffect(() => {
    if (phase !== 'tapped' || myMs === null || peerMs === null) return;
    const w = roundWinner(myMs, peerMs);
    setScores((s) => ({
      me: s.me + (w === 'me' ? 1 : 0),
      peer: s.peer + (w === 'peer' ? 1 : 0),
    }));
    setPhase('result');
  }, [phase, myMs, peerMs]);

  // Auto-advance out of the result beat: end the match or start the next round.
  useEffect(() => {
  ...
  }, [phase, scores, round, amHost, armRound]);
...
function TapZone({
  ...
  return (
    <button
      ...
      aria-label={heading}
      className={
        'flex h-56 w-full select-none touch-manipulation flex-col items-center justify-center gap-2 ' +
