  ...
  return (
    <Accordion.Root type="single" collapsible>
      <Accordion.Item value="motions">
        <Accordion.Header>
          <Accordion.Trigger className="group flex w-full items-center justify-between rounded-md px-1 py-0.5 text-left text-[11px] font-medium text-ink2 transition-colors hover:bg-panel2 hover:text-accent">
            Reference — all motions
            <ChevronDown className="h-3 w-3 transition-transform group-data-[state=open]:rotate-180" />
          </Accordion.Trigger>
        </Accordion.Header>
        <Accordion.Content className="overflow-hidden data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
          <ul className="space-y-1 pb-1 pt-1 text-[10px] text-ink3">
            {MOTIONS.map((m) => (
              <li key={m.keys}>
