...
function record({ values, target }: FllInput): Frame<FllState>[] {
  ...
  const emit = (type: string, note: string, caption: string, mid: number | null, tone?: 'good' | 'bad') =>
    frames.push({
      ...
      state: { values, target, phase, lo, hi, mid, dead: dead.slice(), first, last, done: tone != null },
    });
  ...
  emit('FIRST', `search lower bound`, `Phase 1 — searching for the FIRST position of ${target}. Whenever values[mid] ≥ ${target} we keep mid as a candidate and shrink right; otherwise we move past it.`, null);
  lo = 0;
  hi = n - 1;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
  ...
  if (values[lo] === target) {
    first = lo;
    emit('FOUND', `first=${first}`, `Converged at index ${lo} with value ${values[lo]} = ${target}. The FIRST position is ${first}.`, lo);
  } else {
    emit('MISS', `absent`, `Converged at index ${lo} with value ${values[lo]} ≠ ${target}. The target ${target} is not in the array, so the answer is [-1, -1].`, lo, 'bad');
    return frames;
  }
  ...
  lo = 0;
  hi = n - 1;
  emit('LAST', `search upper bound`, `Phase 2 — searching for the LAST position of ${target}. Reset the window. Whenever values[mid] ≤ ${target} we keep mid as a candidate and shrink left.`, null);
  while (lo < hi) {
  ...
  last = lo;
  emit('DONE', `[${first}, ${last}]`, `Converged at index ${lo} with value ${values[lo]} = ${target}. The LAST position is ${last}. Final answer: [${first}, ${last}].`, lo, 'good');
  return frames;
}
  ...
  return (
    <div className="board-area">
      <div className={cn(vizText.sm, 'text-ink3')}>
        sorted · target = <span className="font-mono text-ink">{s.target}</span> · phase ={' '}
        <span className="font-mono text-ink">{s.phase === 'first' ? 'lower bound' : 'upper bound'}</span>
      </div>
