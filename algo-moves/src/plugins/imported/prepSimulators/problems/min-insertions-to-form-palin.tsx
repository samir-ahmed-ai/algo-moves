...
function record({ s }: PalinInput): Frame<PalinState>[] {
  ...
  if (n === 0) {
    emit('DONE', '0 insertions', `The empty string is already a palindrome, so 0 insertions are needed.`, { lps: 0, answer: 0, done: true }, 'good');
    return frames;
  }
...
function View({ frame }: PluginViewProps<PalinState>) {
  ...
  if (s.r !== null && s.r !== s.l) pointers.push({ i: s.r, label: 'r', tone: 'good', place: 'above' });
  const tone = (i: number) => {
  ...
  const cur = s.l !== null && s.r !== null && s.dp[s.l]?.[s.r] !== undefined && s.dp[s.l][s.r] !== NONE ? s.dp[s.l][s.r] : null;
  return (
    <div className="board-area">
      ...
      <div className={cn('mt-1 font-mono', vizText.sm, 'text-ink3')}>
        LPS = {s.lps ?? '…'}
        {s.answer !== null && (
          <span className="text-good"> → insertions = {s.answer}</span>
        )}
      </div>
    </div>
  );
