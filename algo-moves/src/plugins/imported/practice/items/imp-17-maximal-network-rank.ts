  ...
  "quiz": [
    {
      ...
      "choices": [
        {
          "label": "Precompute degree of each city and an edge set, then check all pairs in O(n²)",
          "correct": true
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "The direct road between i and j is counted in both deg[i] and deg[j], so it must be subtracted once to avoid double-counting",
          "correct": true
        },
        {
          "label": "Direct roads are penalised to discourage selecting adjacent cities"
        },
        {
          "label": "The subtraction converts the rank from a sum to a difference"
        },
        {
          "label": "It accounts for the road being bidirectional (counted twice in the roads array)"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "Ensures undirected edges are stored as a single canonical key (min, max), so the lookup `edges[[2]int{i, j}]` with i < j always hits",
          "correct": true
        },
        {
          "label": "Sorts the edge list to enable binary search during the pair-checking loop"
        },
        {
          "label": "Prevents the map from storing duplicate entries for (u, v) and (v, u) separately, saving memory"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "Each unordered pair (i, j) should be evaluated exactly once; i < j prevents checking (i, i) and counting (j, i) twice",
          "correct": true
        },
        {
        ...
        {
          "label": "The edge set only contains edges with u < v, so querying with j < i would always miss"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "0 — every city has degree 0 and no direct edges, so every pair has rank 0",
          "correct": true
        },
        {
        ...
        {
          "label": "A runtime panic because the edges map is never initialised"
        }
      ],
