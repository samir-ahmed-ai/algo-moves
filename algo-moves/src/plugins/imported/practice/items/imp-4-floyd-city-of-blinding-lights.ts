  ...
  "quiz": [
    {
      ...
      "choices": [
        ...
        {
          "label": "Bellman-Ford — single-source shortest paths with negative-weight detection"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "`dist[i][j] = dist[i][k] + dist[k][j]` when that sum is smaller than the current `dist[i][j]`",
          "correct": true
        },
        {
          "label": "`dist[i][j] = dist[i][k] + dist[k][j]` unconditionally"
        },
        {
          "label": "`dist[k][j] = dist[k][i] + dist[i][j]` when that sum is smaller"
        },
        {
          "label": "`dist[i][j] = min(dist[i][k], dist[k][j])` (taking the minimum of the two partial distances)"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "The distance from any node to itself is 0; INF represents 'no known path' for all other pairs before relaxation",
          "correct": true
        },
        {
          "label": "Setting the diagonal to 0 prevents the algorithm from using self-loops as shortcuts"
        },
        {
          "label": "It is required so the outer k loop can terminate early when i == k"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "To ensure that when we relax through k, `dist[i][k]` and `dist[k][j]` already reflect optimal paths using only intermediates 1..k-1",
          "correct": true
        },
        {
          "label": "Because the i and j loops are O(n) each and k must dominate for cache efficiency"
        },
        {
          "label": "The order doesn't matter — all three loops are symmetric and can be reordered freely"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "To signal that no path exists between u and v — `dist[u][v]` was never relaxed below INF",
          "correct": true
        },
        {
          "label": "To handle integer overflow when `dist[u][v]` exceeds the maximum int value"
        },
        {
          "label": "Because -1 is the sentinel for invalid node indices, consistent with bounds checking above"
        },
        {
          "label": "To distinguish unreachable pairs from pairs with zero-weight paths"
        }
      ],
