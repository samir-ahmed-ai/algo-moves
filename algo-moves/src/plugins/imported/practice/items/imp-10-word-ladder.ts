  ...
  "quiz": [
    {
      ...
      "choices": [
        {
          "label": "Unweighted graph BFS to find the shortest transformation sequence length",
          "correct": true
        },
        {
        ...
        {
          "label": "Dijkstra's shortest path on a weighted word graph"
        },
        {
          "label": "Dynamic programming on the edit-distance between word pairs"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "It deletes the word from `wordSet` when it is first enqueued, so lookups remain O(1)",
          "correct": true
        },
        {
          "label": "It maintains a separate `visited map[string]bool` and checks it before enqueuing"
        },
        {
          "label": "It tracks visited words in a sorted slice and uses binary search"
        },
        {
          "label": "It relies on the queue never containing duplicates because single-character edits are unique"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "For each of L positions it tries all 26 letters (skipping the original), checks wordSet, and enqueues matches",
          "correct": true
        },
        {
          "label": "It compares the current word against every remaining word in wordSet and enqueues those differing by one character"
        },
        {
          "label": "It precomputes an adjacency list of all word pairs with edit distance 1 before BFS starts"
        },
        {
          "label": "It generates all anagrams of the current word and intersects them with wordSet"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        ...
        {
          "label": "After enqueuing a neighbor — if the neighbor equals endWord, return steps+1"
        },
        {
          "label": "After each full BFS level completes — check whether endWord is in the queue"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "Returns 0 immediately; without endWord in the set it can never be reached via the BFS neighbor-generation step",
          "correct": true
        },
        {
          "label": "Returns -1; the BFS will exhaust all reachable words without success"
        },
        {
          "label": "Returns 1; beginWord and endWord are treated as directly connected"
        },
        {
          "label": "Returns len(endWord); the length of the word is used as a heuristic"
        }
      ],
