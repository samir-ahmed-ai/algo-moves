  ...
  "quiz": [
    {
      ...
      "choices": [
        {
          "label": "Tarjan's DFS with discovery and low-link arrays",
          "correct": true
        },
        {
        ...
        {
          "label": "Union-Find with edge removal queries"
        },
        {
          "label": "Floyd-Warshall all-pairs reachability"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        ...
        {
          "label": "After recursing into a child v, sets `low[u] = min(low[u], disc[v])`"
        },
        {
          "label": "It stores low-link values in a separate stack and pops them post-order"
        },
        {
          "label": "It does not propagate; each node computes its own low from adjacent disc values"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "To avoid treating the tree edge back to the parent as a back edge, which would incorrectly lower `low[u]`",
          "correct": true
        },
        {
          "label": "To avoid infinite recursion since the graph is directed"
        },
        {
          "label": "Because the parent is already marked visited, so `disc[parent] != -1` would handle it anyway"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        ...
        {
          "label": "The code iterates over all nodes to start new DFS components, so all bridges are found"
        },
        {
          "label": "The code returns an empty slice because connectivity is a precondition"
        },
        {
          "label": "The code panics because `disc[v] == -1` is never set for unreachable nodes"
        }
      ],
