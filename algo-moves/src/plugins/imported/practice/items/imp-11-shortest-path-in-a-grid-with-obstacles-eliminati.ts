  ...
  "quiz": [
    {
      ...
      "choices": [
        {
          "label": "The BFS state is (row, col, remainingEliminations), so each distinct triple is visited at most once",
          "correct": true
        },
        {
          "label": "Dijkstra with edge weight = 1 for empty cells and k+1 for obstacles"
        },
        {
          "label": "DFS with memoization on (row, col) only"
        },
        {
          "label": "A greedy approach that always eliminates obstacles on the Manhattan-distance path first"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "nextRem = rem - grid[nr][nc]; grid value is 1 for obstacle (costs one elimination) and 0 for empty",
          "correct": true
        },
        {
        ...
        {
          "label": "nextRem = rem - grid[nr][nc]² so hitting two obstacles costs 4 eliminations"
        },
        {
          "label": "nextRem = rem only if the cell is empty; the move is blocked if it is an obstacle and rem==0"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "If k is large enough to eliminate every obstacle on the Manhattan path, the shortest possible path (m+n-2 steps) is always achievable",
          "correct": true
        },
        {
          "label": "When k exceeds the grid perimeter, the answer must equal the perimeter length"
        },
        {
          "label": "k >= m+n-3 means the grid has no obstacles, so the direct path is always open"
        },
        {
          "label": "The BFS would overflow its queue at that k value, so an approximation is returned"
        }
      ],
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "Remaining eliminations range from 0 to k inclusive, requiring k+1 distinct indices",
          "correct": true
        },
        {
          "label": "The extra slot stores a sentinel for the starting cell (0,0,k)"
        },
        {
          "label": "k is 0-indexed so valid values are 1 through k, requiring k+1 slots"
        },
        {
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "O(m·n·k) time and O(m·n·k) space",
          "correct": true
        },
        {
          "label": "O(m·n·k²) time and O(m·n) space"
        },
        {
          "label": "O(m·n) time and O(m·n·k) space"
        },
        {
          "label": "O(m²·n²) time and O(m·n·k) space"
        }
    ...
    },
    {
      ...
      "choices": [
        {
          "label": "Returns -1 after the BFS queue empties without hitting the target cell",
          "correct": true
        },
        {
          "label": "Returns 0 to indicate an impossible case"
        },
        {
          "label": "Returns m+n-2 as a fallback estimate"
        },
        {
          "label": "Returns k to indicate the budget was fully consumed"
        }
      ],
