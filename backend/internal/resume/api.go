...
type Store interface {
	...
	CreateResume(ctx context.Context, ownerID, title, filename, contentType string, rawPDF []byte, rawText string, mapping json.RawMessage, isPublic bool) (*Resume, error)
	GetResume(ctx context.Context, id string) (*Resume, error)
	GetResumeForOwner(ctx context.Context, id, ownerID string) (*Resume, error)
	UpdateResume(ctx context.Context, id, ownerID string, title *string, mapping json.RawMessage, isPublic *bool) (*Resume, bool, error)
	DeleteResume(ctx context.Context, id, ownerID string) (bool, error)
	CreateResumeVariant(ctx context.Context, resumeID, ownerID, label, focus, targetRole, mode string, mapping json.RawMessage) (*ResumeVariant, error)
	ListResumeVariants(ctx context.Context, resumeID, ownerID string) ([]ResumeVariant, error)
	DeleteResumeVariant(ctx context.Context, variantID, ownerID string) (bool, error)
}

type ProfileStore interface {
	GetOpenAIKeyForProfile(ctx context.Context, profileID string) (string, bool, error)
}
...
func NewHandler(repo Store, profiles ProfileStore, auth profile.Authenticator, ai *resumeopenai.Client) *Handler {
...
func (h *Handler) HandleCreateResume(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.LogAndWriteErr(w, err, http.StatusBadGateway, "resume parse failed")
		return
	}
...
func (h *Handler) HandleCustomize(w http.ResponseWriter, r *http.Request) {
	...
	case "ai":
		...
		if err != nil {
			httputil.LogAndWriteErr(w, err, http.StatusBadGateway, "ai rewrite failed")
			return
		}
	default:
		customized, err = ReorderMappingForFocus(resume.Mapping, focus)
		if err != nil {
			httputil.LogAndWriteErr(w, err, http.StatusInternalServerError, "customize failed")
			return
		}
	}
	...
	if body.Save {
		...
		if err != nil {
			httputil.LogAndWriteErr(w, err, http.StatusInternalServerError, "save variant failed")
			return
		}
