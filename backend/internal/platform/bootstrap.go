...
func BootstrapPlatformAdmin(ctx context.Context, store *Store) {
	if store == nil {
		return
	}
	...
	if password == "" {
		ok, err := store.SetAdmin(ctx, email)
		if err != nil {
	...
	p, _, err := store.ProfileByEmail(ctx, email)
	if err != nil {
		log.Printf("arcade: admin bootstrap warning: %v", err)
		return
	}
	...
	if p == nil {
		if _, err := store.CreateEmailUser(ctx, email, string(hash), displayName); err != nil {
	...
	} else if err := store.UpdatePasswordHash(ctx, email, string(hash)); err != nil {
